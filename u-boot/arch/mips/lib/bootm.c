...
static void linux_cmdline_append(bootm_headers_t *images)
{
	...
	if (rd_size) {
		sprintf(buf, "rd_start=0x%08lX", rd_start + 0x80000000);    //shift PAGE_OFFSET for Linux check
	...
	if (images->rd_start) {
		sprintf(buf, "rd_start=0x%08lX", images->rd_start + 0x80000000);    //shift PAGE_OFFSET for Linux check
		printf("=>%s", buf);
		linux_cmdline_set(buf, strlen(buf));
		sprintf(buf, "rd_size=0x%lX", images->rd_end - images->rd_start);
		printf(" %s\n", buf);
		linux_cmdline_set(buf, strlen(buf));
	}
}
