		...
		u8 *idcode)
{
	...
	flash->status = spi_flash_cmd_read_status;
...
static struct spi_flash *spi_flash_probe_slave(struct spi_slave *spi)
{
	...
	/* Unlock write protect anyway*/
	ret = spi_flash_cmd(spi, CMD_READ_STATUS, &word, sizeof(word));
	if (ret) {
		printf("SF: Failed to get FLASH_CMD_RDSR\n");
		goto err_read_id;
	}
		...
		unsigned int max_hz, unsigned int spi_mode)
{
	...
	flash = spi_flash_probe_slave(spi);
