...
static int nvt_spinor_reset(struct drv_nand_dev_info *info)
{
	...
	reg = INW(0xC0020000 + 0x84);
	reg |= (0x1);
	OUTW(0xC0020000 + 0x84, reg);
	...
	reg = INW(0xC0020000 + 0x40);
	reg &= ~(0x3F000);
	reg |= (clk_div << 12);
	OUTW(0xC0020000 + 0x40, reg);

	/* Release SRAM */
	reg = INW(0xC0010000 + 0x1000);
	reg &= ~(0x20000);
	OUTW(0xC0010000 + 0x1000, reg);

	/* Config driving */
	reg = INW(0xC0030000 + 0x40);
	reg &= ~(0xC300FF);
	reg |= 0x450055;
	OUTW(0xC0030000 + 0x40, reg);
