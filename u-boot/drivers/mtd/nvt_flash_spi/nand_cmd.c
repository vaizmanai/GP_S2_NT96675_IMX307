...
#define OP_ERASE 0
#define OP_PROGRAM 1
#define FLICKER_BASE_PROGRAM 500
#define FLICKER_BASE_ERASE 2
...
static ER spi_nand_unlock_BP(struct drv_nand_dev_info *info)
{
	...
	if (info->flash_info->spi_nand_status.bBlockUnlocked == FALSE) {
		...
		/*
		___|cmd(0x1F)|___|SR(0x01)(row byte0)|___|SR(0x0)(row byte1)|__
		*/
			...
			uint32_t uiColAddr, uint8_t* pBuf, uint32_t uiBufSize)
{
	...
	UINT8 ucStatus;
	UINT32 uiPageCnt;
	UINT32 uiSpareAreaAddr, *pulBuf_spare;
	UINT32 j;
	...
	UINT32  uiNandSectionCountPerPage = (info->flash_info->page_size \
					/ 0x200);
...
static int spiflash_read_data(struct drv_nand_dev_info *info, u32 addr, u32 byte_size, u8 *pbuf)
{
	...
	ret = spinor_read_status(info, NAND_SPI_NOR_STS_RDCR, &rdcr_status);
	if(ret != E_OK)
		return ret;
#endif
	/*Disable clk_auto_gating before set cs_manual*/
...
#ifdef CONFIG_SF_DEFAULT_SPEED
		/*Dummy 2 T*/
		if (((rdcr_status & 0xC0) == 0xC0) && (CONFIG_SF_DEFAULT_SPEED > 96000000)) {
			printf("SF: dummy 10T mode with RDCR 0x%08x\n", rdcr_status);
			transParam.type = _DUMMY_CLOCK;
			nand_host_setup_transfer(info, &transParam, 0, 0, _SINGLE_PAGE);
...
#if defined(CONFIG_NVT_FW_UPDATE_LED) && !defined(CONFIG_NVT_PWM)
	flicker_base = FLICKER_BASE_PROGRAM;
	led_flicker();
#endif
...
#if defined(CONFIG_NVT_FW_UPDATE_LED) && !defined(CONFIG_NVT_PWM)
	flicker_base = FLICKER_BASE_ERASE;
	led_flicker();
#endif
...
int spiflash_boot_detect(struct drv_nand_dev_info *info)
{
	...
	if (0 == (reg & CPU1_BOOT_MASK)) {
		*(u32*) 0xC029001C |= CPU2_BOOT_MASK;
	}

	return reg & CPU1_BOOT_MASK;
}
