 ...
 * na51023:
 *    NOVATEK's NA51023 chip. This configuration uses an AXI master/DMA bus, an
...
#define SW_RESET_ADDRESS     0xC02B3800
...
#define set_break_link_timer() do { \
        writel(0x53, (void *)0xC02B3A88);\
        writel(0x07, (void *)0xC02B3A8C);\
        writel(0xC9, (void *)0xC02B3A84);\
}while(0)

#define set_led_blinking() do { \
        writel(0x40, (void *)0xC02B3900);\
        writel(0x12, (void *)0xC02B300C);\
}while(0)

/* Core registers */
#define NOVATEK_NA51023_EQOS_BASE 0xC02B0000
#define BIT(x) (1 << (x))
...
int eqos_recv(struct eth_device *eth_dev)
{
    ...
    mb();
	NetReceive((void*)packetp, length);
...
{
	u32 varCGSys = *(uint32_t*)0xC0020010;
    ...
    // global setting
	*(uint32_t*) 0xC0020000 |= 0x40;	    /*Enable PLL6*/
	*(uint32_t*) 0xC0020074 |= 0x20000000;	/*Enable Clock*/
...
int na51023_eth_initialize(bd_t *bis)
{
	struct eth_device *dev;
	...
	dev->priv = eqos;
	eqos->dev = dev;
	eqos->regs = NOVATEK_NA51023_EQOS_BASE;
