...
static int pad_setdrivingsink(u32 name, u32 paddriving)
{
	...
	if ((name >= PAD_DS_CGPIO12) && (name <= PAD_DS_CGPIO19)) {
		dwoffset  = 0x68 - 0x40;
		bitoffset = (name - (PAD_DS_CGPIO12))*4;
	} else {
		dwoffset  = 0x6C - 0x40;
		bitoffset = (name - (PAD_DS_CGPIO20))*4;
	}
...
static int sdiohost_setpaddriving(int id, SDIO_SPEED_MODE mode)
{
	...
	if (id == SDIO_HOST_ID_1) {
		pad_setdrivingsink(PAD_DS_CGPIO13, cmd_uidriving);
		...
		pad_setdrivingsink(PAD_DS_CGPIO17, data_uidriving);
		return pad_setdrivingsink(PAD_DS_CGPIO12, clk_uidriving);
	} else {
		pad_setdrivingsink(PAD_DS_CGPIO19, cmd_uidriving);
		...
		pad_setdrivingsink(PAD_DS_CGPIO23, data_uidriving);
		return pad_setdrivingsink(PAD_DS_CGPIO18, clk_uidriving);
	}
...
{
	if (host->id == SDIO_HOST_ID_1) {
		*(uint32_t*) 0xC0010004 |= 0x4000;
		udelay(10);
		*(uint32_t*) 0xC00100A0 &= ~(0x3F000);
		udelay(10);
		*(uint32_t*) 0xC0020020 &= ~0x30;
		udelay(10);
		*(uint32_t*) 0xC0030074 &= ~0x3;
		udelay(10);
		*(uint32_t*) 0xC0030074 |= 0x10;
		udelay(10);
		*(uint32_t*) 0xC0011000 &= ~0x20000000;
		udelay(10);
		*(uint32_t*) 0xC002003C &= ~(0x7FF);
		udelay(10);
		*(uint32_t*) 0xC002003C |= 0x1F0;
		udelay(10);
		*(uint32_t*) 0xC0020084 &= ~(0x4);
		udelay(10);
		*(uint32_t*) 0xC0020084 |= 0x4;
		udelay(10);
		*(uint32_t*) 0xC0020074 |= 0x4;
	} else if (host->id == SDIO_HOST_ID_2) {
		*(uint32_t*) 0xC0010004 |= 0x8000;
		udelay(10);
		*(uint32_t*) 0xC00100A0 &= ~(0xFC0000);
		udelay(10);
		*(uint32_t*) 0xC0020020 &= ~0x300;
		udelay(10);
		*(uint32_t*) 0xC0030070 &= ~0x3;
		udelay(10);
		*(uint32_t*) 0xC0030070 |= 0x10;
		udelay(10);
		*(uint32_t*) 0xC0011000 &= ~0x40000000;
		udelay(10);
		*(uint32_t*) 0xC002003C &= ~(0x7FF0000);
		udelay(10);
		*(uint32_t*) 0xC002003C |= 0x1F00000;
		udelay(10);
		*(uint32_t*) 0xC0020084 &= ~(0x8);
		udelay(10);
		*(uint32_t*) 0xC0020084 |= 0x8;
		udelay(10);
		*(uint32_t*) 0xC0020074 |= 0x8;
	} else {
		*(uint32_t*) 0xC0020024 &= ~0x3;
		udelay(10);
		*(uint32_t*) 0xC0011000 &= ~0x80000000;
		udelay(10);
		*(uint32_t*) 0xC0020040 &= ~(0x7FF);
		udelay(10);
		*(uint32_t*) 0xC0020040 |= 0x1F0;
		udelay(10);
		*(uint32_t*) 0xC0020084 &= ~(0x4000);
		udelay(10);
		*(uint32_t*) 0xC0020084 |= 0x4000;
		udelay(10);
		*(uint32_t*) 0xC0020074 &= ~0x1;
		udelay(10);
		*(uint32_t*) 0xC0020074 |= 0x4000;
		udelay(10);
		*(uint32_t*) 0xC0030040 = 0x155555;
	}
	...
	sdiohost_setphyclkindly(host, 0x140000);
...
int nvt_mmc_init(int id)
{
	...
	if (id == SDIO_HOST_ID_1) {
		...
		reg = *(uint32_t*) 0xC0010004;
		if (!(reg & 0x20000))
			return E_NOSPT;

		if ((reg & 0x80000000) == 0x80000000)
			enable_8bits = 1;
