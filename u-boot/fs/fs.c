...
static int fs_rename_unsupported(const char *oldname, const char *newname)
{
	printf("** rename unsupported **\n");
	return -1;
}

static int fs_rm_unsupported(const char *filename)
{
	printf("** rm unsupported **\n");
	return -1;
}
...
static struct fstype_info fstypes[] = {
...
#ifdef CONFIG_FS_EXFAT
	{
		...
		.null_dev_desc_ok = false,
		.probe = exfat_set_blk_dev,
		.close = exfat_close,
		.ls = file_exfat_ls,
		.exists = exfat_exists,
		.read = exfat_read_file,
		.write = exfat_write_file,
		.rename = file_exfat_rename,
		.rm = file_exfat_rm,
	},
#endif
	{
...
#ifndef CONFIG_NVT_INGNORE_FSREAD_SIZE_CHECK
...
int fs_rename(const char *oldname, const char *newname)
{
	...
	ret = info->rename(oldname, newname);
...
int do_write(cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[],
		int fstype)
{
	long size;
	unsigned long addr;
	...
	size = fs_write(argv[4], (ulong)buf, 0, count);
	unmap_sysmem(buf);
	if (size == -1) {
...
int do_rm(cmd_tbl_t *cmdtp, int flag,
		int argc, char * const argv[],  int fstype)
{
	if (argc != 4) {
		return cmd_usage(cmdtp);
	}
...
int do_rename(cmd_tbl_t *cmdtp, int flag,
		int argc, char * const argv[], int fstype)
{
	if (argc != 5) {
		return cmd_usage(cmdtp);
