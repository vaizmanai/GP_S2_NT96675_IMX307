	       ...
	       unsigned long maxsize, int dols, int dogetsize)
{
	...
	while (1) {
		...
		for (i = 0; i < DIRENTSPERBLOCK; i++) {
            ...
            if(FAT2CPU32(dentptr->size) > CONFIG_NVT_RUNFW_SDRAM_SIZE) {
                printf("Load RootName: %s over reserve DRAM size 0x%x\n", s_name, CONFIG_NVT_RUNFW_SDRAM_SIZE);
                while(1);
            }
#endif
...
int fat_write_file(const char *filename, void *buf, int offset, int len)
{
	...
	len_write = file_fat_write(filename, buf, len);
	if (len_write == -1) {
