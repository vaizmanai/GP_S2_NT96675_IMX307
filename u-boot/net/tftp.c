...
store_block(int block, uchar *src, unsigned len)
{
	...
	{
		/* fix tftp overwrite u-boot issue */
		cpy_addr = load_addr + offset;
		uboot_end_addr = CONFIG_UBOOT_SDRAM_BASE + CONFIG_UBOOT_SDRAM_SIZE;
		if((cpy_addr < CONFIG_UBOOT_SDRAM_BASE) && ((cpy_addr + len) < CONFIG_UBOOT_SDRAM_BASE))
			(void)memcpy((void *)(load_addr + offset), src, len);
		else if((cpy_addr >= uboot_end_addr) && ((cpy_addr + len) >= uboot_end_addr))
			(void)memcpy((void *)(load_addr + offset), src, len);
		else
			printf("overwrite u-boot area 0x%x~0x%x\n",(unsigned int)CONFIG_UBOOT_SDRAM_BASE, (unsigned int)uboot_end_addr);
	}
#ifdef CONFIG_MCAST_TFTP
	if (Multicast)
