...
typedef struct {
    ...
    __u32	rootdir_clust;	/* Start cluster of root directory */
...
int file_exfat_rename(const char *oldname, const char *newname);
int file_exfat_rm(const char *filename);
int exfat_read_file(const char *filename, void *buf, int offset, int len);
int exfat_write_file(const char *filename, void *buf, int offset, int len);
