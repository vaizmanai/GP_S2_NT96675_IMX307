...
static int do_fat_fsrename(cmd_tbl_t *cmdtp, int flag,
		int argc, char * const argv[])
{
	block_dev_desc_t *dev_desc = NULL;
	disk_partition_t info;
	...
	if (fat_set_blk_dev(dev_desc, &info) != 0) {
		printf("\n** Unable to use %s %d:%d for fatrename **\n",
			argv[1], dev, part);
		return 1;
	}

	if (-1 == file_fat_rename(argv[3], argv[4])) {
...
U_BOOT_CMD(
	...
	"remove file from a dos filesystem",
