 ...
 */
P2PAPI_API void IOTC_WakeUp_Init(void);
...
/**
 * \brief Send IOTC WakeUp request to wake device up. (device: IOTC_WakeUp_Get_KeepAlivePacket)
 *
...
/**
 * \brief Send IOTC WakeUp request to wake device up. (device: IOTC_WakeUp_Get_SleepPacket)
 * 
 * \param pcszUID[in] The UID of a device that client wants to wake
 *
 * \return Error code if return value < 0
 */
P2PAPI_API int IOTC_WakeUp_WakeDevice2(const char *pcszUID);
