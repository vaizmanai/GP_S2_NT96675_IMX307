...
{
    cyg_int32 i;
    cyg_int32  noncacheable = 0;

    bitmap = (cyg_uint32 *)base;
    blocksize = alloc_unit;
    if((cyg_int32)base & 0x40000000)
    {
//        diag_printf("Modify Cyg_Mempool_Fixed_Implementation non cache\r\n");

        // Non-Cache is designed from the uitron RTOS fix mempool
        noncacheable = 1;
    }
    ...
    //diag_printf("numblocks = %d  size=0x%08X  alloc_unit=0x%08X\r\n",numblocks,size,alloc_unit);
    ...
    {
        numblocks = (size-32) / blocksize;

        CYG_ASSERT( numblocks <= 256, "NVT Mempool Too Many Block Number" );

        maptop = 8;
        mempool = ((cyg_uint8*)bitmap) + 32;//8word only for 32blocks
...
inline cyg_int32
Cyg_Mempool_Fixed_Implementation::get_allocation_size( cyg_uint8 *p )
{
    if ( p < mempool || p >= top )
        return -1;               // address way out of bounds
    cyg_int32 i = p - mempool;
    i = i / blocksize;
    if ( &mempool[ i * blocksize ] != p )
    ...
    CYG_ASSERT( 0 <= i && i < 32, "map bit index escaped" );
    if ( ! ((1 << i) & bitmap[ j ] ) )
        return -1;               // block was not allocated
    return blocksize;
}
