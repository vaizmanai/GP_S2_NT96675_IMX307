...
valid_hostname(const char *hostname)
{
    ...
    int isnum = true;  //for check label is number
    int isIP = true; //for check hostname is IP
    if (!hostname) {
        return false;
    }
    label = hostname;
    ...
    while (*label) {
        //(RFC 1123) permitted hostname labels to start with digits
        ...
        if(isalpha(*label))
            ...
            isnum = true; //check label is number
...
gethostbyaddr(const char *addr, int len, int type)
{
    ...
    len = send_recv(msg, len, MAXDNSMSGSIZE);
    if (len < 0) {
        cyg_drv_mutex_unlock(&dns_mutex);
        CYG_REPORT_RETVAL( NULL );
        free(msg);
...
do_query(const char * hostname)
{
    ...
    /* Decode the answer */
    pParseAns = parse_answer(msg, DNS_TYPE_A);
    free(msg);
    return pParseAns;
}
...
#define NAME_BUF_SIZE 256
    ...
    name = (char*)malloc(NAME_BUF_SIZE);
    if (name == NULL)
    {
        CYG_REPORT_RETVAL( NULL );
        return NULL;
