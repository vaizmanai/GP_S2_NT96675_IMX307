...
struct Dot11StationConfigEntry {
	...
	int sc_fix_offset;
#endif	
/* for CONFIG_RTL_SIMPLE_CONFIG*/
#ifdef RX_LOOP_LIMIT
	unsigned int	limit_rxloop;
#endif
    unsigned char   sdio_card_power_cycle;
};
...
struct Dot11RFEntry {
	...
	//unsigned int	disable_ch1213;  // remove replace by autoch_maskmap
	unsigned int    autoch_maskmap_2g;     /*bit(1~14)  map to ch1~14 , when bit nonzero means the channel won't choose by auto channel , 20160317 plus*/ 	
	unsigned int	xcap;
	unsigned int	tssi1;
	unsigned int	tssi2;
	unsigned int	ther;
...
struct MiscEntry {
	...
	unsigned short  crystal_force;  /* 0:disable force crystal, 1:26M,2:40M,3:25M*/
