...
#include "BC_lib.h"
#else
#define BC_MAX_OBJ_NUM  1
#define BC_MAX_BLK_NUM  1
#endif
...
#define NVT_FALSE   0
#define NVT_TRUE    1
#define NVT_MD_WND_X 32
#define NVT_MD_WND_Y 32
...
typedef struct {
	...
	UINT32	uiAinG711Sample;
	CodecAinCodec uiAinCodec;
	CodecAudFiltSetting sAinAudFiltSetting;
	UINT32  uiAinAudSetting;
} CodecAinSetting;
...
}CodecFTGFaceJPEGData;

typedef struct{

	UINT32 		uiFaceNum;  // the number of detected face
	UINT32   	uiSnapshotNum; //how mamy face be snapshoted
	FTG_FACE	sFaceInfo[MAX_FTGNUM]; ////face information
	UINT32		uiFullJPEGAddr;
	UINT32		uiFullJPEGSize;
	UINT32		uiFullYUVAddr;
	UINT32		uiFullYUVSize;
	CodecFTGFaceJPEGData sFaceData[MAX_FTGNUM];
...
typedef struct{
	...
	UINT32 uiLevel;
	UINT8 uiWnd[NVT_MD_WND_Y][NVT_MD_WND_X];
...
typedef struct {
	...
	BOOL bAECEnable;		  ///< Enable/Disable the AEC
	BOOL bLeakEstEnable;      ///< Enable/Disable the AEC leak estimate.
	INT32 iLeakEstValue;      ///< Enable/Disable the AEC leak estimate.
...
extern void       UCtrlAppMovieIni_InstallID(void);
extern void       UCtrlAppMovieIni_WaitFinish(void);
#endif
