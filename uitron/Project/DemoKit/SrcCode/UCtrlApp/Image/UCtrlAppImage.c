...
static INT32 param_image_setting_get(INT32 argc, char *argv[])
{
	...
	UINT32                       rotate_dir = ISF_VDO_DIR_NONE;
...
static INT32 param_image_setting_set(INT32 argc, char *argv[])
{
	...
	case ISP_IRCUT_TYPE_NIGHT:
		pImageInfo->ircut.value = IQ_IRCUT_NIGHT;
		break;
	case ISP_IRCUT_TYPE_DAY:
		pImageInfo->ircut.value = IQ_IRCUT_DAY;
		break;
...
static INT32 param_dis_get(INT32 argc, char *argv[])
{
	...
	switch (ImageUnit_GetParam(ISF_IPL(ipl_id), ISF_CTRL, IPL_SEL_DISCOMP_IMM)) {
	case SEL_DISCOMP_ON:
...
static INT32 param_crop_set(INT32 argc, char *argv[]){
		...
		ISIZE DeviceSize;
		DeviceSize = GxVideo_GetDeviceSize(0);
		#endif
		...
		for(uiPath = 0; uiPath < UIAPPIPCAM_PATH_MAX_ID ; uiPath++){
			...
			if(uiSensorIndex ==  pStreamInfo[uiPath].sensor && uiIMEIndex == pStreamInfo[uiPath].ime){
				...
				if(sCropSetting.bEnable == TRUE){
					...
					if((sCropSetting.sCropInfo.x + sCropSetting.sCropInfo.w) >  sCropSetting.scale_size_w){
						DBG_ERR("crop  x:%d w:%d  resolution width:%d  (x+w) > resolution width error.\r\n",
							sCropSetting.sCropInfo.x,sCropSetting.sCropInfo.w, sCropSetting.scale_size_w);
						return -1;
					}
					if((sCropSetting.sCropInfo.y + sCropSetting.sCropInfo.h) >  sCropSetting.scale_size_h){
						DBG_ERR("crop  y:%d h:%d  resolution height:%d  (y+h) > resolution height error.\r\n",
							sCropSetting.sCropInfo.y,sCropSetting.sCropInfo.h, sCropSetting.scale_size_h);
						return -1;
					}
					#if (DISPLAY_FUNC == ENABLE)
                    	if (uiPath == g_iDisplayPath){
                        if((sCropSetting.sCropInfo.w != (UINT32)DeviceSize.w) || sCropSetting.sCropInfo.h != (UINT32)DeviceSize.h){
                            DBG_ERR("Display device size is %d x %d. You can only crop this size in display path.\n", DeviceSize.w, DeviceSize.h);
                            return -1;
                        }
					}
					#endif

					if(pCropInfo[uiSensorIndex].sCropInfo[uiIMEIndex].h != sCropSetting.sCropInfo.h||
						pCropInfo[uiSensorIndex].sCropInfo[uiIMEIndex].w !=sCropSetting.sCropInfo.w){
						ImageUnit_Begin(&ISF_UserProc, 0);
							ImageUnit_SetVdoImgSize(uiPath + ISF_IN_BASE, sCropSetting.sCropInfo.w, sCropSetting.sCropInfo.h);
						ImageUnit_End();
						if (uiPath != g_iDisplayPath){
					...
					if(pStreamInfo[uiPath].type == IPCAM_STREAM_MAIN){
						...
						ImageUnit_Begin(&ISF_ImgTrans, 0);
							ImageUnit_SetVdoImgSize(uiPath + ISF_IN_BASE, sCropSetting.scale_size_w, sCropSetting.scale_size_h);
						...
						DBG_ERR("crop function dosesn't support ext stream path\r\n");
					}

					pCropInfo[uiSensorIndex].sScaleSize[uiIMEIndex].w = sCropSetting.scale_size_w;
					pCropInfo[uiSensorIndex].sScaleSize[uiIMEIndex].h = sCropSetting.scale_size_h;
					pCropInfo[uiSensorIndex].sCropInfo[uiIMEIndex].h =sCropSetting.sCropInfo.h;
					pCropInfo[uiSensorIndex].sCropInfo[uiIMEIndex].w =sCropSetting.sCropInfo.w;
					pCropInfo[uiSensorIndex].sCropInfo[uiIMEIndex].x =sCropSetting.sCropInfo.x;
					pCropInfo[uiSensorIndex].sCropInfo[uiIMEIndex].y =sCropSetting.sCropInfo.y;
				}
				else{
					ImageUnit_Begin(&ISF_UserProc, 0);
					...
					if(pStreamInfo[uiPath].type == IPCAM_STREAM_MAIN){
						...
						ImageUnit_End();
						pCropInfo[uiSensorIndex].sScaleSize[uiIMEIndex].w = 0;
						pCropInfo[uiSensorIndex].sScaleSize[uiIMEIndex].h = 0;
						pCropInfo[uiSensorIndex].sCropInfo[uiIMEIndex].h =0;
						pCropInfo[uiSensorIndex].sCropInfo[uiIMEIndex].w =0;
						pCropInfo[uiSensorIndex].sCropInfo[uiIMEIndex].x =0;
						pCropInfo[uiSensorIndex].sCropInfo[uiIMEIndex].y =0;
					}
				}
...
static INT32 param_crop_get(INT32 argc, char *argv[])
{
		...
		sCropSetting.scale_size_w = pCropInfo[uiSensorIndex].sScaleSize[uiIMEIndex].w;
		sCropSetting.scale_size_h = pCropInfo[uiSensorIndex].sScaleSize[uiIMEIndex].h;
		sCropSetting.sCropInfo.x = pCropInfo[uiSensorIndex].sCropInfo[uiIMEIndex].x;
		sCropSetting.sCropInfo.y = pCropInfo[uiSensorIndex].sCropInfo[uiIMEIndex].y;
		sCropSetting.sCropInfo.w = pCropInfo[uiSensorIndex].sCropInfo[uiIMEIndex].w;
		sCropSetting.sCropInfo.h = pCropInfo[uiSensorIndex].sCropInfo[uiIMEIndex].h;



		DBG_DUMP("sensor%d ime%d crop info=%d %d %d %d %d\r\n",uiSensorIndex,
			        uiIMEIndex,
...
static INT32 param_hdr_get(INT32 argc, char* argv[]){
	...
	sHDRSetting.eHDREnable = pIPLFunc[ipl_id].sHDR.eHDREnable;
	sHDRSetting.uiFrameNum = pIPLFunc[ipl_id].sHDR.uiFrameNum;
	sHDRSetting.uiHDRdEV = pIPLFunc[ipl_id].sHDR.uiHDRdEV;
...
//
	...
	DBG_DUMP("set ipl_id=%d HDR:%d frameNum:%d\r\n",ipl_id,sHDRSetting.eHDREnable,sHDRSetting.uiFrameNum);

	pIPLFunc[ipl_id].sHDR.eHDREnable = sHDRSetting.eHDREnable;
	pIPLFunc[ipl_id].sHDR.uiFrameNum = sHDRSetting.uiFrameNum;
	pIPLFunc[ipl_id].sHDR.uiHDRdEV = sHDRSetting.uiHDRdEV;
	UINT32 uiIPLFunc = ImageUnit_GetParam(ISF_IPL(ipl_id),ISF_CTRL,IMAGEPIPE_PARAM_FUNC);
	if(pIPLFunc[ipl_id].sHDR.eHDREnable == TRUE){
	...
	ImageUnit_Begin(ISF_IPL(ipl_id), 0);
		ImageUnit_SetParam(ISF_CTRL, IPL_SEL_SHDR, pIPLFunc[ipl_id].sHDR.eHDREnable);
		ImageUnit_SetParam(ISF_CTRL, IPL_SEL_SHDR_FRAME_NUM, pIPLFunc[ipl_id].sHDR.uiFrameNum);
		ImageUnit_SetParam(ISF_CTRL, IPL_SEL_SHDR_DELTA_EV, pIPLFunc[ipl_id].sHDR.uiHDRdEV);
		////work around for change ipl mode
		//UINT32 cur_mode = 0;
		//cur_mode = ImageUnit_GetParam(pSensorList[ipl_id],ISF_CTRL,IMAGEPIPE_PARAM_MODE);

		//if(cur_mode== IMAGEPIPE_MODE_VIDEO){
...
static INT32 param_end(UTOKEN_END_INFO *p_info)
{
...
#if _TODO   //sa4 add
	//#NT#2017/10/06#Iyen Chiang -begin
	...
	if (pImageInfo->ircut.isSet) {
		...
		iq_set_ui_info(ipl_id, IQ_UI_IRCUT,pImageInfo->ircut.value);
	}
	...
	if(bUpdate == TRUE){
		...
		ImageUnit_Begin(ISF_IPL(ipl_id), 0);
			...
			//}
			//else{
			//	ImageUnit_SetParam(ISF_CTRL, IMAGEPIPE_PARAM_MODE, IMAGEPIPE_MODE_VIDEO);
			//}
		ImageUnit_End();
		////end work around
