...
void PhotoDisplay2_Update(UINT8 iDD, ISIZE *pSize)
{
	...
	UINT32    uiPoolAddr2, uiPoolSize2;
    static UINT32 cutIndex =0;
	uiPoolAddr = OS_GetMempoolAddr(POOL_ID_DISP_VDO2TEMP);
	uiPoolSize = OS_GetMempoolSize(POOL_ID_DISP_VDO2TEMP)/2;
	uiPoolAddr2 = uiPoolAddr+uiPoolSize;
	uiPoolSize2 = uiPoolSize;
    ...
    //Use ping-pong buffer to avoid displaying the rotation process
    if(cutIndex%2){
	    GxImg_RotateData(&DispImg, uiPoolAddr2, uiPoolSize2, GX_IMAGE_ROTATE_270, &DispOutImg);
    } else {
...
//    Img.buf_LineOffset = Img.buf_w;
    ...
    cutIndex++;
#endif
	DBG_MSG("Img Y:0x%x,CB:0x%x,loff:0x%x format:%x\r\n", Img.buf_Y_Addr,Img.buf_Cb_Addr,Img.buf_LineOffset,Img.buf_Format);
