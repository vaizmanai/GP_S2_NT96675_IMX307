...
static void init_wifi_settings(char *mode, char *security)
{
    ...
    if (strcmp(pwifi->security, "wep64-auto") == 0) {
        ...
        strncpy(pwifi->wep_key4, "12345678901234567890123456", sizeof(pwifi->wep_key4));
    }
}
...
static int do_wifi_set_cmd(int argc, char *argv[]){
    ...
    if (strcmp(argv[0], "set") == 0) {
       if(strcmp(argv[1],"ssid")==0 ){
           ...
           strncpy((char *)gssid, argv[2], sizeof(gssid) - 1);
           printf("[ATD] ssid=%s\n", gssid);
           return 0;
       }
           ...
           if (valid == 1){
                //20180207 nash: coverity fix, CID#110322
                strncpy((char *)gauth, argv[2], sizeof(gauth) - 1);
                printf("[ATD] auth=%s\n", gauth);
           }
           else
