...
#define VS_WATERLOGO_ID 16
#define VS_WATERLOGO_REGION 2

#define VS_MULTISTAMP_ID 24
#define VS_MULTISTAMP_REGION 3
#endif
//variable declare
...
#if (THUMB_STAMP_FUNC == ENABLE)
	MovieStamp_UpdateData_Thumb(0, 1);
#endif
...
#if (THUMB_STAMP_FUNC == ENABLE)
    	MovieStamp_Setup_Thumb (0, STAMP_OFF, 0, 0, NULL);
#endif
}
...
void MovieStamp_Setup(UINT32 uiVidEncId, UINT32 uiFlag, UINT32 uiImageWidth, UINT32 uiImageHeight, WATERLOGO_BUFFER *pWaterLogoBuf)
{
...
#if defined (WATERLOGO_FUNCTION) && (WATERLOGO_FUNCTION == ENABLE)
	{
		if (g_bWaterLogoEnable[uiVidEncId]) {
			...
			uiPxlAddr[0]    = (UINT32)&g_ucWaterMark_ARGB4444[0];
			GxImg_InitBuf(&SrcImg,
							pWaterLogoBuf->uiWidth,
							pWaterLogoBuf->uiHeight,
							GX_IMAGE_PIXEL_FMT_ARGB4444_PACKED,
							0,
							uiPxlAddr[0],
							(pWaterLogoBuf->uiWidth * pWaterLogoBuf->uiHeight *2));

			// Init destination buffer
			uiPxlAddr[0]    =  g_MovieWaterInfo[uiVidEncId].uiWaterLogoYAddr;

			GxImg_InitBuf(&DstImg,
							g_MovieWaterInfo[uiVidEncId].uiWidth,
							g_MovieWaterInfo[uiVidEncId].uiHeight,
							GX_IMAGE_PIXEL_FMT_ARGB4444_PACKED,
							0,
							uiPxlAddr[0],
							(g_MovieWaterInfo[uiVidEncId].uiWidth*g_MovieWaterInfo[uiVidEncId].uiHeight*2));
			// Set scale region
			SrcRegion.x = 0;
			SrcRegion.y = 0;
			SrcRegion.w = pWaterLogoBuf->uiWidth;
	...
	if ((uiFlag & STAMP_OPERATION_MASK) == STAMP_AUTO) {
		...
		default:
			if ((uiFlag & STAMP_POS_END_MASK) == STAMP_POS_END) {
			 ...
			 g_MovieStampPos[uiVidEncId].uiX =  g_MovieStampPos[uiVidEncId].uiX - 160;///////////////////yxf_debug ×óÒÆ¶¯90¸öÏñËØµã
#if defined (MULTILINGUAL_FUNCTION) && (MULTILINGUAL_FUNCTION == ENABLE)
			{
				g_LingualStampPos[uiVidEncId].uiX = uiStampWidth; // 1 font width gap
				g_LingualStampPos[uiVidEncId].uiY = uiImageHeight - (pStampInfo->ui32DstHeight * 3) / 2;
	...
	UINT32 Ipl_id=ImageApp_MovieMulti_VePort2Imglink(uiVidEncId);
	if(Ipl_id<IPL_ID_MAX_NUM){
		IQS_GetLogString(Ipl_id, &g_cMovieStampStr[uiVidEncId][0], 256);
	}
	...
	//DBG_IND("VsOsdWidth[%d]=%d, %d\r\n",uiVidEncId,g_VsOsdWidth[uiVidEncId], g_VsOsdHight[uiVidEncId] );

      //MovieStamp_VsConfig(uiVidEncId, g_VsOsdWidth[uiVidEncId], g_VsOsdHight[uiVidEncId]);
#if (_BOARD_DRAM_SIZE_ == 0x04000000)
	MovieStamp_VsConfig(uiVidEncId, uiImageWidth, pStampInfo->pDataBase->pIconHeader[0x20].uiHeight);
#else
      MovieStamp_VsConfig(uiVidEncId, uiImageWidth, STAMP_HEIGHT_MAX); //from MovieStamp_CalcBufSize(), uiImageWidth may be 2560
#endif

      MovieStamp_VsFontConfig(uiVidEncId);
      //MovieStamp_VsAllocFontBuf(uiVidEncId, g_VsOsdWidth[uiVidEncId], g_VsOsdHight[uiVidEncId]);
#if (_BOARD_DRAM_SIZE_ == 0x04000000)
	MovieStamp_VsAllocFontBuf(uiVidEncId, uiImageWidth, pStampInfo->pDataBase->pIconHeader[0x20].uiHeight);
#else
      MovieStamp_VsAllocFontBuf(uiVidEncId, uiImageWidth, STAMP_HEIGHT_MAX);
#endif
#endif
}
		...
		if ((g_uiMovieStampSetup[i] & STAMP_SWITCH_MASK) == STAMP_ON) {
			...
			if ((g_uiMovieStampSetup[i] & STAMP_DATE_TIME_MASK) == STAMP_DATE_TIME) {
				...
				case STAMP_DD_MM_YY:
					...
					snprintf(&g_cMovieStampStr[i][0], MOVIE_STAMP_MAX_LEN, "GP-S2 %02d/%02d/%04d %02d:%02d:%02d", CurDateTime.tm_mday, CurDateTime.tm_mon, CurDateTime.tm_year, CurDateTime.tm_hour, CurDateTime.tm_min, CurDateTime.tm_sec);
					break;
				case STAMP_MM_DD_YY:
					...
					snprintf(&g_cMovieStampStr[i][0], MOVIE_STAMP_MAX_LEN, "GP-S2 %02d/%02d/%04d %02d:%02d:%02d", CurDateTime.tm_mon, CurDateTime.tm_mday, CurDateTime.tm_year, CurDateTime.tm_hour, CurDateTime.tm_min, CurDateTime.tm_sec);
					break;
				default:
					...
					snprintf(&g_cMovieStampStr[i][0], MOVIE_STAMP_MAX_LEN, "GP-S2 %04d/%02d/%02d %02d:%02d:%02d", CurDateTime.tm_year, CurDateTime.tm_mon, CurDateTime.tm_mday, CurDateTime.tm_hour, CurDateTime.tm_min, CurDateTime.tm_sec);
					break;
				}
			...
			if(Ipl_id<IPL_ID_MAX_NUM){
				IQS_GetLogString(Ipl_id, &g_cMovieStampStr[i][0], 256);
			}
			...
			DBG_DUMP("time2: %d us\r\n", Perf_GetCurrent() - uiTime);
...
#if(VIDEOSPRITE_STAMP_FUNC == ENABLE)
#if (THUMB_STAMP_FUNC == ENABLE)
                   MovieStamp_UpdateData_Thumb(i, 2);
#endif
                   ...
                   g_VsOsdWidth[i]=g_VsFontOut[i].GenImg.Width;
                   g_VsOsdHight[i]=g_VsFontOut[i].GenImg.Height;
                   MovieStamp_VsUpdateOsd(i, TRUE, i, 1, 1, g_MovieStampPos[i].uiX, g_MovieStampPos[i].uiY, g_VsOsdWidth[i], g_VsOsdHight[i], (void*)g_pVsFontDataAddr[i]);
#if defined (WATERLOGO_FUNCTION) && (WATERLOGO_FUNCTION == ENABLE)
                   if (g_bWaterLogoEnable[i]){
	                   DBG_IND("water Width=%dx%d\r\n",g_MovieWaterInfo[i].uiWidth, g_MovieWaterInfo[i].uiHeight);
	                   MovieStamp_VsUpdateOsd(i, TRUE, (16+i), 2, 2, g_MovieWaterInfo[i].uiXPos, g_MovieWaterInfo[i].uiYPos, g_MovieWaterInfo[i].uiWidth, g_MovieWaterInfo[i].uiHeight, (void*)g_MovieWaterInfo[i].uiWaterLogoYAddr);
                   }
#endif
...
#if(VIDEOSPRITE_STAMP_FUNC == ENABLE)
	//DBG_DUMP("vid=%d\r\n",vid);
    MovieStamp_VsUpdateOsd(vid, TRUE, vid, 1, 1, g_MovieStampPos[vid].uiX, g_MovieStampPos[vid].uiY, g_VsOsdWidth[vid], g_VsOsdHight[vid], (void*)g_pVsFontDataAddr[vid]);
#if defined (WATERLOGO_FUNCTION) && (WATERLOGO_FUNCTION == ENABLE)
	if (g_bWaterLogoEnable[vid]){
		MovieStamp_VsUpdateOsd(vid, TRUE, (VS_WATERLOGO_ID+vid), 2, VS_WATERLOGO_REGION, g_MovieWaterInfo[vid].uiXPos, g_MovieWaterInfo[vid].uiYPos, g_MovieWaterInfo[vid].uiWidth, g_MovieWaterInfo[vid].uiHeight, (void*)g_MovieWaterInfo[vid].uiWaterLogoYAddr);
	}
#endif
...
#if (THUMB_STAMP_FUNC == ENABLE)
void MovieStamp_UpdateData_Thumb(UINT32 vid, UINT16 uiFlag)
{
	GX_IMAGE_PIXEL_FMT GxImgFormat;
	STAMP_BUFFER    StampBuffer;
#if (VIDEOSPRITE_STAMP_FUNC == DISABLE)
	...
	if(uiFlag == 1){//Init
        	...
        	//DBG_DUMP("update pi8Str=%s\r\n", g_MovieStampInfo[i].pi8Str);
        	//DBG_DUMP("i=%d, w=%d, %d\r\n", i,g_VsOsdWidth[i], g_VsOsdHight[i]);
	}else if(uiFlag == 2){ //update
        	g_VsFontIn[vid].pFont=(FONT *)gDateStampFontTbl12x20;
        	g_VsFontIn[vid].pStr=g_ThumbStampInfo.pi8Str;
        	if (FontConv(&g_VsFontIn[vid], &g_VsFontOut[vid]) != E_OK) {
        	    DBG_ERR("FontConv err\r\n");
        	}
		VsOsdWidth=g_VsFontOut[vid].GenImg.Width;
		VsOsdHight=g_VsFontOut[vid].GenImg.Height;
		memcpy((void *)g_uiThumbStampYAddr, (void *)g_VsFontOut[vid].GenImg.PxlAddr[0], (VsOsdWidth*VsOsdHight*2));
		DBG_IND("VsOsdWidth=%dx%d\r\n",VsOsdWidth,VsOsdHight);
	}else{//update data to coe from MOVIE_USER_CB_EVENT_STAMP_CB

	       if(VsOsdWidth==0 || VsOsdHight==0 || g_uiThumbStampYAddr==0){
	            DBG_ERR("thumb data err!\r\n");
	       }
       MovieStamp_VsUpdateOsd(vid, TRUE, vid,  1, 1, g_ThumbStampPos.uiX, g_ThumbStampPos.uiY, VsOsdWidth, VsOsdHight, (void*)g_uiThumbStampYAddr);

	       //disable water logo
#if defined (WATERLOGO_FUNCTION) && (WATERLOGO_FUNCTION == ENABLE)
		if (g_bWaterLogoEnable[vid]){
		       MovieStamp_VsUpdateOsd(vid, FALSE, (16+vid), 2, 2, g_MovieWaterInfo[vid].uiXPos, g_MovieWaterInfo[vid].uiYPos, g_MovieWaterInfo[vid].uiWidth, g_MovieWaterInfo[vid].uiHeight, (void*)g_MovieWaterInfo[vid].uiWaterLogoYAddr);
		}
#endif
...
ER MovieStamp_VsAllocOsdBuf(VDS_PHASE phase, UINT32 uiVidEncId, UINT32 width, UINT32 height)
{
    ...
    uiwaterlogoAddr=(g_uiWaterLogoYAddr[uiVidEncId]+g_WaterLogo.uiWidth*g_WaterLogo.uiHeight*4);
    
    if((uiwaterlogoAddr +g_uiVsStampSize) >  (g_uiWaterLogoYAddr[uiVidEncId] + WATERLOGO_BUF_SIZE)){
	    DBG_ERR("Water logo buf overflow!!\r\n");
    }
    
    videosprite_config_ping_pong_buf(phase, (16+uiVidEncId), uiwaterlogoAddr, g_uiVsStampSize);
#endif
...
ER MovieStamp_VsUpdateOsd(UINT32 uiVidEncId, UINT32 enable, UINT32 id, UINT32 layer, UINT32 region, UINT32 uiX, UINT32 uiY, UINT32 uiOSDWidth, UINT32 uiOSDHeight, void *picture)
{
#if(VIDEOSPRITE_STAMP_FUNC == ENABLE)
    CodecOSDIMAGE image;
    ...
    MovieStamp_VsConfigOsd(&image, id, layer, region, uiVidEncId, enable, VDS_FMT_PICTURE_ARGB4444, uiX, uiY, uiOSDWidth, uiOSDHeight, picture);
    if (videoimage_add(&image)) {
        DBG_ERR("fail to add btn osd 0, vid=%d, en=%d, id=%d, reg=%d, (%d,%d), wxh=%dx%d\r\n",uiVidEncId, enable, id, region, uiX, uiY, uiOSDWidth, uiOSDHeight);
        return -1;
    }
...
void MovieStamp_VsFontConfig(UINT32 uiVidEncId)
{
	...
	INT32 r, g, b;
	INT32 y, u, v;
	y = g_MovieStampInfo[uiVidEncId].Color[COLOR_ID_FG].ucY;
	u = g_MovieStampInfo[uiVidEncId].Color[COLOR_ID_FG].ucU;
	v = g_MovieStampInfo[uiVidEncId].Color[COLOR_ID_FG].ucV;
	YUV_GET_RGB(y, u, v, r, g, b);
	g_VsFontIn[uiVidEncId].ciSolid = 0xF000 | ((r>>4)<<8) | ((g>>4)<<4) | (b>>4); //0xFFFF;

	y = g_MovieStampInfo[uiVidEncId].Color[COLOR_ID_FR].ucY;
	u = g_MovieStampInfo[uiVidEncId].Color[COLOR_ID_FR].ucU;
	v = g_MovieStampInfo[uiVidEncId].Color[COLOR_ID_FR].ucV;
	YUV_GET_RGB(y, u, v, r, g, b);
	g_VsFontIn[uiVidEncId].ciFrame = 0xF000 | ((r>>4)<<8) | ((g>>4)<<4) | (b>>4); //0xF000;

	y = g_MovieStampInfo[uiVidEncId].Color[COLOR_ID_BG].ucY;
	u = g_MovieStampInfo[uiVidEncId].Color[COLOR_ID_BG].ucU;
	v = g_MovieStampInfo[uiVidEncId].Color[COLOR_ID_BG].ucV;
	YUV_GET_RGB(y, u, v, r, g, b);
	g_VsFontIn[uiVidEncId].ciTransparet = ((r>>4)<<8) | ((g>>4)<<4) | (b>>4); //0x0000;
...
{
#if((UI_FUNC== DISABLE) && (VIDEOSPRITE_STAMP_FUNC == ENABLE) && (STAMP_FUNC == ENABLE))
	UINT32              uiPoolAddr;
...
#if(MOVIE_MULTISTAMP_FUNC == ENABLE)
#define MOVIE_MULTI_STAMP_CNT_MAX           5           // max 5 count
#define MOVIE_MULTI_STAMP_MAX_NAMECHAR_CNT  30

static NVTMPP_VB_POOL g_MovieMultiStampPool[MOVIE_MULTI_STAMP_CNT_MAX]=
                                                                                        {NVTMPP_VB_INVALID_POOL,
                                                                                            NVTMPP_VB_INVALID_POOL,
                                                                                            NVTMPP_VB_INVALID_POOL,
                                                                                            NVTMPP_VB_INVALID_POOL,
...
static UINT32 g_MovieMultiStampPoolAddr[MOVIE_MULTI_STAMP_CNT_MAX]={0};

UINT32 MovieStamp_GetMultiBufAddr(UINT32 Id, UINT32 blk_size)
{
        NVTMPP_VB_BLK  blk=0;
        CHAR pool_name[20] ={0};
        ...
        if(g_MovieMultiStampPool[Id]==NVTMPP_VB_INVALID_POOL)  {
            sprintf(pool_name,"MovieMultiStamp_%d",Id);
            g_MovieMultiStampPool[Id]=nvtmpp_vb_create_pool(pool_name, blk_size , 1, NVTMPP_DDR_1);
            if (NVTMPP_VB_INVALID_POOL == g_MovieMultiStampPool[Id])	{
        		DBG_ERR("create private pool err\r\n");
        		return NVTEVT_CONSUME;
            }

            blk = nvtmpp_vb_get_block(0, g_MovieMultiStampPool[Id], blk_size, NVTMPP_DDR_1);
            if (NVTMPP_VB_INVALID_BLK == blk) {
        		DBG_ERR("get vb block err\r\n");
                	return NVTEVT_CONSUME;
            }
            g_MovieMultiStampPoolAddr[Id]=nvtmpp_vb_block2addr(blk);
        }
        if(g_MovieMultiStampPoolAddr[Id] == 0)
    		DBG_ERR("get buf addr err\r\n");
        return g_MovieMultiStampPoolAddr[Id];
}
void MovieStamp_DestroyMultiStampBuff(void)
{
	INT32 i, ret;
	for (i=0;i<MOVIE_MULTI_STAMP_CNT_MAX;i++) {
		if(g_MovieMultiStampPool[i] != NVTMPP_VB_INVALID_POOL){
			ret = nvtmpp_vb_destroy_pool(g_MovieMultiStampPool[i]);
			if (NVTMPP_ER_OK != ret) {
				DBG_ERR("destory pool error pool 0x%x   ret=%d\r\n", g_MovieMultiStampPool[i], ret);
				break;
			}
			g_MovieMultiStampPool[i]=NVTMPP_VB_INVALID_POOL;
			g_MovieMultiStampPoolAddr[i]=0;
		}
	}
}
...
//*Pos: Stamp position
//*pStr: Stamp data
void MovieStamp_DrawMultiStamp(UINT32 uiVidEncId, UINT32 CntId, UPOINT *Pos, char *pStr)
{
#if(VIDEOSPRITE_STAMP_FUNC == ENABLE)
	FONT_CONV_IN VsFontIn[MOVIE_MULTI_STAMP_CNT_MAX]={0};
	FONT_CONV_OUT VsFontOut[MOVIE_MULTI_STAMP_CNT_MAX]={0};
	STAMP_INFO   sMovieStampInfo;
	UINT16 uiFontWidth=0;
	UINT16 uiFontHeight;
	UINT16* pVsMultiFontDataAddr[MOVIE_MULTI_STAMP_CNT_MAX];
	...
	if(strlen(pStr) > MOVIE_MULTI_STAMP_MAX_NAMECHAR_CNT){
		DBG_ERR("str len reach MAX\r\n");
		return;
	}
	if(CntId>=MOVIE_MULTI_STAMP_CNT_MAX){
		DBG_ERR("CntId reach MAX\r\n");
		return;
	}
	if ((g_uiMovieStampSetup[uiVidEncId] & STAMP_SWITCH_MASK) != STAMP_ON) {
	...
	uiFontHeight=sMovieStampInfo.pDataBase->pIconHeader[0x20].uiHeight;
	//DBG_DUMP("FontWidth=%d, FontHeight=%d\r\n",uiFontWidth,uiFontHeight);
	...
	MemAddr[CntId]=MovieStamp_GetMultiBufAddr(CntId, MemSize);

	memcpy(&VsFontIn[CntId], &g_VsFontIn[uiVidEncId], sizeof(FONT_CONV_IN));
	VsFontIn[CntId].MemAddr = MemAddr[CntId]+ MemSize*4*10/95;
	VsFontIn[CntId].MemSize = uiFontWidth*uiFontHeight*VSFONT_BUF_RATIO/100;
	VsFontIn[CntId].pStr = pStr;

       if (FontConv(&VsFontIn[CntId], &VsFontOut[CntId]) != E_OK) {
            DBG_ERR("FontConv err\r\n");
       }
       pVsMultiFontDataAddr[CntId]=(UINT16 *)VsFontOut[CntId].GenImg.PxlAddr[0];
       //DBG_DUMP("Str=%s\r\n",pStr);
	...
	MovieStamp_VsUpdateOsd(uiVidEncId, TRUE, (VS_MULTISTAMP_ID+CntId), 2, (VS_MULTISTAMP_REGION+CntId), Pos->x, Pos->y, VsFontOut[CntId].GenImg.Width, VsFontOut[CntId].GenImg.Height, (void*)pVsMultiFontDataAddr[CntId]);
#endif
}
#endif
