...
static INT32 XML_MakeJpgFromIFrame(IDR_TO_JPG *pIdrToJpg, PIMG_BUF pDecodedImgBuf, UINT32 JpgBsBuf)
{
	...
	if (NULL == pDecodedImgBuf) { //make JPG from I frame
		...
		#else
        // Modified for TS video without thumbnail. Dynamic allocate memory.
	...
	if (XML_CheckSub(pch, TYPE_MOV_MP4) || XML_CheckSub(pch, TYPE_TS)) {
		if (segmentCount == 0) {
			...
			if (gMovFilehdl) {
				...
				// double buffer size for GxVidFile_ParseVideoInfo.
				{
					uiBufferNeeded *= 2;
				}

				TempBuf = XML_AllocPrivateMem(uiBufferNeeded);
				if (!TempBuf) {
...

				{
					...
					if ((result == GXVIDEO_PRSERR_OK) &&
						...
						DBG_ERR("No Thumbnail and I frame.\r\n");
                                      ...
                                      CHKPNT;
					      //////////////////////////////////////////////////----------------------///////////
					    //20180810 fix bad file when get thumbnail start
					    ...
					    SMediaPlay_FileRecovery(tempPath, uiBuffAddr, uiBuffSize);
					    FileDB_Refresh(0);//2016/04/11
					    //20180810 fix bad file when get thumbnail end
...
//#NT#2016/02/15#Ben Wang -end
	if (result != 0) {
           // hwmem_open();
	    //  hwmem_memcpy((UINT32)buf, testthumbnail, 630);
		//hwmem_close();
		// *bufSize = 630;
		//  remain=0;
		//  ThumbBuf =0;
...
int XML_SRK(char* path, char* argument, cyg_uint32 bufAddr, cyg_uint32* bufSize, char* mimeType, cyg_uint32 segmentCount)
{
	char*             pch=0;
	char       parStr[64];  
	char * buf = NULL;
	...
	XML_StringResult(WIFIAPP_CMD_GET_KEY, BB_EE(buf, strlen(buf)), bufAddr, (HFS_U32*)bufSize, mimeType);		 
	return CYG_HFS_CB_GETDATA_RETURN_OK;
}

int XML_Movie_Liveview(char* path, char* argument, HFS_U32 bufAddr, HFS_U32* bufSize, char* mimeType, HFS_U32 segmentCount)
{
    XML_ValueResult(WIFIAPP_CMD_MJPEG_RTSP,0,bufAddr,bufSize,mimeType);
    return CYG_HFS_CB_GETDATA_RETURN_OK;
}
