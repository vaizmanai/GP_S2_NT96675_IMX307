...
bool bAutoRecordEn = TRUE;
#else
bool bAutoRecordEn = FALSE;
#endif
...
{
	if(TRUE == bAutoRecordEn)
	{
		bAutoRecordEn = FALSE;
		if(UIStorageCheck(STORAGE_CHECK_ERROR, NULL) == TRUE) {
			debug_msg("\r\nMemory error or No Sdcard!!!!\r\n");
		} else {
	...
	{
		UIMenuWndWiFiMobileLinkOK_UpdateMoiveIcons(FALSE,TRUE);
	...
	else
	{
		UIMenuWndWiFiMobileLinkOK_UpdatePhotoIcons(FALSE);
		UIMenuWndWiFiMobileLinkOK_UpdateMoiveIcons(FALSE,TRUE);
		FlowMovieWifi_IconHideEV(&UIMenuWndWiFiMobileLinkOK_StatusICN_EVCtrl);
		FlowMovieWifi_IconDrawPlayMode(&UIMenuWndWiFiMobileLinkOK_StaticI_PlayModeCtrl);
	}
      WifiCmd_Done(WIFIFLAG_MODE_DONE, WIFIAPP_RET_OK);
	return NVTEVT_CONSUME;
}
