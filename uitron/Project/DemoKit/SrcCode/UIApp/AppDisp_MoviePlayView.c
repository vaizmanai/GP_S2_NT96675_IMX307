...
#define DISP_QUEUE_MAXSIZE              3                               ///< display queue maximum size
...
//#NT#2018/12/11#Isiah Chang -begin
//#NT# Use GXIMG_SCALE_BILINEAR to due to IME: path1 does not support ISD. JiraID: NA51023-296.
#if 0//!defined(_MODEL_Q09_2_)
	GxImg_ScaleMultiOut(pSrc, &SrcRect, pMultiDst);
#else
	GxImg_SetParm(GXIMG_PARM_SCALE_METHOD, GXIMG_SCALE_BILINEAR);
	GxImg_ScaleMultiOut(pSrc, &SrcRect, pMultiDst);
	GxImg_SetParm(GXIMG_PARM_SCALE_METHOD, GXIMG_SCALE_AUTO);
#endif
//#NT#2018/12/11#Isiah Chang -end
}
