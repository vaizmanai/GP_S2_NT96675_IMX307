...
static ER xUIPlay_FList_FDB_ProtectFile(UINT32  cmdID)
{
    ...
    #if (PROTECT_DELETE_PATCH == ENABLE)
        INT32 curFileId;
        curFileId = FileDB_GetIndexByPath(FileDBHdl,g_chFullName);
        //debug_msg("^G>>>>>>>>>>cmdID=0x%02x   g_chFullName=%s   curFileId=%d\r\n",cmdID,g_chFullName,curFileId);
        FileAttr = FileDB_SearhFile(FileDBHdl,curFileId);
    #else
