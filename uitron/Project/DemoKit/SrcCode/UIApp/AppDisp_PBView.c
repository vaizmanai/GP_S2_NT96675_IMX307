...
static void PBView_DrawSingleView(PIMG_BUF pSrc, PIMG_BUF pDst, PUSIZE dst_ratio)
{
	BOOL   bIsMovieFile;
	PIRECT  SrcRegion = 0;
	IRECT  DstRegion = {0};
...
static ER PBView_DrawMethod(PIMG_BUF pSrc, PIMG_BUF pDst, PUSIZE dst_ratio)
{
	...
	if (PLAY_ROTATE_DIR_UNKNOWN != rot_dir) {
		...
		PBView_DrawSingleView(pSrc, pDst, dst_ratio);
	}
	//--------------------Customer Draw End---------------------------------
...
static void PBView_DrawThumbView(UINT32 i, PIMG_BUF pSrc, PIMG_BUF pDst, PUSIZE dst_ratio)
{
	...
	// decode ok
		...
		if ((SrcWidth == DstWidth) && (SrcHeight == DstHeight)) {
			...
			ImageRatioTrans = PB_GetImageRatioTrans(SrcWidth, SrcHeight, DstWidth*dst_ratio->w*pDst->Height/pDst->Width/dst_ratio->h, DstHeight);
		}
...
ER PBView_OnThumbDraw(PIMG_BUF pSrc, PIMG_BUF pDst, PUSIZE dst_ratio)
{
	...
	for (i = 0; i < thumb_num; i++) {
		...
		if(ImageUnit_PullData(ImageUnit_Out(&ISF_UserProc, ISF_OUT1), (ISF_DATA *)&isf_data, 0) == ISF_OK) {
			pSrc = (IMG_BUF *)&isf_data.Desc[0];

			//--------------------Customer Draw Begin---------------------------
			PBView_DrawThumbView(i, pSrc, pDst, dst_ratio);
			//--------------------Customer Draw End-----------------------------
...
{
	ISF_DATA         isf_data = {0}, isf_new_data = {0};
	ISF_UNIT         *pUnit;
	ISF_PORT         *pSrc;
	ISIZE            device_size = GxVideo_GetDeviceSize(DOUT1);
	UINT32           img_w = (UINT32)device_size.w, img_h = (UINT32)device_size.h;
	UINT32           img_buf_size = img_w * img_h * 3 / 2, buff_addr;
	IMG_BUF          dst_img;
	...
	if (ImageUnit_IsAllowPull(pSrc)) {
		...
		GxImg_InitBuf(&dst_img, img_w, img_h, GX_IMAGE_PIXEL_FMT_YUV420, GXIMAGE_LINEOFFSET_ALIGN(16), buff_addr, img_buf_size);

		if (gPbState == PB_VIEW_STATE_THUMB) {
			gfpDrawCb(0, &dst_img, &aspect_ratio);
		} else {
			isf_data.Sign = ISF_SIGN_DATA;
			if (ImageUnit_PullData(pSrc, (ISF_DATA *)&isf_data, 0) == ISF_OK) {
				gfpDrawCb((IMG_BUF *)&isf_data.Desc[0], &dst_img, &aspect_ratio);
				ImageUnit_ReleaseData(&isf_data);
			} else {
				DBG_ERR("Pull data failed\r\n");
