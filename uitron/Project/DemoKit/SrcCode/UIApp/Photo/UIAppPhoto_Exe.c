...
#define PHOTO_ROOT_PATH         "A:\\GP-S2\\"
...
static UINT32 uiPhotoFileSerialNum = 0;
...
PHOTO_STRM_CBR_INFO g_tStrmCbrInfo[SENSOR_MAX_NUM] =  {
    {
        ...
        {1, 4, 100 * 1024, 30, 15, 26, 10, 40, 26, 10, 40, 0, 1, 8, 4}
#else
        {1, 4, 200 * 1024, 30, 15, 26, 10, 40, 26, 10, 40, 0, 1, 8, 4}
#endif
...
static USERPROC_TRIG_PROC_VAL PhotoDisp_UserProc_PushCB(ISF_PORT *pPort, ISF_DATA *pData)
{
	ISF_PORT *pDist;

	pDist = ImageUnit_In(&ISF_ImgTrans, ISF_IN1);
	ImageUnit_PushData(pDist, pData, 0);
	return USERPROC_TRIG_PROC_NONE;
}
static USERPROC_TRIG_PROC_VAL PhotoWiFi_UserProc_PushCB(ISF_PORT *pPort, ISF_DATA *pData)
{
	ISF_PORT *pDist;

	pDist = ImageUnit_In(&ISF_VdoEnc, ISF_IN1);
	ImageUnit_PushData(pDist, pData, 0);
	return USERPROC_TRIG_PROC_NONE;
}
static void PhotoExe_PIP_DispCB(void)
{
	// PIP view
	...
	APPDISP_VIEW_DRAW         *pdraw = &pip_draw;
    	UINT32 i;
	ISF_PORT         *pSrc[SENSOR_CAPS_COUNT];
	ISF_DATA         isf_data[SENSOR_CAPS_COUNT];
	UINT8 SrcNullCnt=0;
	UINT32    rotate_dir=0;
	UINT32 VdoOut_w, VdoOut_h;

	pDist = ImageUnit_In(&ISF_ImgTrans,   (ISF_IN1 + PHOTO_VID_OUT_1));
	for (i=0; i<SENSOR_CAPS_COUNT; i++){
		pSrc[i] = ImageUnit_Out(&ISF_UserProc, (ISF_OUT1 + PHOTO_DISP_ID_1 + i));
		if(pSrc[i] ){
			if (ImageUnit_IsAllowPull(pSrc[i]) && ImageUnit_PullData(pSrc[i], (ISF_DATA *)&isf_data[i], 500) == ISF_OK) {
				pdraw->p_src_img[i] = (IMG_BUF *)&isf_data[i].Desc[0];
			}
		}else{
			SrcNullCnt++;
			if(SrcNullCnt == SENSOR_CAPS_COUNT){
				return;
			}
		}
	}
	...
	for (i=0; i<SENSOR_CAPS_COUNT; i++){
		if (pdraw->p_src_img[i]) {
			isf_new_data.TimeStamp = isf_data[i].TimeStamp;
			break;
		}
	}
	...
	pdraw->viewcnt   = SENSOR_CAPS_COUNT;

	for (i=0; i<SENSOR_CAPS_COUNT; i++){
		DBG_IND("p_src_img %d w =%d, h=%d \r\n",i, pdraw->p_src_img[i]->Width,pdraw->p_src_img[i]->Height);
	}
...
static void PhotoExe_PIP_WiFiCB(void)
{
    // PIP view
	ISF_DATA         isf_new_data;
	ISF_PORT         *pDist;
	...
	pDist = ImageUnit_In(&ISF_VdoEnc, ISF_IN1);
	for (i=0; i<SENSOR_CAPS_COUNT; i++){
		pSrc[i] = ImageUnit_Out(&ISF_UserProc,  (ISF_OUT1 + PHOTO_STRM_ID_1 + i));
		if(pSrc[i]){
		    	if (ImageUnit_IsAllowPull(pSrc[i]) && ImageUnit_PullData(pSrc[i], (ISF_DATA *)&isf_data[i], 500) == ISF_OK) {
				pdraw->p_src_img[i] = (IMG_BUF *)&isf_data[i].Desc[0];
			}
	...
	ImageUnit_GetVdoImgSize(&ISF_VdoEnc, ISF_IN1, &dst_w, &dst_h);
	if (dst_w == 0 || dst_h == 0) {
		DBG_ERR("dst_w=%d, dst_h=%d\r\n",dst_w,dst_h);
		goto isf_data_rel;
	}
	dst_pxlfmt = GX_IMAGE_PIXEL_FMT_YUV420;
	img_buf_size = GxImg_CalcRequireSize(dst_w, dst_h, dst_pxlfmt, GXIMAGE_LINEOFFSET_ALIGN(16));
...
static INT32 PhotoExe_InitCommonMem(void)
{
	...
	DBG_IND("disp_size.w=%d,  disp_size.h=%d\r\n", disp_size.w, disp_size.h);

#if defined(_BSP_NA51023_)
	UINT16 pool_id=0;
	UIAPP_PHOTO_SENSOR_INFO *pSensorInfo;
	pSensorInfo = UIAppPhoto_get_SensorInfo(0);
	if(pSensorInfo->bIME3DNR){
		st_conf.common_pool[pool_id].blk_size = 1920 * 1080 * 3 / 2 + 1024;
		st_conf.common_pool[pool_id].ddr = NVTMPP_DDR_1;
#if (SENSOR_CAPS_COUNT == 1)
		st_conf.common_pool[pool_id].blk_cnt = 2;
#else
		st_conf.common_pool[pool_id].blk_cnt = 4;
#endif
		pool_id++;
	}else{
		pool_id=0;
	}
	st_conf.common_pool[pool_id].blk_size = 868 * 480 * 3 / 2 + 1024;
	st_conf.common_pool[pool_id].blk_cnt = 5;
	st_conf.common_pool[pool_id].ddr = NVTMPP_DDR_1;
	pool_id++;
	st_conf.common_pool[pool_id].blk_size = disp_size.w * disp_size.h * 3 / 2 + 1024;
	st_conf.common_pool[pool_id].blk_cnt = 5;
	st_conf.common_pool[pool_id].ddr = NVTMPP_DDR_1;
#else
#if (SENSOR_CAPS_COUNT == 1)
...
static void PhotoExe_FileNamingCB(UINT32 id, char *pFileName)
{
	...
	snprintf(pFileName, NMC_TOTALFILEPATH_MAX_LEN, "%04d%02d%02d%02d%02d%02d_%06d%c",
		...
		CurDateTime.tm_hour, CurDateTime.tm_min, CurDateTime.tm_sec, uiPhotoFileSerialNum, NH_endChar);
#else
	snprintf(pFileName, NMC_TOTALFILEPATH_MAX_LEN, "%04d%02d%02d%02d%02d%02d_%06d",
		CurDateTime.tm_year, CurDateTime.tm_mon, CurDateTime.tm_mday,
		CurDateTime.tm_hour, CurDateTime.tm_min, CurDateTime.tm_sec, uiPhotoFileSerialNum);
#endif
}
...
static void PhotoExe_InitFileNaming(void)
{
	...
	uiPhotoFileSerialNum=MovieExe_GetFileSN();
}
...
#if defined(_BSP_NA51023_)
		//DEFAULT: enable direct mode for 1 sensor
		if(localInfo->sensorCount ==1){
			ImageUnit_SetParam(ISF_CTRL, IMAGEPIPE_PARAM_FUNC2, IMAGEPIPE_FUNC2_DIRECT);//enable IPL direct
			ImageUnit_SetParam(ISF_CTRL, IMAGEPIPE_PARAM_MAX_FUNC2, IMAGEPIPE_FUNC2_DIRECT);//enable IPL direct
		}else{
			ImageUnit_SetParam(ISF_CTRL, IMAGEPIPE_PARAM_FUNC2, 0);
			ImageUnit_SetParam(ISF_CTRL, IMAGEPIPE_PARAM_MAX_FUNC2, 0);
		}
#endif
			/////set IPL mirror and flip
			#if 0
			if(pSensorInfo->eFlipType != FLIP_NONE){
				DBG_DUMP("set sensor %d flip=%d\r\n",sensor_index,pSensorInfo->eFlipType);
			}
        	if (pSensorInfo->eFlipType == FLIP_H_V) {
        		ImageUnit_SetVdoDirection(ISF_IN1, ISF_VDO_DIR_MIRRORX|ISF_VDO_DIR_MIRRORY);
        	} else if (pSensorInfo->eFlipType == FLIP_H) {
        		ImageUnit_SetVdoDirection(ISF_IN1, ISF_VDO_DIR_MIRRORX);
        	} else if (pSensorInfo->eFlipType == FLIP_V) {
...
static void PhotoExe_InitDisplayStream(UINT32 dualCam, USIZE *pImageRatioSize)
{
	...
	if (dualCam == DUALCAM_FRONT) {
		...
		ImageUnit_SetParam(ISF_OUT1+ PHOTO_DISP_ID_1, USERPROC_PARAM_PUSH_CALLBACK_IMM, (UINT32)PhotoDisp_UserProc_PushCB);
		ImageUnit_End();
	}
	// sensor 2 display
	...
	// PIP view
        ...
        for (i=0;i<localInfo->sensorCount;i++) {
        	...
        	ImageApp_Photo_DispConfig(PHOTO_CFG_DISP_REG_CB ,(UINT32)&PhotoExe_PIP_DispCB);
	}
}
static void PhotoExe_OpenDisplayStream(void)
{
	PHOTO_DISP_INFO    *p_disp = NULL;
	p_disp = UIAppPhoto_get_DispConfig(UIAPP_PHOTO_DISP_ID_1);
	if(p_disp->multi_view_type != PHOTO_MULTI_VIEW_SINGLE){
		PhotoDispTsk_Open();
	}
}
static void PhotoExe_InitNetworkStream(UINT32 dualCam, USIZE *pImageRatioSize)
{
	...
	if (dualCam == DUALCAM_FRONT) {
		...
		ImageUnit_Begin(&ISF_UserProc, 0);
		ImageUnit_SetParam(ISF_OUT1+ PHOTO_STRM_ID_1, USERPROC_PARAM_PUSH_CALLBACK_IMM, (UINT32)PhotoWiFi_UserProc_PushCB);
		ImageUnit_End();
	}
	// sensor 2 display
	else if (dualCam == DUALCAM_BEHIND) {
	...
	// PIP view
		...
		for (i=0;i<localInfo->sensorCount;i++) {
        	...
        	ImageApp_Photo_WiFiConfig(PHOTO_CFG_WIFI_REG_CB, (UINT32)PhotoExe_PIP_WiFiCB);
		if (p_strm->strm_type == PHOTO_STRM_TYPE_HTTP) {
			PhotoExe_InitNetHttp();
		}
	}
...
static void PhotoExe_StartNetworkStream(void)
{
	PHOTO_STRM_INFO    *p_strm = NULL;
	p_strm = UIAppPhoto_get_StreamConfig(UIAPP_PHOTO_STRM_ID_1);
	if(p_strm->multi_view_type != PHOTO_MULTI_VIEW_SINGLE)
		PhotoWiFiTsk_Open();
}
...
static void PhotoExe_InitIME3DNRStream(UINT32 dualCam, USIZE *pImageRatioSize)
{
	...
	for (sensor_id=0;sensor_id<localInfo->sensorCount;sensor_id++) {
		...
		if (pSensorInfo && pSensorInfo->bIME3DNR) {
			...
			//ime3dnr.enable          =  TRUE;
			ime3dnr.enable          = PHOTO_PORT_STATE_EN_RUN;
#if ((SENSOR_CAPS_COUNT == 2) && (SENSOR_INSERT_MASK != 0))
			if (System_GetEnableSensor() == SENSOR_1 && sensor_id==PHOTO_IME3DNR_ID_2){
				ime3dnr.enable          = PHOTO_PORT_STATE_EN;
			}
#endif
...
static void PhotoExe_InitAllImageStreams(UINT32 dualCam, USIZE *pImageRatioSize)
{
	...
	ImageUnit_Begin(&ISF_Cap, 0);
	ImageUnit_SetParam(ISF_CTRL, CAP_PARAM_RESV_BUFSIZE, ResvBufSize);
#if ((SENSOR_CAPS_COUNT == 2) && (SENSOR_INSERT_MASK != 0))
	if (System_GetEnableSensor() == SENSOR_1){
		IPL_BUF_INFOR BufInfo = {0};
		BufInfo.id = IPL_ID_1;
		IPL_GetCmd(IPL_GET_CUR_BUF_INFOR, (void *)&BufInfo);
		//for hot plug, reserve size for IPL2
 		ImageUnit_SetParam(ISF_CTRL, CAP_PARAM_RESV_BUFSIZE, (ResvBufSize+BufInfo.buf_size));
	}
#endif
	ImageUnit_End();
...
INT32 PhotoExe_OnOpen(VControl *pCtrl, UINT32 paramNum, UINT32 *paramArray)
{
	...
	ImageApp_Photo_Open();
	PhotoExe_OpenDisplayStream();
	PhotoExe_StartNetworkStream();
	...
	for (i = 0; i < sensor_cnt; i++) {
		if (sensor_cnt >= 2) {
			...
			if (localInfo->DualCam == DUALCAM_BEHIND) {
				...
				if(sensor_cnt == 1) { // inactive front sensor of capture.
					PhotoExe_Cap_SetUIInfo(0, CAP_SEL_ACTFLAG, FALSE);
				}
			}
			else {
				sensor_id = 0;
		...
		PhotoExe_Cap_SetUIInfo(sensor_id, CAP_SEL_DEFOG_EN, UI_GetData(FL_MOVIE_DEFOG));

		if(PhotoExe_GetCapYUV420En()){
			PhotoExe_Cap_SetUIInfo(sensor_id, CAP_SEL_PRI_FMT, IPL_IMG_Y_PACK_UV420);
			PhotoExe_Cap_SetUIInfo(sensor_id, CAP_SEL_THUMB_FMT, IPL_IMG_Y_PACK_UV420);
			PhotoExe_Cap_SetUIInfo(sensor_id, CAP_SEL_QV_FMT, IPL_IMG_Y_PACK_UV420);
			PhotoExe_Cap_SetUIInfo(sensor_id, CAP_SEL_SCREEN_FMT, IPL_IMG_Y_PACK_UV420);
		}else{
			PhotoExe_Cap_SetUIInfo(sensor_id, CAP_SEL_PRI_FMT, IPL_IMG_Y_PACK_UV422);
			PhotoExe_Cap_SetUIInfo(sensor_id, CAP_SEL_THUMB_FMT, IPL_IMG_Y_PACK_UV422);
			PhotoExe_Cap_SetUIInfo(sensor_id, CAP_SEL_QV_FMT, IPL_IMG_Y_PACK_UV422);
			PhotoExe_Cap_SetUIInfo(sensor_id, CAP_SEL_SCREEN_FMT, IPL_IMG_Y_PACK_UV422);
		}
		...
		case CONTINUE_SHOT_BURST:
			...
			if(PhotoExe_GetCapYUV420En()){
				PhotoExe_Cap_SetUIInfo(sensor_id, CAP_SEL_JPGFMT, SEL_JPGFMT_420);
			}else{
...
INT32 PhotoExe_OnDefog(VControl *pCtrl, UINT32 paramNum, UINT32 *paramArray)
{
	...
	UI_SetData(FL_MOVIE_DEFOG, uhSelect);
...
static void PhotoExe_2sensor_HotPlug_Disp(void)
{
	...
	DBG_IND("^M%s: u32CurrSensorEn = 0x%x\r\n", __func__, u32CurrSensorEn);
	DBG_IND("^M%s: u32PrevSensorEn = 0x%x\r\n", __func__, u32PrevSensorEn);
