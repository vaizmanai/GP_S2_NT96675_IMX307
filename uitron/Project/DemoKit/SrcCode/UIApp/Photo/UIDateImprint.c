...
#define DATE_COLOR_WHITE        0x008080C0
...
static NVTMPP_VB_POOL g_DateImprintWaterLogoPool=NVTMPP_VB_INVALID_POOL;
static UINT32 g_DateImprintWaterLogoMemAddr;
static DS_STAMP_INFOR StampWaterLogoInfor = {0};
#endif
...
void UiDateImprint_InitBuff(void)
{
	...
	for (i=0;i<DATE_IMPRINT_EVENT_MAX;i++) {
		...
		if(PhotoExe_GetCapYUV420En()){
			pInfo->StampInfo.Img.PxlFmt = GX_IMAGE_PIXEL_FMT_YUV420;
		}else{
			pInfo->StampInfo.Img.PxlFmt = GX_IMAGE_PIXEL_FMT_YUV422;
		}
	}
#if defined (WATERLOGO_PHOTOCAP_FUNCTION) && (WATERLOGO_PHOTOCAP_FUNCTION == ENABLE)
	if (g_DateImprintWaterLogoPool == NVTMPP_VB_INVALID_POOL) {
		UINT32 MemSize = 0x90000;
	     	g_DateImprintWaterLogoPool=nvtmpp_vb_create_pool("DateImprint_Water", MemSize, 1, NVTMPP_DDR_1);
	    	if (NVTMPP_VB_INVALID_POOL == g_DateImprintWaterLogoPool)	{
	    		DBG_ERR("create private pool err\r\n");
	    		return;
	    	}
	    	blk = nvtmpp_vb_get_block(0, g_DateImprintWaterLogoPool, MemSize, NVTMPP_DDR_1);
	    	if (NVTMPP_VB_INVALID_BLK == blk) {
	    		DBG_ERR("get vb block err\r\n");
	    		return;
	    	}
		g_DateImprintWaterLogoMemAddr = nvtmpp_vb_block2addr(blk);
	}
#endif
...
#if defined (WATERLOGO_PHOTOCAP_FUNCTION) && (WATERLOGO_PHOTOCAP_FUNCTION == ENABLE)
	if(g_DateImprintWaterLogoPool != NVTMPP_VB_INVALID_POOL){
			ret = nvtmpp_vb_destroy_pool(g_DateImprintWaterLogoPool);
			if (NVTMPP_ER_OK != ret) {
				DBG_ERR("destory pool error pool 0x%x   ret=%d\r\n", g_DateImprintWaterLogoPool, ret);
				return;
			}
			g_DateImprintWaterLogoPool=NVTMPP_VB_INVALID_POOL;
	}
#endif
}
...
static INT32 UiDateImprint_CalcPosition(DATE_IMPRINT_INFO   *pInfo, UPOINT *pos)
{
	...
	{
		DBG_ERR("event=%d, GenImg w %d> photo_w %d\r\n",pCapInfo->event,pYuvInfo->GenImg.Width,photo_w);
		return -1;
	}
...
BOOL UiDateImprint_GenData(IMG_CAP_DATASTAMP_INFO *pCapInfo)
{
	...
	if(PhotoExe_GetCapYUV420En()){
		GxImg_InitBufEx(&Img, pCapInfo->ImgInfo.ch[0].width, pCapInfo->ImgInfo.ch[0].height, GX_IMAGE_PIXEL_FMT_YUV420_PACKED, LineOff, PxlAddr);
	}else{
...
#if defined (WATERLOGO_PHOTOCAP_FUNCTION) && (WATERLOGO_PHOTOCAP_FUNCTION == ENABLE)
	UiDateImprint_WaterLogoConfig((WATERLOGO_BUFFER *)&g_WaterLogoCap, pInfo->pCapInfo->ImgInfo.ch[0].width,pInfo->pCapInfo->ImgInfo.ch[0].height, pCapInfo->event);

	DS_STAMP_INFOR *Info=&StampWaterLogoInfor;
	UINT32     x, y, uiYAddrOffset, uiUAddrOffset;
	...
	uiYAddrOffset = Img.LineOffset[0] * y + x;
	if(PhotoExe_GetCapYUV420En()){
		uiUAddrOffset = Img.LineOffset[1] * y/2 + x;
	}else{
		uiUAddrOffset = Img.LineOffset[1] * y + x;
	}
	uiLineOffset[0] = Info->Img.LineOffset[0];
	uiLineOffset[1] = Info->Img.LineOffset[0];
	uiPxlAddr[0]    = Info->Img.PxlAddr[0];
	uiPxlAddr[1]    =  Info->Img.PxlAddr[1];
	...
	uiLineOffset[0] = Img.LineOffset[0];
	uiLineOffset[1] = Img.LineOffset[1];
	uiPxlAddr[0]    = Img.PxlAddr[0] + uiYAddrOffset;
	uiPxlAddr[1]    = Img.PxlAddr[1] + uiUAddrOffset;
	...
	if (Info->ColorKey== WATERLOGO_FILL_COLOR){
		GxImg_CopyData(&SrcImg, REGION_MATCH_IMG, &DstImg, REGION_MATCH_IMG);
	}else{
		GxImg_CopyColorkeyData(&DstImg, REGION_MATCH_IMG, &SrcImg, REGION_MATCH_IMG, Info->ColorKey, FALSE, pInfo->copyEngine);
	}
#endif
...
void UiDateImprint_WaterLogoConfig(WATERLOGO_BUFFER *pWaterLogoBuf, UINT32 ImgW, UINT32 ImgH, UINT32 CapEvent)
{
#if defined (WATERLOGO_PHOTOCAP_FUNCTION) && (WATERLOGO_PHOTOCAP_FUNCTION == ENABLE)
	DS_STAMP_INFOR * pWaterLogpCtrl=&StampWaterLogoInfor;
	DATE_IMPRINT_CTRL* pCtrl = &gDateImprintCtrl;
	IMG_BUF     SrcImg, DstImg;
	IRECT       SrcRegion = {0};
	IRECT       DstRegion = {0};
	UINT32      uiScaleYAddr, uiScaleUVAddr;
	UINT32      uiScaleWidth, uiScaleHeight;
	UINT32          uiLineOffset[2] = {0};
	UINT32          uiPxlAddr[2] = {0};
	UINT32          MemAddr = g_DateImprintWaterLogoMemAddr;
	const UINT32 STAMP_LOGO_GAP=60;

	if(pWaterLogoBuf != NULL){
		pWaterLogpCtrl->ColorKey =pWaterLogoBuf->uiWaterLogoColorKey;
			...
			GxImg_InitBufEx(&SrcImg,
                                ...
                                pWaterLogoBuf->uiWaterLogoFormat,
                                uiLineOffset,
                                uiPxlAddr);


			uiScaleYAddr = MemAddr;
			uiScaleUVAddr = uiScaleYAddr + pWaterLogoBuf->uiWidth * pWaterLogoBuf->uiHeight ;
			...
			uiScaleWidth=pWaterLogoBuf->uiWidth *ImgW/GetPhotoSizeWidth(0);
			uiScaleHeight=pWaterLogoBuf->uiHeight * ImgH/GetPhotoSizeHeight(0);
			uiScaleWidth = ALIGN_CEIL_4(uiScaleWidth);
			uiScaleHeight = ALIGN_CEIL_4(uiScaleHeight);

			// debug_msg("Scale w %d, h %d\r\n", uiScaleWidth, uiScaleHeight);
			...
			//debug_msg("^YLogo Src: x %d, y %d, w %d, h %d\r\n", SrcRegion.x, SrcRegion.y, SrcRegion.w, SrcRegion.h);


			DstRegion.x = 0;
			DstRegion.y = 0;
			DstRegion.w = uiScaleWidth;
			DstRegion.h = uiScaleHeight;
			//debug_msg("^YLogo Dst: x %d, y %d, w %d, h %d\r\n", DstRegion.x, DstRegion.y, DstRegion.w, DstRegion.h);

			// Scale image by ISE
			GxImg_ScaleData(&SrcImg, &SrcRegion, &DstImg, &DstRegion);
            }
            pWaterLogpCtrl->Img =DstImg;


            if (pWaterLogoBuf->uiXPos == WATERLOGO_AUTO_POS && pWaterLogoBuf->uiYPos == WATERLOGO_AUTO_POS){
			pWaterLogpCtrl->PosX=STAMP_LOGO_GAP*ImgW/GetPhotoSizeWidth(0);
			pWaterLogpCtrl->PosY= pCtrl->info[CapEvent-1].StampInfo.PosY+ pCtrl->info[CapEvent-1].StampInfo.Img.Height- pWaterLogpCtrl->Img.Height;
            }
	}
#endif
}
