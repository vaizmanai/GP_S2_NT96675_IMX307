...
void IQSISP_get_pathcurve(void);
...
void IQSISP_set_pathcurve(void);
...
void IQSISP_get_pathcurve(void)
{
	UINT32 *pData = (UINT32 *)MsdcNvt_GetDataBufferAddress();
	UINT32 uiLength = MsdcNvt_GetTransSize();
	sISP_PATHCURVE *sPtr = (sISP_PATHCURVE *)pData;
	sPtr->Id = sensor_id;

	iqt_get_param(ISP_PATHCURVE, (UINT32)pData, uiLength);
}
...
void IQSISP_set_pathcurve(void)
{
	...
	iqt_set_param(ISP_PATHCURVE, (UINT32)pData, uiLength);
}
