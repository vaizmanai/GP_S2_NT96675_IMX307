...
#define MSDCNVT_GETMSDCOBJ() 			Msdc_getObject(MSDC_ID_USB30)
#else
#define MSDCNVT_GETMSDCOBJ() 			Msdc_getObject(MSDC_ID_USB20)
#endif
#else //NA51023
#define MSDCNVT_GETMSDCOBJ() 			Msdc_getObject(MSDC_ID_USB20) //na51023 only obj 2.0
#endif
#else //!(USB_MODE)
#define MSDCNVT_GETMSDCOBJ()			NULL
#endif
...
#define CFG_MSDC_NVT_CB_DISP    DISPLAY_FUNC  //!< Display Engine Adjustment (IDE Gamma)
#define CFG_MSDC_NVT_CB_FILE    FS_FUNC  //!< Remote File System
#define CFG_MSDC_NVT_CB_ADJ     ENABLE  //!< Universal Adjustment
#define CFG_MSDC_NVT_CB_UPDFW   FWS_FUNC  //!< Update Fw
#if (defined(_MODEL_IPCAM1_EVB_) || defined(_MODEL_DVCAM1_EVB_) || defined(_MODEL_DVCAM2_EVB_))
...
#define CFG_SI_MODULE SI_MODULE_NONE
#else
...
static BOOL m_IsUsbConnected = FALSE;
...
#error "You have to select a cache source!"
	...
	if (m_bInit == FALSE) {
		...
		MsdcNvtInit.pMsdc            = MSDCNVT_GETMSDCOBJ();
...
BOOL MsdcNvtCb_IsUsbConnected(void)
{
