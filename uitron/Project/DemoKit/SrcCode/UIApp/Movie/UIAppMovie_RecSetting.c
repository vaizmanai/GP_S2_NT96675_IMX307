...
MOVIE_STRM_INFO gMovie_Strm_Info = {
	...
	_CFG_AUD_CODEC_NONE,//_CFG_AUD_CODEC_AAC,			//MOVIE_CFG_AUD_CODEC
...
MOVIEMULTI_RECODE_FOLDER_NAMING gMovie_Folder_Naming[SENSOR_MAX_NUM] = {
	{
		_CFG_REC_ID_1,			//MOVIE_CFG_REC_ID
		"DCIM",				//MOVIE_CFG_FOLDER_NAME   movie
		"LOCK",					//MOVIE_CFG_FOLDER_NAME   emr
		"DCIM",				//MOVIE_CFG_FOLDER_NAME   snapshot  modify for autotest
...
MOVIEMULTI_RECODE_FOLDER_NAMING gMovie_Clone_Folder_Naming[SENSOR_MAX_NUM] = {
	{
		_CFG_CLONE_ID_1,		//MOVIE_CFG_REC_ID
		"DCIM_S",				//MOVIE_CFG_FOLDER_NAME   movie
		"LOCK_S",				//MOVIE_CFG_FOLDER_NAME   emr
...
MOVIEMULTI_AUDIO_INFO   gMovie_Audio_Info_right = {
#if ((_BOARD_DRAM_SIZE_ == 0x04000000) || (SENSOR_CAPS_COUNT >= 2) || defined(_ASR_ENABLE_))
	16000,                     //MOVIE_CFG_AUD_RATE
	AUDIO_CH_RIGHT,            //MOVIE_CFG_AUD_CH
	1,                         //MOVIE_CFG_AUD_CH_NUM
#else
	48000,                     //MOVIE_CFG_AUD_RATE  yxf_debug 20191127
	AUDIO_CH_RIGHT,            //MOVIE_CFG_AUD_CH
	1,                         //MOVIE_CFG_AUD_CH_NUM
#endif
};
...
MOVIEMULTI_AUDIO_INFO   gMovie_Audio_Info_left = {
#if ((_BOARD_DRAM_SIZE_ == 0x04000000) || (SENSOR_CAPS_COUNT >= 2) || defined(_ASR_ENABLE_))
	16000,                     //MOVIE_CFG_AUD_RATE
	AUDIO_CH_LEFT,            //MOVIE_CFG_AUD_CH
	1,                         //MOVIE_CFG_AUD_CH_NUM
#else
	48000,                     //MOVIE_CFG_AUD_RATE 20191127
	AUDIO_CH_LEFT,            //MOVIE_CFG_AUD_CH
	1,                         //MOVIE_CFG_AUD_CH_NUM
#endif
};
...
UINT32 Movie_GetFreeSec(void)
{
	...
	for (i = 0; i < SENSOR_CAPS_COUNT; i++) {
		if (movie_rec_mask & mask) {
			...
			movie_setting.info[rec_id].audSampleRate = gMovie_Audio_Info_right.aud_rate;
			if(UI_GetData(FL_MOVIE_AUDIO) == MOVIE_INTER_MIC)
			{
				movie_setting.info[rec_id].audChs = gMovie_Audio_Info_right.aud_ch;
			}
			else
			{
				movie_setting.info[rec_id].audChs = gMovie_Audio_Info_left.aud_ch;	
			}
