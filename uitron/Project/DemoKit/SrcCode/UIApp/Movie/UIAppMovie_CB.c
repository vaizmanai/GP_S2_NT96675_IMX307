...
void Movie_3NDRCB(UINT32 id, UINT32 p3DNRConfig)
{
	UINT32 link_id, sensor_id;

	link_id = ImageApp_MovieMulti_VePort2Imglink(id);
	if (link_id < SENSOR_CAPS_COUNT) {
		sensor_id = IPL_PATH(ImageApp_MovieMulti_VePort2Imglink(id));
...
static UINT32 UIMovie_ViewTracking_CB(UINT32 id, UINT32 frame_cnt, INT32* pOffx, INT32* pOffy)
{
...
{
	UINT32 i, mask, movie_rec_mask, clone_rec_mask;
	...
	if (System_GetEnableSensor() == (SENSOR_2)) {
        ...
        #if MOVIE_DIS
        ImageUnit_SetParam(ISF_CTRL, IMAGEPIPE_PARAM_VIEWTRACKING_CB, (UINT32)UIMovie_ViewTracking_CB);
	...
	for (i = 0; i < SENSOR_CAPS_COUNT; i++) {
		if (movie_rec_mask & mask) {
			ImageApp_MovieMulti_SetParam(gMovie_Rec_Info[i].rec_id, MOVIEMULTI_PARAM_3DNR_CB, (UINT32)Movie_3NDRCB);
		}

		if (clone_rec_mask & mask) {
			ImageApp_MovieMulti_SetParam(gMovie_Clone_Info[i].rec_id, MOVIEMULTI_PARAM_3DNR_CB, (UINT32)Movie_3NDRCB);
		}
