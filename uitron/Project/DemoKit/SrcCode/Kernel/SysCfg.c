...
//#include "UsockIpcAPI.h"        //sa4 removed
...
//#NT#2016/06/28#KCHong -end
    ...
    //#NT#2017/11/06#Charlie Chang -begin
    //#NT# install detect tv task
    tv_installID();
    //#NT#2017/11/06#Charlie Chang -end
    alg_InstallID();
...
#if (PTY_FUNC == ENABLE)
	nvt_ivot_ptyd_install_id();
#endif

#if (IPCAM_PARAM_INI == ENABLE)
	UCtrlAppMovieIni_InstallID();
#endif
...
//for fileDB
	...
	OS_CONFIG_MEMPOOL(POOL_ID_FILEDB,        POOL_SIZE_FILEDB * 2,          POOL_CNT_FILEDB);        // FileDB working buffer
	#else
...
#if defined(_ASR_ENABLE_)
	OS_CONFIG_MEMPOOL(POOL_ID_ASR,           POOL_SIZE_ASR,             POOL_CNT_ASR);               // ASR (automatic sppech recognition)
#endif
}
...
#else
	#if ((STAMP_FUNC == ENABLE) && (VIDEOSPRITE_STAMP_FUNC == ENABLE))
...
#if (DISPLAY_OSD_FMT == DISP_PXLFMT_ARGB8888_PK)
		osd1_size = ALIGN_CEIL_32(OSD_W*OSD_H*4);
#elif (DISPLAY_OSD_FMT == DISP_PXLFMT_ARGB8565_PK)
		osd1_size = ALIGN_CEIL_32(OSD_W*OSD_H*3);
#elif ((DISPLAY_OSD_FMT == DISP_PXLFMT_ARGB4444_PK)||(DISPLAY_OSD_FMT == DISP_PXLFMT_ARGB1555_PK))
		osd1_size = ALIGN_CEIL_32(OSD_W*OSD_H*2);
#elif (DISPLAY_OSD_FMT == DISP_PXLFMT_INDEX8)
		osd1_size = ALIGN_CEIL_32(OSD_W*OSD_H);
#else
		debug_msg("ERR: invalid OSD1 fmt!\r\n");
#endif

	OS_CONFIG_MEMPOOL(POOL_ID_DISP_OSD1,     osd1_size,       POOL_CNT_DISP_OSD1*2);     // Display OSD1 buffer
	}
	#endif
...
#if (VDO_USE_ROTATE_BUFFER==ENABLE) // save mem if not a rotated panel.
...
#if (MSDCVENDOR_NVT == ENABLE)
	OS_CONFIG_MEMPOOL(POOL_ID_MSDCNVT, POOL_SIZE_MSDCNVT, POOL_CNT_MSDCNVT);
#endif
