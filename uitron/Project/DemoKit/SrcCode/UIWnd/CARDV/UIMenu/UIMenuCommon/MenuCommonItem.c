...
#define MENU_KEY_RELEASE_MASK      FLGKEY_MODE
...
EVENT_ITEM(NVTEVT_KEY_ENTER,MenuCommonItem_OnKeyRight)
...
#if (PROTECT_DELETE_PATCH == ENABLE)
#include "PlaybackTsk.h"
CHAR g_chFullName[60] = {0};

void __clear_filepath__(void)
{
    memset((void *)g_chFullName, 0, sizeof(g_chFullName));
}
...
INT32 MenuCommonItem_OnOpen(VControl *pCtrl, UINT32 paramNum, UINT32 *paramArray)
{
    ...
    pCurMenu->pPages[1]= gSetupMenu.pPages[0];
...
#if (PROTECT_DELETE_PATCH == ENABLE)
    if ((pMenu->pPages[pMenu->SelPage]).TextId == STRID_PLAYBACK)
    {
        PB_GetParam(PBPRMID_CURR_FILEPATH, (UINT32 *)&g_chFullName);
        //debug_msg("^G>>>>>>>>>>save g_chFullName=%s\r\n",g_chFullName);
    }
#endif
...
INT32 MenuCommonItem_Menu_OnKeyExit(VControl *, UINT32, UINT32 *);
...
//EVENT_ITEM(NVTEVT_KEY_SHUTTER2,MenuCommonItem_Menu_OnKeyExit)
...
INT32 MenuCommonItem_Menu_OnKeyExit(VControl *pCtrl, UINT32 paramNum, UINT32 *paramArray)
{
    Ux_CloseWindow(&MenuCommonItemCtrl,0);
    return NVTEVT_CONSUME;
}
