...
BOOL Judge_Allfile_Prop(void)
{
    FILEDB_HANDLE     FileDBHdl = 0;
    PFILEDB_FILE_ATTR FileAttr = NULL;
    INT32             FileNum,i;
    BOOL  isCurrFileReadOnly = TRUE;
    //Perf_Mark();
    ...
    for (i=FileNum-1;i>=0;i--)
    {
	   ...
	   {
		isCurrFileReadOnly = FALSE;
		break;
	    }
        }
    }
    return  isCurrFileReadOnly;
}

#if (PROTECT_DELETE_PATCH == ENABLE)
extern CHAR g_chFullName[];
extern void __clear_filepath__(void);
#endif
    ...
    if(UxMenu_GetData(pCtrl,MNU_CURITM)==UIMenuWndPlayConfirmDel_Menu_STRID_OK)
    {
        ...
        case UIMenuWndPlayConfirmDel_StatusText_STRID_ERASE_THIS:
	      ...
	      FileAttr = FileDB_CurrFile(FileDBHdl);
            uiLockStatus = M_IsReadOnly(FileAttr->attrib);
