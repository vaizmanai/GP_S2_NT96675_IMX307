...
void FlowPB_IconDrawEVIcon(BOOL bShow)
{
    ...
    else if (!strncmp(str,"+5/3",4) || !strncmp(str,"+1.3",4))
...
static void FlowPB_IconDrawDCFFileID(BOOL bShow)
{
    ...
    if (uiPBFileFmt & PBFMT_AVI ||
        ...
        uiPBFileFmt & PBFMT_JPG)
    {
		strncpy(item1_Buf, (strrchr(gchaFullName, '\\')+1),strstr(gchaFullName,".")-(strrchr(gchaFullName, '\\')+1));
		item1_Buf[strstr(gchaFullName,".")-(strrchr(gchaFullName, '\\')+1)] = '\0';	
    }
    else 
    {
...
static void FlowPB_IconDrawImageSize(BOOL bShow)
{
    ...
    if(uiFileFmt & (PBFMT_MOVMJPG | PBFMT_AVI | PBFMT_MP4 | PBFMT_TS))
    {
        ...
        case 2880: //2880X2160P24
            snprintf(item1_Buf,32,"2160P");
            break;
        case 1920:
            if(MovieInfo.uiVidRate == 60)
                snprintf(item1_Buf,32,"1080P60");
            else if(MovieInfo.uiVidRate == 96)
                snprintf(item1_Buf,32,"1080P96");//yxf_debug 20191114
            else
		   snprintf(item1_Buf,32,"1080P30");//yxf_debug 20191114			
            break;
        case 1440:
		...
		else if(MovieInfo.uiVidRate == 30)	
...
void FlowPB_IconDrawMovBwd(BOOL bShow)
{
    if (bShow==TRUE)
    {
        ...
        {
            UxState_SetItemData(&UIFlowWndPlay_StatusICN_MovBwdCtrl,UIFlowWndPlay_StatusICN_MovBwd_ICON_FB_LEFT,STATE_ITEM_ICONID,ICON_PLAY_BACKWARD);
        }
        else
        {
            UxState_SetItemData(&UIFlowWndPlay_StatusICN_MovBwdCtrl,UIFlowWndPlay_StatusICN_MovBwd_ICON_FB_LEFT,STATE_ITEM_ICONID,ICON_FB_LEFT);
        }
...
void FlowPB_IconDrawMovFwd(BOOL bShow)
{
    if (bShow==TRUE)
    {
        if(g_PlbData.State == PLB_ST_PAUSE_MOV)
        {
            UxState_SetItemData(&UIFlowWndPlay_StatusICN_MovFwdCtrl,UIFlowWndPlay_StatusICN_MovFwd_ICON_FF_RIGHT,STATE_ITEM_ICONID,ICON_PLAY_FORWARD);
        }
        else
        {
            UxState_SetItemData(&UIFlowWndPlay_StatusICN_MovFwdCtrl,UIFlowWndPlay_StatusICN_MovFwd_ICON_FF_RIGHT,STATE_ITEM_ICONID,ICON_FF_RIGHT);
        }
...
void FlowPB_IconDrawMovPlay(BOOL bShow)
{
    ...
    else if(g_PlbData.State == PLB_ST_PAUSE_MOV)
    {
        ...
        UxState_SetItemData(&UIFlowWndPlay_StatusICN_MovPlayCtrl,UIFlowWndPlay_StatusICN_MovPlay_ICON_PLAY_PLAY,STATE_ITEM_ICONID,ICON_PLAY_STEP);
    }
    else
    {
        UxState_SetData(&UIFlowWndPlay_StatusICN_MovPlayCtrl,STATE_CURITEM,UIFlowWndPlay_StatusICN_MovPlay_ICON_PLAY_PLAY);
        UxState_SetItemData(&UIFlowWndPlay_StatusICN_MovPlayCtrl,UIFlowWndPlay_StatusICN_MovPlay_ICON_PLAY_PLAY,STATE_ITEM_ICONID,ICON_PLAY_PLAY);
    }
...
void FlowPB_IconDrawMovPlayTime(BOOL bShow)
{
    ...
    uiPlayTimeSecs = AppDisp_MoviePlayView_GetCurrPlaySecs();//yxf_debug 20191112
