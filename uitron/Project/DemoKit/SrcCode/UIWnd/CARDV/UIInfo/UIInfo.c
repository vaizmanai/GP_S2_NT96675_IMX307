...
void KeyScan_EnableMisc(BOOL bEn)
{
    if (bEn == FALSE)
    {
        ...
        //SxTimer_SetFuncActive(SX_TIMER_DET_USB_ID, FALSE);//yxf_debug
        ...
        SxTimer_SetFuncActive(SX_TIMER_DET_USB_ID, TRUE);//yxf_debug
...
void SysCheckFlag(void)
{
    ...
    SysLimitFlag(FL_MOVIE_RATE,   0,  MOVIE_RATE_MAX,     DEFAULT_MOVIE_RATE);
    ...
    SysLimitFlag(FL_MOVIE_AUDIO_LAST,        0,  MOVIE_AUDIO_ID_MAX,         DEFAULT_MOVIE_AUDIO);
    SysLimitFlag(FL_MOVIE_AUDIO_VOL,        0,  MOVIE_VOL_MAX,         DEFAULT_MOVIE_REC_VOLUME);
    ...
    //#NT#2017/03/29#Isiah Chang -begin
...
void SysResetFlag(void)
{
    ...
    SysSetFlag(FL_MOVIE_RATE,          DEFAULT_MOVIE_RATE);
    ...
    SysSetFlag(FL_MOVIE_AUDIO_LAST,          DEFAULT_MOVIE_AUDIO);
    SysSetFlag(FL_MOVIE_AUDIO_VOL,          DEFAULT_MOVIE_REC_VOLUME);
    ...
    //memset(currentInfo.strSSID,'\0', NVT_WSC_MAX_SSID_LEN);
	//memcpy(currentInfo.strSSID,UINet_GetDefaultAPSSID,NVT_WSC_MAX_SSID_LEN);
	//memcpy(currentInfo.strPASSPHRASE,UINet_GetDefaultAPPASSPHRASE,NVT_MAX_WEP_KEY_LEN);
   // memset(currentInfo.strPASSPHRASE,'\0', NVT_MAX_WEP_KEY_LEN);
    ...
    SysSetFixedFlag();
    memset(currentInfo.strSSID,'\0', NVT_WSC_MAX_SSID_LEN);
    memset(currentInfo.strPASSPHRASE,'\0', NVT_WSC_MAX_SSID_LEN);
	//UINet_Reset_gSSID();
    UINet_SetSSID(UINet_GetDefaultAPSSID(),strlen(UINet_GetDefaultAPSSID()));
    //UINet_Reset_gPASSPHRASE();
    UINet_SetPASSPHRASE(UINet_GetDefaultAPPASSPHRASE(),strlen(UINet_GetDefaultAPPASSPHRASE()));
    //#NT#2017/03/29#Isiah Chang -end
...
void FlowDCF_UpdateName(void)
{
 ...
 CHAR filePath[DCF_FULL_FILE_PATH_LEN];
    ...
    curDate = rtc_getDate();
    curTime = rtc_getTime();
    snprintf(FolderName, sizeof(FolderName),"%1d%02d%02d",curDate.s.year%0x0A,curDate.s.month,curDate.s.day);
    snprintf(FileName,sizeof(FileName),"%02d%02d",curTime.s.hour,curTime.s.minute);
