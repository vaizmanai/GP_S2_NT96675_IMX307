...
//for rotat LCD and TV/HDMI plug in/out
#if (VDO_USE_ROTATE_BUFFER == ENABLE)
	if (CurDevObj == (UINT32)gDevLCDObj) {
...
static void UI_EnableVideo(UINT32 DevID, UINT32 CurDevObj)
{
	...
	if ((id == 0) && (CurDevObj == (UINT32)gDevLCDObj)) {
		#if(VDO_ROTATE_DIR == ISF_VDO_DIR_ROTATE_90)
		View_Window_ConfigAttr(id, OSD_ATTR_ROTATE, OSD_ROTATE_90_DEGREE); //enable rotate 90
		#else
...
void System_ConnectDisplay(UINT32 DisplayMask)
{
	uiDisplayEnableState_hw |= DisplayMask;
}
void System_DisconnectDisplay(UINT32 DisplayMask)
{
	uiDisplayEnableState_hw &= ~DisplayMask;
}
...
#endif
//#NT#2017/11/20#Charlie Chang -begin
//#NT# display default disable
	#if  (DISPLAY1_DEFAULT_DISABLE == ENABLE)
		_System_DetachDisplay(DOUT1);
	#endif
//#NT#2017/11/20#Charlie Chang -end
#if (DISPLAY2_FUNC == ENABLE)
#if( ((LCD2_FUNC == ENABLE) && (DISPLAY2_DEFAULT_ATTACH_DEVICE == LCD2_MASK)) \
      || ((TV_FUNC == ENABLE) && (DISPLAY2_DEFAULT_ATTACH_DEVICE == TV_MASK)) \
      || ((HDMI_FUNC == ENABLE) && (DISPLAY2_DEFAULT_ATTACH_DEVICE == HDMI_MASK)) \
...
#include "NvtDispIpcAPI.h"
	{
		NVTDISPIPC_OPEN disp_open = {0};
		disp_open.sharedMemAddr = OS_GetMempoolAddr(POOL_ID_DISP_OSD1);
		disp_open.sharedMemSize = OS_GetMempoolSize(POOL_ID_DISP_OSD1);
		if (E_OK != NvtDispIpc_Open(&disp_open)) {
			DBG_ERR("NvtDispIpc_Open fail\r\n");
			return;
		}
...
{
    //do enable to TV device
	// now only for IPCAM TV!!!!!
#if(IPCAM_FUNC == ENABLE)
...
void SysVideo_OnVideoRemove(void)
{
	...
	if(iDispPath != -1){
		...
		ImageApp_IPCam_VideoOut_set_status(iDispPath,ISF_PORT_STATE_READY);
#endif
