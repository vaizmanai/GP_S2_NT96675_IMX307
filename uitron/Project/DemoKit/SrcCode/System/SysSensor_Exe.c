...
{
	SENSOR_MULTI_ID sensor_gp[VDOIN_MAX_NUM], sensor_subgp[VDOIN_MAX_NUM];
...
void System_OnSensorInit(void)
{
	...
	// init sensor mapping between logic and physical
	if (bFirst) {
		_System_MapSensor();
		bFirst = FALSE;
	}
...
void System_OnSensorInit2(void)
{
	...
	if (System_GetState(SYS_STATE_POWERON) == SYSTEM_POWERON_NORMAL)
		if (System_GetConnectSensor() != (SENSOR_1 | SENSOR_2 | SENSOR_3)) {
			INT32 iCurrMode = System_GetBootFirstMode();
#if (PHOTO_MODE==ENABLE)
			if (iCurrMode == PRIMARY_MODE_PHOTO) {
				System_EnableSensor_FastDevice(); //enable 1 sensors
...
#if (SENSOR_INSERT_MASK == SENSOR_2)
static BOOL g_bFirstBootHotPlug = TRUE;
#endif

void _System_InitSensor(void)
{
	if (g_SensorInit == TRUE)
		return;
...
#endif
#if (SENSOR_INSERT_MASK == SENSOR_2)
    drv_sensor_power_turn_on(IPL_PATH(1));
	if (drv_sensor_det_plug_in(IPL_PATH(1))) {
		uiSensorEnableState_hw |= SENSOR_2;
	}else{
		if (g_bFirstBootHotPlug) {
			g_bFirstBootHotPlug = FALSE;
		}
	}
#endif
	g_SensorInit = TRUE;
...
static void _System_AttachSensor(UINT32 phy_SensorID)
{
	...
	if (sensor_is_open(phy_SensorID) == FALSE) {
		sensor_open(phy_SensorID, &sen_handle);
	}
#if (POWERON_FAST_SENSOR == ENABLE)
	if (bFirst) {
		SEN_CHGMODE_DATA sen_chgmode;
		sen_chgmode.frame_rate = IPL_AlgGetUIInfo(0, IPL_SEL_VIDEO_FPS);
		sen_chgmode.mode = POWERON_FAST_SENSOR_MODE;
		sen_chgmode.multi_id = 1 << phy_SensorID;
		sensor_chg_mode(phy_SensorID, sen_chgmode);
	}
#endif
...
//#NT#2018/10/03#KS Hung -begin
//#NT#680 will not change mode again.
#if 0//(WIFI_FUNC==ENABLE)
//#NT#2016/06/23#Isiah Chang -begin
//#NT#Notify APP to restart RTSP live view becaues of sensor plug/unplug event.
	...
	if (uiSensorLUT[new_device_id] == SENSOR_1) {
		...
		if (uiSensorLUT[new_device_id] == SENSOR_2) {
			...
			if (uiSensorLUT[new_device_id] == SENSOR_1) {
				...
				if (uiSensorLUT[new_device_id] == SENSOR_2) {
		 			if (g_bFirstBootHotPlug == FALSE){
		 				INT iCurrMode = System_GetState(SYS_STATE_CURRMODE);
		 				if (iCurrMode == PRIMARY_MODE_MOVIE){
		 					Ux_SendEvent(0, NVTEVT_SENSOR_ATTACH, 1, PRIMARY_MODE_MOVIE);
		 					Ux_SendEvent(Ux_GetActiveApp(), NVTEVT_EXE_MOVIE_SENSORHOTPLUG, 0);
		 				}else if (iCurrMode == PRIMARY_MODE_PHOTO){
		 					Ux_SendEvent(0, NVTEVT_SENSOR_ATTACH, 1, PRIMARY_MODE_PHOTO);
		 					Ux_SendEvent(Ux_GetActiveApp(), NVTEVT_EXE_SENSORHOTPLUG, 0);
		 				}else if (iCurrMode == PRIMARY_MODE_PLAYBACK){
							//Ux_SendEvent(0, NVTEVT_SYSTEM_MODE, 1, System_GetState(SYS_STATE_CURRMODE));
						}else if (iCurrMode == PRIMARY_MODE_USBPCC){
							Ux_SendEvent(0, NVTEVT_SYSTEM_MODE, 1, System_GetState(SYS_STATE_CURRMODE));
		 				}else{
							//Ux_SendEvent(0, NVTEVT_SYSTEM_MODE, 1, System_GetState(SYS_STATE_CURRMODE));
		 				}
				...
				if (uiSensorLUT[old_device_id] == SENSOR_2) {
					if (g_bFirstBootHotPlug == FALSE){
						...
						if (iCurrMode == PRIMARY_MODE_MOVIE){
							Ux_SendEvent(0, NVTEVT_SENSOR_ATTACH, 1, PRIMARY_MODE_MOVIE);
							Ux_SendEvent(Ux_GetActiveApp(), NVTEVT_EXE_MOVIE_SENSORHOTPLUG, 0);
							ipl_ext_uninit(IPL_PATH(1));//uninit again
						}else if (iCurrMode == PRIMARY_MODE_PHOTO){
							Ux_SendEvent(0, NVTEVT_SENSOR_ATTACH, 1, PRIMARY_MODE_PHOTO);
							Ux_SendEvent(Ux_GetActiveApp(), NVTEVT_EXE_SENSORHOTPLUG, 0);
							ipl_ext_uninit(IPL_PATH(1));//uninit again
...
#if (SENSOR_CAPS_COUNT == 3)
			{
				System_EnableSensor((SENSOR_1 | SENSOR_2 | SENSOR_3)); //enable all connected sensors
