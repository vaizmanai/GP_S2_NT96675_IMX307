...
UINT32 iq_get_iso_range(UINT32 Id, IQS_FLOW_MSG MsgID, UINT32 *IsoValue, UINT32 *IsoStart, UINT32 *IsoEnd, UINT32 *IQIdxL, UINT32 *IQIdxH)
{
    ...
    IPL_SEL_ITEM ipl_sel_item = IPL_SEL_PRVSENMODE;
	...
	if (iq_check_capture(MsgID) == FALSE) { // movie mode
        ...
        ipl_sel_item = IPL_SEL_PRVSENMODE;
	} else {                            // capture mode
		AE_GetCapAEArg(Id, &ISO, &ExpT, &Iris);
        ipl_sel_item = IPL_SEL_CAPSENMODE;
	}

    sensor_status.sensor_mode = IPL_AlgGetUIInfo((IPL_PROC_ID)Id, ipl_sel_item);
...
UINT32 iq_get_ycurve_mode(UINT32 Id)
{
	if (IPL_CtrlGetInfor(Id, IPLCTRL_EXT_FUNC_EN) & ICF_FUNC_WDR) {
        ...
        if(IPL_AlgGetUIInfo((IPL_PROC_ID)Id, IPL_SEL_SHDR) == SEL_SHDR_OFF) {
            ...
            if(g_hdr_high_addr[Id]->uiCtrlMode == SHDR_CTRL_byIQPara) {
                return IQS_Y_MANUAL;
            } else {
                return IQS_Y_AUTO;
            }
...
void iq_get_dce_param(UINT32 Id, UINT32 MsgID, UINT32 ZoomIdx, DCE_Distortion *pDistortion, DIST *pDce_Dist, Fact_Norm *pRadious)
{
    ...
    if (IPL_CtrlGetInfor(Id, IPLCTRL_PRVFLOW)==ICF_FLOW_DIRECT) {
        pDistortion->gdc_mode_xonly = 1;    ///< 0 -> both X-Y, 1 -> X only
    } else {
        pDistortion->gdc_mode_xonly = 0;    ///< 0 -> both X-Y, 1 -> X only
    }
...
IQS_TMNR_YCC_COMPRESSION_INFO iqs_tmnr_ycc_compression_info =
{
    #if 0
    0x07,
    0x05
...
IQS_TMNR_YCC_COMPRESSION_INFO *iqs_get_ycc_compression_info(void)
{
    return &iqs_tmnr_ycc_compression_info;
};
...
INT32 UI_3DNR[ISP_IQ_MAX_NUM] = {-1,-1};
...
void iqs_get_isp_info(sISP_isp_info	*info)
{
	...
	for (i=0; i<Total_SENIDNum; i++)
	{
        if (pqs_iq_check(i) == TRUE) {
            rt = sensor_get_info((SENSOR_ID)i, SEN_GET_NAME, sen_name);
			if (rt != E_OK) {
		        DBG_ERR("sensor_get_info fail\r\n");
			}
            memcpy(drv_name->sensor_name, sen_name, 32);
        }
		...
		for(j = 0;j<SENSOR_MODE_Num;j++)
            if(pqs_iq_check(i)==TRUE) {
                ...
                if (IPL_CtrlGetInfor(i, IPLCTRL_PRVFLOW) == ICF_FLOW_C) {
                    isp_info.iqs_info[i].iq_feature |= IQ_FEATURE_RAW_AVAILABLE;
                }

                if (iq_check_rgbir(i) == TRUE) {
                    isp_info.iqs_info[i].iq_feature |= IQ_FEATURE_RGBIR;
                }

                //debug_msg("~G iq_feature = 0x%X \r\n", isp_info.iqs_info[i].iq_feature);
            }
	}
...
UINT32* iq_get_pathcurve_param(UINT id)
{
	return g_pathcurve_buf[id];
}
...
void iq_get_status(sISP_IQ_STATUS *iq_status)
{
    ...
    iq_status->hdr_strength = 128;
}
UINT32 g_qp_value[ISP_IQ_MAX_NUM][3]={{-1,-1,-1},{-1,-1,-1}};
UINT32 iq_get_qp_value(UINT32 id,UINT32 uiSliceType )
{
    return g_qp_value[id][uiSliceType];
}
void iq_set_qp_value(UINT32 id,UINT32 uiSliceType, UINT32 qp)
{
    g_qp_value[id][uiSliceType] = qp;
}