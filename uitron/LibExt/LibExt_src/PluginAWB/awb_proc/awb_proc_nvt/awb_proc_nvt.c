...
static UINT16 acc_tab_temp[AWBALG_CH_MAX][AWB_CABUFFER_SIZE];
static UINT16 acc_cnt[AWB_PROC_CNT][AWB_CABUFFER_SIZE];
static UINT16 acc_cnt_temp[AWB_CABUFFER_SIZE];
...
void AWB_Init(UINT32 id, UINT32 CurrentStatus)
{
    ...
    awb_info[id].rgb_ir.level = 0;
    awb_info[id].rgb_ir.irsub_rg = 0;
    awb_info[id].rgb_ir.irsub_b = 0;
...
void AWB_Process(UINT32 id, UINT32 CurrentStatus)
{
    ...
    if (ui_r_gain[id] != 0 && ui_g_gain[id] != 0 && ui_b_gain[id] != 0) {
        ...
        awb_gain_set.r_gain = awb_info[id].AWBGain.r_gain * awb_info[id].AwbUserRratio / 100;
		awb_gain_set.g_gain = awb_info[id].AWBGain.g_gain;
		awb_gain_set.b_gain = awb_info[id].AWBGain.b_gain * awb_info[id].AwbUserBratio / 100;

        if (awb_gain_set.r_gain > 2047) awb_gain_set.r_gain = 2047;
        if (awb_gain_set.b_gain > 2047) awb_gain_set.b_gain = 2047;
        if (awb_gain_set.r_gain < 1) awb_gain_set.r_gain = 1;
        if (awb_gain_set.b_gain < 1) awb_gain_set.b_gain = 1;

        AWB_setGain(&awb_info[id], awb_gain_set.r_gain, awb_gain_set.g_gain, awb_gain_set.b_gain);
...
void awb_get_rgbir_level(UINT32 id, UINT32 ir_sub_rg,UINT32 ir_sub_b, UINT32 *level)
{
    awb_info[id].rgb_ir.irsub_rg = ir_sub_rg;
    awb_info[id].rgb_ir.irsub_b = ir_sub_b;
    *level = awb_info[id].rgb_ir.level;
}
...
static void AWB_getCA(UINT32 id, UINT16 *R, UINT16 *G, UINT16 *B, UINT16 *IR, UINT16 *Rth, UINT16 *Gth, UINT16 *Bth, UINT16 *Cnt, UINT32 Size)
{
    ...
    ER rt = E_OK;
	sen_info.mode = NULL;
    SIE_CA_WIN_INFO ca_win_info;
    SIE_CA_IRSUB_INFO ir_sub;
    SIE_STCS_CA_RSLT_INFO ca_info;

    if (awb_app_mode == AWB_APPMODE_NORMAL) {
        CA1Info.id = id;
        CA1Info.r = acc_tab_temp[AWBALG_CH_R];
        CA1Info.g = acc_tab_temp[AWBALG_CH_G];
        CA1Info.b = acc_tab_temp[AWBALG_CH_B];
        CA1Info.ir = acc_tab_temp[AWBALG_CH_IR];
        CA1Info.r_th = acc_tab_temp[AWBALG_CH_Rth];
        CA1Info.g_th = acc_tab_temp[AWBALG_CH_Gth];
        CA1Info.b_th = acc_tab_temp[AWBALG_CH_Bth];
        CA1Info.acc_cnt = acc_cnt_temp;
        ...
        for (i = 0; i < AWB_WIN_TOTAL; i++) {
            R[i] = ((CA1Info.r[i]*DGain) >> (19 - AWB_ACC_BIT));
            G[i] = ((CA1Info.g[i]*DGain) >> (19 - AWB_ACC_BIT));
            B[i] = ((CA1Info.b[i]*DGain) >> (19 - AWB_ACC_BIT));
            IR[i] = ((CA1Info.ir[i]*DGain) >> (19 - AWB_ACC_BIT));
            Rth[i] = ((CA1Info.r_th[i]*DGain) >> (19 - AWB_ACC_BIT));
            Gth[i] = ((CA1Info.g_th[i]*DGain) >> (19 - AWB_ACC_BIT));
            Bth[i] = ((CA1Info.b_th[i]*DGain) >> (19 - AWB_ACC_BIT));

            if (CA1Info.acc_cnt[i] == 0) {
                CA1Info.acc_cnt[i] = 1;
            }
            Cnt[i] = (CA1Info.acc_cnt[i]);
        }
        ...
        for (j = 0; j < AWB_WIN_TOTAL; j++) {
            if (CA1Info.acc_cnt[j] > CA2Info.acc_cnt[j]) {
                R[j] = ((CA1Info.r[j]*DGain) >> (19 - AWB_ACC_BIT));
                G[j] = ((CA1Info.g[j]*DGain) >> (19 - AWB_ACC_BIT));
                B[j] = ((CA1Info.b[j]*DGain) >> (19 - AWB_ACC_BIT));
                IR[j] = ((CA1Info.ir[j]*DGain) >> (19 - AWB_ACC_BIT));
                Rth[j] = ((CA1Info.r_th[j]*DGain) >> (19 - AWB_ACC_BIT));
                Gth[j] = ((CA1Info.g_th[j]*DGain) >> (19 - AWB_ACC_BIT));
                Bth[j] = ((CA1Info.b_th[j]*DGain) >> (19 - AWB_ACC_BIT));
                Cnt[j] = (CA1Info.acc_cnt[j]);
            } else {
                R[j] = ((CA2Info.r[j]*DGain) >> (19 - AWB_ACC_BIT));
                G[j] = ((CA2Info.g[j]*DGain) >> (19 - AWB_ACC_BIT));
                B[j] = ((CA2Info.b[j]*DGain) >> (19 - AWB_ACC_BIT));
                IR[j] = ((CA2Info.ir[j]*DGain) >> (19 - AWB_ACC_BIT));
                Rth[j] = ((CA2Info.r_th[j]*DGain) >> (19 - AWB_ACC_BIT));
                Gth[j] = ((CA2Info.g_th[j]*DGain) >> (19 - AWB_ACC_BIT));
                Bth[j] = ((CA2Info.b_th[j]*DGain) >> (19 - AWB_ACC_BIT));
    ...
    if (rt != E_OK || sen_info.mode == NULL) {
        awb_info[id].rgb_ir.level= 0;
        DBG_ERR("sensor_get_info fail\r\n");
        return;
    }

    if ((sen_info.mode->stpix == SENSOR_STPIX_RGBIR_RG) || (sen_info.mode->stpix == SENSOR_STPIX_RGBIR_BG)) {
        ca_win_info.uiWinNmX = AWB_WIN_X;
        ca_win_info.uiWinNmY = AWB_WIN_Y;
        ca_win_info.uiWinSzX = 0;
        ca_win_info.uiWinSzY = 0;
        ir_sub.uiCaIrSubRWet = awb_info[id].rgb_ir.irsub_rg;
        ir_sub.uiCaIrSubGWet = awb_info[id].rgb_ir.irsub_rg;
        ir_sub.uiCaIrSubBWet = awb_info[id].rgb_ir.irsub_b;
        ca_info.puiBufR = R;
        ca_info.puiBufG = G;
        ca_info.puiBufB = B;
        ca_info.puiBufIR = IR;
        ca_info.puiBufRth = Rth;
        ca_info.puiBufGth = R;
        ca_info.puiBufBth = Bth;
        ca_info.puiAccCnt = Cnt;
        awb_info[id].rgb_ir.level= 0;
        //awb_info[id].rgb_ir.level= RgbIr_calcIrLevel(&ca_info, ca_win_info, ir_sub);
    } else {
        awb_info[id].rgb_ir.level = 0;
    }
