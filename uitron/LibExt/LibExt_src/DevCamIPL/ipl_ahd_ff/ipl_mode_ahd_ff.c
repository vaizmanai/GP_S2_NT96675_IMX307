...
#if 1	//special case for manual select sensor mode
	{1280,   720,	IPL_UTI_CONV2_UINT32(30, 1),	SEL_SHDR_FRAME_NONE, 	SENSOR_MODE_1},
	{1920,   1080, 	IPL_UTI_CONV2_UINT32(30, 1),	SEL_SHDR_FRAME_NONE,	SENSOR_MODE_2},
#endif
};
...
DBG_ERR("id = %d, img_size = %d, %d\r\n", id, img_size_h, img_size_v);
	...
	for (i = 0; i < tbl_size; i ++) {
		if (sensor_mode_sel_table[i].img_size_h == img_size_h &&
			...
			DBG_ERR("id = %d, Sensor mode = %d\r\n", id, sensor_mode_sel_table[i].sen_mode);
			return;
		}
	}
...
IPL_EXT_LIB_TAB *IPL_AHD_FF_EXT_LIB_TAB(void)
{
