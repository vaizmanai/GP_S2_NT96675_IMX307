...
static void IPL_SetSensorMode(IPL_PROC_ID id, IPL_MODE mode)
{
	...
	//#NT#2019/10/30#Jarkko Chang -begin
	//#NT# add for ad sensor
	sen_status.sensor_mode = SENSOR_MODE_1;
	rt = sensor_get_info(IPL_UTI_CONV2_SEN_ID(IPL_CtrlGetInfor(id, IPLCTRL_MF_BASE_ID)), SEN_GET_STATUS, (void *)&sen_status);
	if (rt == E_OK) {
		if (sen_status.sensor_info.sen_type == SENSOR_TYPE_AD) {
			sen_ad_param.ch_active = TRUE;
			sen_ad_param.b_manual_info = IPL_AlgGetUIInfo(IPL_CtrlGetInfor(id, IPLCTRL_MF_BASE_ID), IPL_SEL_AD_AUTO);
			sen_ad_param.fmt = IPL_AlgGetUIInfo(IPL_CtrlGetInfor(id, IPLCTRL_MF_BASE_ID), IPL_SEL_AD_FMT);
			sen_ad_param.fps = IPL_AlgGetUIInfo(IPL_CtrlGetInfor(id, IPLCTRL_MF_BASE_ID), IPL_SEL_VIDEO_FPS);
			sen_ad_param.width = img_size_h;
			sen_ad_param.height = img_size_v;
			sensor_set_info(id, SEN_SET_AD_INFO, (void*)&sen_ad_param);
		}
	} else {
		DBG_ERR("rt %d\r\n", rt);
	}
	//#NT#2019/10/30#Jarkko Chang -end
...
IPL_EXT_LIB_TAB *IPL_SAMPLE_FF_EXT_LIB_TAB(void)
{
