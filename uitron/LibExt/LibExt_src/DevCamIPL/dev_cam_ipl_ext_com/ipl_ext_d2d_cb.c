...
static UINT32 RHEIME_CAP_D2D_DCECB(IPL_PROC_ID id, USIZE *pRef_1x, Coordinate *pRef_1xCen, DCE_SUBFUNC *subF, DCE_UPDATE *Update, UINT32 eventID, fpRegDump *fpRegD)
{
	...
	iq_get_ref_image_info(id, IPL_AlgGetUIInfo(id, IPL_SEL_CAPSENMODE), &Refimg_info);
...
static UINT32 RHEIME_CAP_D2D_PHDRCB(IPL_PROC_ID id, RHE_SUBFUNC *psubF, RHE_UPDATE *Update,void *extinfo )
{
	...
	wdr_info.WdrCalMode = SR_WDR_CAL_MODE_CAP;
	...
	wdr_chg_par.uiChgSel = (SR_WDR_CHG_AE_TAG_Y | SR_WDR_CHG_SMOOTH_FACTOR);
	wdr_chg_par.uiChgAeTagY = AE_AlgGetUIInfo(IPL_UTI_CONV2_AE_ID(id), AE_SEL_CURR_EXPY);
	wdr_chg_par.uiChgSmoothFactor = 16;
	...
	//wdr set parameters
	psubF->path_curve_info.PathCurveParamMode = RHE_HAL_PARAM_AUTO_MODE;
	...
	psubF->rhe_func_enable |= (SubImgFilterEn|ImgBlendingEn);

	*Update |= RHE_SUBF_UPDATE|RHE_PATH_CURVE|RHE_BLEND|RHE_SUBIMG_FILTER;
...
static UINT32 RHEIME_CAP_D2D_SHDRCB(IPL_PROC_ID id, RHE_SUBFUNC *psubF, RHE_UPDATE *Update,void *extinfo)
{
#if IPL_SHDR_FUNC_SUPPORT
	...
	HdrCalInfo.uiAEExpectLumRaw = AE_AlgGetUIInfo(IPL_UTI_CONV2_AE_ID(id), AE_SEL_TARGET_RAW);
	HdrCalInfo.uiAECurrentLumRaw = AE_AlgGetUIInfo(IPL_UTI_CONV2_AE_ID(id), AE_SEL_CURR_RAW);
	...
	HdrCalInfo.pYoutInfo[0] = &hdr_yout_info[0];
	HdrCalInfo.pYoutInfo[0]->uiYoutAddr = yout_info[0].yout_addr;
	HdrCalInfo.pYoutInfo[0]->uiYoutLofs = yout_info[0].yout_lofs;
	HdrCalInfo.pYoutInfo[0]->uiYoutSizeH = yout_info[0].yout_sizeh;
	HdrCalInfo.pYoutInfo[0]->uiYoutSizeV = yout_info[0].yout_sizev;
	HdrCalInfo.pYoutInfo[0]->uiOB = iq_get_ob(IPL_ID_1);
	HdrCalInfo.pYoutInfo[1] = &hdr_yout_info[1];
	HdrCalInfo.pYoutInfo[1]->uiYoutAddr = yout_info[1].yout_addr;
	HdrCalInfo.pYoutInfo[1]->uiYoutLofs = yout_info[1].yout_lofs;
	HdrCalInfo.pYoutInfo[1]->uiYoutSizeH = yout_info[1].yout_sizeh;
	HdrCalInfo.pYoutInfo[1]->uiYoutSizeV = yout_info[1].yout_sizev;
	HdrCalInfo.pYoutInfo[1]->uiOB = iq_get_ob(IPL_ID_2);
	...
	psubF->rheFuncEn |= (SubImgFilterEn | ImgBlendingEn);
	*Update |= (RHE_SUBIMG_FILTER |RHE_BLEND | RHE_PATH_CURVE_WEIGHT);
...
UINT32 RHEIME_D2D_EXTFUNC_RHE_CB(IPL_PROC_ID id, RHE_SUBFUNC *psubF, RHE_UPDATE *Update, UINT32 eventID, void *extinfo)
{
	...
	if (((eventID & (~IPL_FLOW_RESERVE)) >= IPL_FLOW_CAPD2D_MIN) && ((eventID & (~IPL_FLOW_RESERVE)) < (IPL_FLOW_CAPD2D_MAX + 1))) {
		...
		case IPL_FLOW_RAW2YCC_HDR:
			RHEIME_CAP_D2D_SHDRCB(id, psubF, Update,extinfo);
			break;
				...
				DCE_SUBFUNC *subF, IPL_HAL_GROUP *Update, UINT32 eventID)
{
	...
	if (eventID == IPL_FLOW_CAL_STRIPE) {
		...
		IPL_DzoomGetSIECropSize(id, 0, TRUE, &(SieOutSize->w), &(SieOutSize->h));
		iq_get_ref_image_info(id, IPL_AlgGetUIInfo(id, IPL_SEL_CAPSENMODE), &Refimg_info);
