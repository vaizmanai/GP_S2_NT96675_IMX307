...
static ER ExtFunc_UnInitHDR(UINT32 Id)
{
	...
	for (i = IPL_ID_1, j = 0; i < IPL_ID_MAX_NUM; i++) {
		if (IPL_CtrlGetInfor(Id, IPLCTRL_MF_TOTAL_FRM_BIT) & (1 << i)) {
			if (j >= SHDR_MAXFRAMES) {
				DBG_WRN("IPLCTRL_MF_TOTAL_FRM_BIT (0x%x) error. Too many frames (>%u).\r\n", IPL_CtrlGetInfor(Id, IPLCTRL_MF_TOTAL_FRM_BIT), SHDR_MAXFRAMES);
				return E_SYS;
			}
			Info = IPL_Ctrl_Query(i, ICF_SIE_YOUT_RDY);
		    HdrCalInfo.pYoutInfo[j]              	= &hdr_yout_info[j];
			HdrCalInfo.pYoutInfo[j]->uiYoutAddr  	= Info.sie_rdy_yout.addr;
			HdrCalInfo.pYoutInfo[j]->uiYoutLofs  	= Info.sie_rdy_yout.lofs;
			HdrCalInfo.pYoutInfo[j]->uiYoutSizeH	= Info.sie_rdy_yout.win_num_x;
			HdrCalInfo.pYoutInfo[j]->uiYoutSizeV 	= Info.sie_rdy_yout.win_num_y;
		    HdrCalInfo.pYoutInfo[j]->uiOB        	= iq_get_ob(i);
			j++;
		}
	}
...
static ER ExtFunc_UnInitWDR(UINT32 Id)
{
    ...
    wdr_info.WdrProcMode            = SR_WDR_OFF;
   	wdr_info.WdrCalMode 			= SR_WDR_CAL_MODE_VDO;
	...
	wdr_chg_par.uiChgSel                    = (SR_WDR_CHG_WDR_STRENGTH | SR_WDR_CHG_AE_TAG_Y | SR_WDR_CHG_GCE_LEVEL | SR_WDR_CHG_SMOOTH_FACTOR);
...
static ER ExtFunc_ProcessHDR(UINT32 Id)
{
	...
	rhe_subfunc = (RHE_SUBFUNC *)SHDRCtrl_GetParam(Id, Info.FrameCnt);
...
static ER ExtFunc_ProcessWDR(UINT32 Id)
{
	...
	if (ExtFunc_SetTrigEn(Id)) {
        ...
        if (IPL_CtrlGetInfor(Id, IPLCTRL_EXT_FUNC_EN) & ICF_FUNC_SHDR) // Only for 510. Must query IPL_ID_2(SIE2) for long exposure image
		    Info = IPL_Ctrl_Query(IPL_ID_2, ICF_SIE_YOUT_RDY);
        else
            Info = IPL_Ctrl_Query(Id,       ICF_SIE_YOUT_RDY);
...
static EXTFUNC_DR ExtFunc_GetDRMode(UINT32 Id)
{
    if (IPL_CtrlGetInfor(Id, IPLCTRL_EXT_FUNC_EN) & ICF_FUNC_SHDR) {
        ...
        // Only for 510 direct mode, must be HDR ON
...
static void ExtFunc_SetParam(UINT32 Id, EXTFUNC_MODE mode)
{
	...
	if (mode == EXTFUNC_MODE_DR) {
		...
		group.rheUpdate = RHE_SUBF_EN|RHE_PATH_CURVE|RHE_BLEND|RHE_SUBIMG_FILTER;
		...
		rheSubf.rheFuncEn                           |= (SubImgFilterEn|ImgBlendingEn);
	}
#endif
#if IPL_DEFOG_FUNC_SUPPORT
