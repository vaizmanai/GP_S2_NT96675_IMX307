...
static void IPL_SetSensorMode(IPL_PROC_ID id, IPL_MODE mode)
{
	...
	//auto detect TVI/AHD sensor
	if (sensor_get_info(IPL_UTI_CONV2_SEN_ID(IPL_CtrlGetInfor(id, IPLCTRL_MF_BASE_ID)), SEN_GET_USER_DEFINE1, (void *)&sen_mode) == E_OK) {
		DBG_DUMP("^Gid%d, use sensor mode (%d) selected by sensor driver\r\n", id, sen_mode);
		IPL_AlgSetUIInfo(id, IPL_SEL_PRVSENMODE, sen_mode);
		IPL_CtrlSetInfor(id, IPLCTRL_SEN_MODE, sen_mode);
	} else {
		DBG_DUMP("^Gid%d, use sensor mode (%d) selected by IPL\r\n", id, IPL_AlgGetUIInfo(id, IPL_SEL_PRVSENMODE));
	}
}
...
static ER IPL_Query_Buffer(IPL_MODE_DATA *chg_mode, IPL_MODE_DATA *prv_mode)
{
	...
	if (b_ipl_ctrl_dbg_en) {
		...
		DBG_DUMP("IPL_ID_%d_Query EXT_CTRL_EN = 0x%x\r\n", chg_mode->proc_sen_id+1, chg_mode->ext_ctrl_en);
...
IPL_EXT_LIB_TAB *IPL_TP2825_FF_EXT_LIB_TAB(void)
{
