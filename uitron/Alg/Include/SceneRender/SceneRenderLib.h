...
typedef enum {
	...
	SR_WDR_CHG_SMOOTH_FACTOR    = 0x00000020,  ///< change wdr update smooth factor
...
/*
    Scene Render enum - computation mode selection
*/
typedef enum {
	SR_WDR_CAL_MODE_VDO = 0,
	SR_WDR_CAL_MODE_CAP = 1,
	ENUM_DUMMY4WORD(SR_WDR_CAL_MODE)
} SR_WDR_CAL_MODE;
//----------------------------------------------------------
...
typedef struct _SR_WDR_CAL_INFO {
	...
	SR_WDR_CAL_MODE     WdrCalMode;         ///< WDR computation mode selection
...
typedef struct _SR_WDR_CHG_PARAMS_ {
	...
	UINT32 uiChgSmoothFactor;	///< change wdr update smooth factor, range: 1 ~ 16, fastest: 1
} SR_WDR_CHG_PARAMS;
//----------------------------------------------------------
...
/**
    ...
    @param[in] ProcId       process ID
    @param[in] pGetInfo   get sub-image size parameters
