...
static void IO_InitGSensor(void)
{
	#if 1
	//GSensor_open();
	#else
	//Gsensor HW Reset!!!
	//gpio_setDir(P_GPIO_0, GPIO_DIR_OUTPUT);
	//gpio_clearPin(P_GPIO_0);
	//gpio_setDir(P_GPIO_1, GPIO_DIR_OUTPUT);
	//gpio_clearPin(P_GPIO_1);
	//
	//gpio_setDir(GPIO_GS_RST, GPIO_DIR_OUTPUT);
	gpio_setPin(GPIO_GS_RST);
	Delay_DelayMs(20);

	//gpio_setDir(P_GPIO_0, GPIO_DIR_INPUT);
	//gpio_setDir(P_GPIO_1, GPIO_DIR_INPUT);
	//Delay_DelayMs(1);
	gpio_clearPin(GPIO_GS_RST);
	#endif
}
...
UINT32 GPIOMap_AccSignalDetect(void)
{

	return gpio_getPin(GPIO_ACC_DET);
}

void GPIOMap_ToMcuSystemOK(BOOL Flag)
{
    if(Flag == TRUE )
    {
        gpio_setPin(GPIO_NOTIFY_MCU);
    }
    else
    {
        gpio_clearPin(GPIO_NOTIFY_MCU);
    }
}


void GPIOMap_DVR_5V_Power_Ctrl(BOOL Flag)
{
  	if(Flag == TRUE )
    {
        gpio_setPin(GPIO_HOLD_DVR_POWER);
    }
    else
    {
        gpio_clearPin(GPIO_HOLD_DVR_POWER);
    }
}
void GPIOMap_RecordingModeOut(BOOL Flag)
{
    if(Flag == TRUE )
    {
		gpio_setPin(GPIO_REC_MODE);
    }
    else
    {
		gpio_clearPin(GPIO_REC_MODE);
    }
}

/*
void GPIOMap_CapacityChargeEn(BOOL Flag)
{
    if(Flag == TRUE )
    {
		gpio_setPin(GPIO_CAPACITY_CHARGE_EN);
    }
    else
    {
		gpio_clearPin(GPIO_CAPACITY_CHARGE_EN);
    }
}
*/
BOOL GPIOMap_CardProtect_Det(void)
{
    return (BOOL)(gpio_getPin(GPIO_CARD_WP_DET)?FALSE:TRUE);
}


void GPIOMap_AmpPowerOn(void)
{
	//gpio_setDir(GPIO_VOICE_PWR_ON,GPIO_DIR_OUTPUT);
	//gpio_setPin(GPIO_VOICE_PWR_ON);
}

void GPIOMap_AmpPowerOff(void)
{
	//gpio_setDir(GPIO_VOICE_PWR_ON,GPIO_DIR_OUTPUT);
	//gpio_clearPin(GPIO_VOICE_PWR_ON);
}
void GPIOMap_AudioPowerOff(void)
{
	gpio_setDir(GPIO_VOICE_PWR_ON,GPIO_DIR_OUTPUT);
	gpio_clearPin(GPIO_VOICE_PWR_ON);
}

void GPIOMap_WifiOn_LED(void)
{
	gpio_setDir(GPIO_WIFI_LED,GPIO_DIR_OUTPUT);
	gpio_setPin(GPIO_WIFI_LED);
}

void GPIOMap_WifiOff_LED(void)
{
   gpio_setDir(GPIO_WIFI_LED,GPIO_DIR_OUTPUT);
   gpio_clearPin(GPIO_WIFI_LED);
}

BOOL GPOIMap_GetWifiLED_Status(void)
{
