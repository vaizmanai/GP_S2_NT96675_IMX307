...
UINT32 DrvCARDInit(void *pInitParam) // Set Init Parameters
{
	...
	*/
	...
	sdio_setConfig(SDIO_CONFIG_ID_DS_DRIVING, 150);
	sdio_setConfig(SDIO_CONFIG_ID_DS_CMD_DRIVING, 100);
	sdio_setConfig(SDIO_CONFIG_ID_DS_DATA_DRIVING, 100);

	// High speed driving
	sdio_setConfig(SDIO_CONFIG_ID_HS_DRIVING, 150);
	sdio_setConfig(SDIO_CONFIG_ID_HS_CMD_DRIVING, 100);
	sdio_setConfig(SDIO_CONFIG_ID_HS_DATA_DRIVING, 100);
...
BOOL DrvCARD_DetStrgCard(void)
{
#if 1 //(FS_MULTI_STRG_FUNC == ENABLE)
