...
#if (defined(_CMOS_IMX225P_))
static SENSOR_INIT_OBJ sensor_get_obj_imx225p(SENSOR_ID id)
{
	// set sensor command interface info (Vx1/SIF/I2C/IO/...)
	SENSOR_INIT_OBJ init_obj = {0};
	init_obj.cmd_info.cmd_type = SENSOR_CMD_I2C;
	...
	init_obj.parallel_switch = TRUE;
	return init_obj;
}
static void sen_power_ctrl_imx225p(SENSOR_ID id, POW_CTRL_FLAG flag)
{
	UINT32 reset_pin = S_GPIO_12;
	switch (flag) {
	case POW_CTRL_TURN_ON:
		// set power on sequence
		gpio_setDir(reset_pin, GPIO_DIR_OUTPUT);
...
#if 0
static void csi_hw_rst_imx307(SENSOR_ID sensor_id)
{
    debug_msg("~G csi_hw_rst_imx307 \r\n");
...
#if (defined(_CMOS_TP2852_))
static SENSOR_INIT_OBJ sensor_get_obj_tp2852(SENSOR_ID id)
{
	// set sensor command interface info (Vx1/SIF/I2C/IO/...)
	SENSOR_INIT_OBJ init_obj = {0};
	init_obj.cmd_info.cmd_type = SENSOR_CMD_I2C;
...
static void sen_power_ctrl_tp2852(SENSOR_ID id, POW_CTRL_FLAG flag)
{
	static BOOL bFirstTime = TRUE;
	switch (flag) {
	case POW_CTRL_TURN_ON:
...
static BOOL sen_det_plug_in_tp2852(SENSOR_ID id)
{
#if 1
...
#if (defined(_CMOS_N4_))
static SENSOR_INIT_OBJ sensor_get_obj_n4(SENSOR_ID id)
{
	// set sensor command interface info (Vx1/SIF/I2C/IO/...)
	SENSOR_INIT_OBJ init_obj = {0};
	init_obj.cmd_info.cmd_type = SENSOR_CMD_I2C;
	...
	if (id == SENSOR_ID_1) {
		init_obj.sen_map_if = SENSOR_MAP_IF_DEFAULT;
	} else if (id == SENSOR_ID_2) {
		init_obj.sen_map_if = SENSOR_2_MAP_IF_LVDSMIPI_1;
	}
	//init_obj.serial_clane_switch = TRUE;
	...
	// set mclk change enable to notify sensor driver
	init_obj.sie_mclk_info.chg_mclk_en = DISABLE;
	...
	if (id == SENSOR_ID_1) {
		init_obj.ad_io_port = SEN_AD_IO_PORT_INFO(0,0);
	} else if (id == SENSOR_ID_2) {
		init_obj.ad_io_port = SEN_AD_IO_PORT_INFO(1,1);
	}
	init_obj.ad_dl_num = 4;
CHKPNT;
	return init_obj;
}

static void sen_power_ctrl_n4(SENSOR_ID id, POW_CTRL_FLAG flag)
{
#define TOP_CGPIO_ADDR 0xc00100a0
#define TOP_PGPIO_ADDR 0xc00100a8
#define TOP_SP_CLK2_ADDR 0xc001000c
#define CG_SP_CLKSEL_ADDR 0xc0020024
#define CG_SP_CLKDIV_ADDR 0xc0020038
#define CG_SP_CLKEN_ADDR 0xc0020070

	switch (flag) {
	case POW_CTRL_TURN_ON:
		// set power on sequence
		gpio_setDir(GPIO_TVI_RESET, 	GPIO_DIR_OUTPUT);
		gpio_setDir(GPIO_TVI1_POWER_EN, GPIO_DIR_OUTPUT);
		gpio_setDir(GPIO_TVI2_POWER_EN, GPIO_DIR_OUTPUT);
		gpio_setDir(GPIO_TVI_12EN,		GPIO_DIR_OUTPUT);

		gpio_clearPin(GPIO_TVI_RESET);
		gpio_clearPin(GPIO_TVI1_POWER_EN);
		gpio_clearPin(GPIO_TVI2_POWER_EN);
		gpio_clearPin(GPIO_TVI_12EN);
		Delay_DelayMs(10);

		pll_setPLLEn(PLL_ID_5,      DISABLE);
		pll_setPLL(PLL_ID_5,        0x90000);//54MHz
		pll_setPLLFreq(PLL_ID_5,    27000000);
		pll_setPLLEn(PLL_ID_5,      ENABLE);

		*(UINT32 *)TOP_CGPIO_ADDR |= (1 << 6);
		*(UINT32 *)TOP_PGPIO_ADDR |= ((1 << 2) | (1 << 3) | (1 << 4));
		*(UINT32 *)TOP_SP_CLK2_ADDR &= ~(0x3 << 18);
		*(UINT32 *)TOP_SP_CLK2_ADDR |= (0x2 << 18);		// sp_clk_2
		*(UINT32 *)CG_SP_CLKSEL_ADDR &= ~(0x3 << 8);
		*(UINT32 *)CG_SP_CLKSEL_ADDR |= (0x2 << 8);		// use pll5
		*(UINT32 *)CG_SP_CLKDIV_ADDR &= ~(0xff << 0);
		*(UINT32 *)CG_SP_CLKDIV_ADDR |= (0x0 << 0);		// sp_clkdiv = 1
		*(UINT32 *)CG_SP_CLKEN_ADDR |= (1 << 12);		// sp_clken = 1

		gpio_setPin(GPIO_TVI1_POWER_EN);
		gpio_setPin(GPIO_TVI2_POWER_EN);
		Delay_DelayMs(10);
		gpio_setPin(GPIO_TVI_12EN);
		Delay_DelayMs(10);

		/*sensor_if_get_pll_ctrl_obj()->set_clk_src(id, SEN_CLK_SIEMCLK, SEN_CLK_SRC_SIEMCLK_PLL5);
		sensor_if_get_pll_ctrl_obj()->set_clk_freq(id, SEN_CLK_SIEMCLK, 27000000);
        sensor_if_get_pll_ctrl_obj()->set_clk_en(id, SEN_CLK_SIEMCLK, ENABLE);*/
...
static BOOL sen_det_plug_in_n4(SENSOR_ID id)
{
	BOOL rt = E_OK;
	SENSOR_HANDLE sen_handle = {0};
...
#endif
#if (defined(_CMOS_IMX225P_))
	{"CMOS_IMX225P", sensor_get_drv_tab_imx225p, sensor_get_obj_imx225p, sen_power_ctrl_imx225p, NULL, NULL, NULL},
#endif
#if (defined(_CMOS_IMX291M_))
...
#endif
#if (defined(_CMOS_TP2852_))
	{"CMOS_TP2852", sensor_get_drv_tab_tp2852, sensor_get_obj_tp2852, sen_power_ctrl_tp2852, sen_det_plug_in_tp2852, NULL},
//	{"CMOS_TP2825", sensor_get_drv_tab_tp2825, sensor_get_obj_tp2825, sen_power_ctrl_tp2825, sen_det_plug_in_tp2825, NULL},
#endif
#if (defined(_CMOS_N4_))
	{"CMOS_N4", sensor_get_drv_tab_n4, sensor_get_obj_n4, sen_power_ctrl_n4, sen_det_plug_in_n4, NULL},
#endif
#if (defined(_CSMOS_SIEPATGEN_))
