...
static SPI_FLASH_INFO user_define_spi_flash_support_tbl[] = {
    // XTX PN25F32B (4MB)
	{
        ...
        0x800000 / 0x10000,            	TRUE,                      FALSE,                          TRUE,
...
#if 0
static SPI_ID g_TestSpiID = SPI_ID_1;
BOOL  SPI_readRegTEST(void)
{
  UINT32  index;
  UINT32 uiRecv[4];
  ...
  //----------------
	spi_setConfig(g_TestSpiID, SPI_CONFIG_ID_BUSMODE, SPI_MODE_0);
        ...
        spi_setConfig(g_TestSpiID, SPI_CONFIG_ID_FREQ, 2000000);
    ...
    //spi_writeReadSingle(g_TestSpiID, uiAddr, &uiRecv[0]);
     spi_writeSingle(g_TestSpiID, 0x5A);
	spi_writeSingle(g_TestSpiID, 0x00);
	spi_writeSingle(g_TestSpiID, 0x01);
	spi_writeSingle(g_TestSpiID, 0xB0);

	spi_writeSingle(g_TestSpiID, 0x00);

    spi_writeReadData(g_TestSpiID, 2, NULL, (UINT32*)uiRecv, TRUE);
    spi_waitDataDone(g_TestSpiID);
 ...
 //   spi_close(g_TestSpiID);
    ...
    {
	Rec[index] = uiRecv[0] >>((index)*8);
	Rec1[index] = uiRecv[1] >>((index)*8);
    }
   debug_msg("--lyg---SPI_readRegTEST-%s-%s---%c--\r\n",Rec,Rec1,Rec[0]);
    if((Rec[0]=='Y')&&(Rec[1]=='N')&&(Rec[2]=='D')&&(Rec[3]=='2')&&(Rec1[0]=='0')&&(Rec1[1]=='1')&&(Rec1[2]=='8'))
		return  TRUE;
    else
   	 return FALSE;
}
...
UINT32 DrvEmbMemInit(void *pInitParam) // Set Init Parameters
{
	...
	static BOOL bSpiNandInitOnce = FALSE;
	if (!bSpiNandInitOnce) {
        //nand_setConfig(NAND_CONFIG_ID_FREQ, 96000000);  // lyg
...

	{
		...
		//User define SPI-NOR table sample code
			...
			if(pStrg->ExtIOCtrl(STRG_EXT_CMD_SPI_IDENTIFY_CB, (UINT32)pmc_identifySpi, 0) == E_OK) {
		        DBG_DUMP("^GSet user define SPI-NOR table success--lyg-\r\n");
		    } else {
		        DBG_DUMP("^RSet user define SPI-NOR table fail\r\n");
		    }
		#endif
