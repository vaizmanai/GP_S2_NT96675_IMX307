...
#if (defined(_IQ_PARAM_GC2053_EVB_))
	{"IQ_PARAM_GC2053_EVB", iq_param_init_gc2053_evb},
#endif
...
#if (defined(_AWB_PARAM_GC2053_EVB_))
	{"AWB_PARAM_GC2053_EVB", awb_get_param_gc2053_evb},
#endif
...
static AWB_MAP_INFO *pqs_awb_search(UINT32 id)
{
	...
	for (j = 0; (strcmp(pqs_awb_search_tab[j].name, "END") != 0); j++) {
	    debug_msg("AWB[%d]=>%s <-> %s\r\n",id,pqs_drv_name.awb_name[id], pqs_awb_search_tab[j].name);
...
#if (defined(_AE_PARAM_GC2053_EVB_))
	{"AE_PARAM_GC2053_EVB", ae_get_param_gc2053_evb},
#endif
