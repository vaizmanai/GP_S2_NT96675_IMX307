...
/*static BOOL W25W64_identifySpi(UINT32 uiMfgID, UINT32 uiTypeID, UINT32 uiCapacity, PSPIFLASH_IDENTIFY pIdentify)
{
    //W25W64 (8MBytes)
    //DBG_ERR("=====SPI flash uiMfgID=%d, uiTypeID=%d, uiCapacity=%d\r\n", uiMfgID, uiTypeID, uiCapacity);
    if ((uiMfgID == 0xC2) && (uiCapacity == 0x17)&& (uiTypeID == 0x20))
    {
        pIdentify->uiTotalSize = 8*1024*1024;
        pIdentify->uiSectorSize = 4*1024;
        pIdentify->bSupportEWSR = TRUE;
        pIdentify->bSupportAAI = FALSE;
        pIdentify->flashWidth = SPI_FLASH_BUSWIDTH_QUAD_TYPE5;
...
#else // new spi
        pStrgObj = EMB_GETSTRGOBJ(STRG_OBJ_FW_RSV1);//spiflash_getStorageObject(STRG_OBJ_FW_RSV1);
        if (pStrgObj == NULL)
        {
            DBG_ERR("No FW RSV1 object\r\n");
        }
        pStrgObj->ExtIOCtrl(STRG_EXT_CMD_SPI_IDENTIFY_CB, (UINT32)W25W64_identifySpi, 0);
        v = (UINT32)pStrgObj;
#endif
        break;
    default:
