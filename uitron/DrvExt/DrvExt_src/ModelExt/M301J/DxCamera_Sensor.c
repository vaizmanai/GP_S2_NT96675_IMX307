...
static SENSOR_INIT_OBJ sensor_get_obj_gc2053m(SENSOR_ID id)
{
    ...
    init_obj.sen_2_serial_pin_map[0] = 0;
    init_obj.sen_2_serial_pin_map[1] = 1;
    init_obj.sen_2_serial_pin_map[2] = 2;//SEN_IGNORE;
    init_obj.sen_2_serial_pin_map[3] = 3;//SEN_IGNORE;
    return init_obj;
}

static void sen_power_ctrl_gc2053m(SENSOR_ID id, POW_CTRL_FLAG flag)
{
    UINT32 reset_pin = GPIO_SENSOR_RESET;//S_GPIO_4;
    switch (flag) {
    case POW_CTRL_TURN_ON:
        // set power on sequence: 1.8V -> 1.2V -> 2.8V
        IP6303_set_sensor_power18v();
		Delay_DelayUs(100);
		...
		IP6303_set_sensor_power28v();
		Delay_DelayUs(100);

        gpio_setDir(reset_pin, GPIO_DIR_OUTPUT);
        gpio_setPin(reset_pin);
        Delay_DelayMs(20); //>=1ms
        gpio_clearPin(reset_pin);
        Delay_DelayMs(20); //>=1ms
        gpio_setPin(reset_pin);
        Delay_DelayMs(20); //>=1ms
...
static SENSOR_INIT_OBJ sensor_get_obj_gc2053p(SENSOR_ID id)
{
	...
	SENSOR_INIT_OBJ init_obj = {0};
	init_obj.cmd_info.cmd_type = SENSOR_CMD_I2C;
	init_obj.cmd_info.info.i2c.i2c = SENSOR_I2C1_1ST;//SENSOR_I2C2_1ST;//for TP ; EVB SENSOR_I2C2_1ST
	init_obj.cmd_info.info.i2c.w_addr_sel = SEN_I2C_W_ADDR_DFT;
	init_obj.sen_map_if = SENSOR_MAP_IF_DEFAULT;
	//init_obj.option= SENSOR_OPTION_MIRROR | SENSOR_OPTION_FLIP;
...
static void sen_power_ctrl_tp2825(SENSOR_ID id, POW_CTRL_FLAG flag)
{
	...
	//	gpio_clearPin(S_GPIO_2);
		...
		#else
		// set power on sequence
		gpio_setDir(P_GPIO_25, GPIO_DIR_OUTPUT);
		gpio_setPin(P_GPIO_25);
		gpio_setDir(L_GPIO_11, GPIO_DIR_OUTPUT);
...
static BOOL sen_det_plug_in_tp2825(SENSOR_ID id)
{
	...
	if (sensor_is_open(id) == FALSE) {
		...
		if (drv_sensor_init(id, &sen_handle) == FALSE) {
			debug_msg("sensor_init fail\r\n");
			return FALSE;
		}

		if (sen_handle.drv_tab == NULL) {
			debug_msg("drv_tab NULL\r\n");
			return E_SYS;
		}

		if (sensor_open(id, &sen_handle) != E_OK) {
			debug_msg("sensor open fail\r\n");
			return FALSE;
		}
	}

	if (sensor_get_info(id, SEN_GET_DET_PLUG, &rt) != E_OK) {
		debug_msg("get sensor plugin fail\r\n");
		return FALSE;
	}
	...
	//return (gpio_getPin(GPIO_5150_BackIn) == 0);
	return (gpio_getPin(GPIO_5150_BackIn) == 1);
...
#if (defined(_CMOS_GM7150_))
static SENSOR_INIT_OBJ sensor_get_obj_gm7150(SENSOR_ID id)
{
	// set sensor command interface info (Vx1/SIF/I2C/IO/...)
	SENSOR_INIT_OBJ init_obj = {0};
	init_obj.cmd_info.cmd_type = SENSOR_CMD_I2C;
...
static void sen_power_ctrl_gm7150(SENSOR_ID id, POW_CTRL_FLAG flag)
{
#define TOP_SP_CLK2_ADDR 0xc0010018
#define TOP_P_GPIO_ADDR 0xc00100a8
#define TOP_L_GPIO_ADDR 0xc00100b8
#define CG_SP2_CLKSEL_ADDR 0xc0020024
#define CG_SP2_CLKDIV_ADDR 0xc0020040
#define CG_SP2_CLKEN_ADDR 0xc0020074

	switch (flag) {
	case POW_CTRL_TURN_ON:
		// MCLK = PLL4_freq / (sp2_div+1)
	 	//      = (12M x 3753425 / 131072) / (23+1)
	 	//      = 14.31818008M

		pll_setPLLEn(PLL_ID_4,      DISABLE);
		pll_setPLL(PLL_ID_4,        0x3945D1); //343.63632202 MHz
		pll_setPLLEn(PLL_ID_4,      ENABLE);

		*(UINT32 *)TOP_SP_CLK2_ADDR &= ~(0x3 << 10);
		*(UINT32 *)TOP_SP_CLK2_ADDR |= (0x1 << 10);
		*(UINT32 *)TOP_P_GPIO_ADDR &= ~(1 << 24);
		UINT32 i;
		for (i = 0; i < 8; i++)
		{
			*(UINT32 *)TOP_P_GPIO_ADDR &= ~(1 << i); // tmp set P_GPIO0~7 to functional pin
		}
		*(UINT32 *)CG_SP2_CLKSEL_ADDR &= ~(0x3 << 10);
		*(UINT32 *)CG_SP2_CLKSEL_ADDR |= (0x1 << 10);
		*(UINT32 *)CG_SP2_CLKDIV_ADDR &= ~(0xff << 24);
		*(UINT32 *)CG_SP2_CLKDIV_ADDR |= (23 << 24);
		*(UINT32 *)CG_SP2_CLKEN_ADDR |= (1 << 30);

		*(UINT32 *)TOP_L_GPIO_ADDR |= (1 << 17);
		*(UINT32 *)TOP_L_GPIO_ADDR |= (1 << 18);
		*(UINT32 *)TOP_P_GPIO_ADDR |= (1 << 9);

		gpio_setDir(GPIO_5150_PCTL, GPIO_DIR_OUTPUT);
		gpio_setDir(GPIO_5150_PDN, GPIO_DIR_OUTPUT);
		gpio_setDir(GPIO_5150_RESET, GPIO_DIR_OUTPUT);

		gpio_setPin(GPIO_5150_PCTL);
		Delay_DelayMs(20);
		gpio_setPin(GPIO_5150_PDN);
		Delay_DelayMs(20);
		gpio_clearPin(GPIO_5150_RESET);
		Delay_DelayMs(500);
		gpio_setPin(GPIO_5150_RESET);
		Delay_DelayMs(20);
		break;
...
static BOOL sen_det_plug_in_gm7150(SENSOR_ID id)
{
#if 0
...
#endif
#if (defined(_CMOS_GM7150_))
	{"CMOS_GM7150", sensor_get_drv_tab_gm7150, sensor_get_obj_gm7150, sen_power_ctrl_gm7150, sen_det_plug_in_gm7150, NULL},
#endif
#if (defined(_CMOS_SIEPATGEN_))
...
BOOL drv_sensor_init(SENSOR_ID id, SENSOR_HANDLE *handle)
{
	...
	//debug_msg("^C%s id %d, name %s\r\n", __func__, id, sensor_info->name);
...
void drv_sensor_power_turn_on(SENSOR_ID id)
{
	...
	debug_msg("^C%s sensor[%d]: %s\r\n", __func__, id, sensor_name[id]);
	if (sensor_debug_item[id] & SENSOR_DEBUG_MODELEXT) {
		debug_msg("^C%s id %d\r\n", __func__, id);
	}
