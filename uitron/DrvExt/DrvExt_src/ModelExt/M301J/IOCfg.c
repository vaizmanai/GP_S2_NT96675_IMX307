...
{
#if 0 // M301J g-sensor uses RC reset
	//Gsensor HW Reset!!!
...
BOOL GetGpio5150BackIn(void);
BOOL GetGpio5150BackIn(void)
{
 //  return FALSE;
   //  debug_msg("--------GPIO_5150_BackIn-----%d--\r\n",gpio_getPin(GPIO_5150_BackIn));
   return !gpio_getPin(GPIO_5150_BackIn);
}

void Set_Mic_AMPLIFIER(void);
void Clear_Mic_AMPLIFIER(void);
BOOL Get_Mic_AMPLIFIER(void);
void Set_Mic_AMPLIFIER(void)
{
   gpio_setPin(GPIO_MIC_POWER_AMPLIFIER);
}
void Clear_Mic_AMPLIFIER(void)
{
   gpio_clearPin(GPIO_MIC_POWER_AMPLIFIER);
}
BOOL Get_Mic_AMPLIFIER(void)
{
   return gpio_getPin(GPIO_MIC_POWER_AMPLIFIER);
}
