...
UINT32          g_TVEnable = FALSE; //not yet enable
// Exported Variables
...
static UINT32 DrvTVGetcaps(UINT32 CapID, UINT32 Param1) // Get Capability Flag (Base on interface version)
{
	...
	case DISPLAY_CAPS_PLUG:
        ...
        //#NT#using new function to detect TV insert or not
		#ifdef NEW_TV_DETECT
			v = tv_loaddet_getdet();
			static UINT32 pre_v = 0;
            if(v == TV_LOADDET_PLUGIN){
            	v = 1;
				pre_v = v;
            }
            else if(v == TV_LOADDET_PLUGOUT){
                v =0;
				pre_v = v;
            }
			else if(v== TV_LOADDET_UNKNOW){
				////if TV is unknow,skip this detect
				v = pre_v;
			}
            else{
                DBG_ERR("detect tv error v:%x\r\n",v);
                v =0;
            }
...
static UINT32 DrvTVInit(void *pInitParam) // Set Init Parameters
{
	...
	#ifdef NEW_TV_DETECT
    //#NT#2017/11/06#Charlie Chang -begin
    //#NT# install detect tv task, always open!!!!
	if(!tv_loaddet_getopen()){
		DBG_DUMP("^Y tv_loaddet_open~~~~~~~~~~~~~~~~~~~~~~~~~~\r\n");
		tv_loaddet_open();
	}
    //#NT#2017/11/06#Charlie Chang -end
    #endif
...
static UINT32 DrvTVOpen(void) // Common Constructor
{
	...
	/*
	if (g_TVDout == 1) {
	    pCtrl->SEL.SET_SRCCLK.SrcClk = DISPCTRL_SRCCLK_IDE2_PLL6;
	    pObj->dispCtrl(DISPCTRL_SET_SRCCLK, pCtrl);
	}
	*/
#endif
...
static BOOL GPIOMap_DetTV(void)
{
	...
	//debug_msg("gpio_getPin(GPIO_TV_PLUG) = %d\r\n",gpio_getPin(GPIO_TV_PLUG));
	return (gpio_getPin(GPIO_TV_PLUG) ==0 ? TRUE : FALSE);
#endif
#if (TV_DET_CTRL == TV_DET_BY_ADC)
	UINT32          uiTVDetADC;
