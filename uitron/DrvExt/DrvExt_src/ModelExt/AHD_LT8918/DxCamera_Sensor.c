...
static SENSOR_INIT_OBJ sensor_get_obj_imx291m(SENSOR_ID id)
{
	...
	#else
       init_obj.option= SENSOR_OPTION_ALL;
	#endif
	// set sensor interface pin map (LVDS/CSI/...)
	init_obj.sen_2_serial_pin_map[0] = 0;
	init_obj.sen_2_serial_pin_map[1] = 1;
...
static BOOL sen_det_plug_in_tp2825(SENSOR_ID id)
{
   ...
   //   debug_msg("sensor_is_open(id) =%d \r\n",sensor_is_open(id));
	if (sensor_is_open(id) == FALSE) {

		drv_sensor_power_turn_on(id);
          // debug_msg("drv_sensor_init(id, &sen_handle) =%d \r\n",drv_sensor_init(id, &sen_handle));
   ...
   //   debug_msg("sensor_get_info(id, SEN_GET_DET_PLUG, &rt) =%d \r\n",sensor_get_info(id, SEN_GET_DET_PLUG, &rt));
...
#else
BOOL sen_detplug_ctrl_tp2825(SENSOR_ID id);
BOOL sen_detplug_ctrl_tp2825(SENSOR_ID id)
{
	CHKPNT;CHKPNT;CHKPNT;
       //return TRUE;
	BOOL rt = E_OK;
	SENSOR_HANDLE sen_handle = {0};
