...
#if ((LED_FOCUS_CTRL == LED_RED_BY_MCU)||(LED_FOCUS_CTRL == LED_GREEN_BY_MCU))
#include "MCUCtrl.h"
#endif
#endif
...
#endif
#if 0
	case GPIOMAP_LED_REC:
		gpio_setPin(GPIO_REC_LED);
		break;
	case GPIOMAP_LED_WIFI:
...
#if 0
	case GPIOMAP_LED_REC:
		bvalue = gpio_getPin(GPIO_REC_LED);
		break;
	case GPIOMAP_LED_WIFI:
		bvalue = gpio_getPin(GPIO_WIFI_LED);
		break;
#endif
	default:
		bvalue = FALSE;
...
#endif
#if (LED_GREEN_CTRL == LED_FOCUS_BY_MCU)
			if (Value == 0) {
				MCUCtrl_SendCmd(UART_CMD_LED, UART_LED_GREEN, UART_PAR_OFF, 0);
			} else if (Value == 1) {
				MCUCtrl_SendCmd(UART_CMD_LED, UART_LED_GREEN, UART_PAR_ON, 0);
			} else if (Value == 2) {
				MCUCtrl_SendCmd(UART_CMD_LED, UART_LED_GREEN, UART_PAR_BLK_F, 0);
			} else if (Value == 3) {
				MCUCtrl_SendCmd(UART_CMD_LED, UART_LED_GREEN, UART_PAR_BLK_S, 0);
			}
#endif
			break;
...
#endif
#if (LED_RED_CTRL == LED_RED_BY_MCU)
			if (Value == 0) {
				MCUCtrl_SendCmd(UART_CMD_LED, UART_LED_RED, UART_PAR_OFF, 0);
			} else if (Value == 1) {
				MCUCtrl_SendCmd(UART_CMD_LED, UART_LED_RED, UART_PAR_ON, 0);
			} else if (Value == 2) {
				MCUCtrl_SendCmd(UART_CMD_LED, UART_LED_RED, UART_PAR_BLK_F, 0);
			} else if (Value == 3) {
				MCUCtrl_SendCmd(UART_CMD_LED, UART_LED_RED, UART_PAR_BLK_S, 0);
			} else if (Value == 4) {
				MCUCtrl_SendCmd(UART_CMD_LED, UART_LED_RED, UART_PAR_BLK_B, 0);
			}
#endif
			break;
...
#endif
#if (LED_FOCUS_CTRL == LED_FOCUS_BY_MCU)
			if (Value == 0) {
				MCUCtrl_SendCmd(UART_CMD_LED, UART_LED_BLUE, UART_PAR_OFF, 0);
			} else if (Value == 1) {
				MCUCtrl_SendCmd(UART_CMD_LED, UART_LED_BLUE, UART_PAR_ON, 0);
			} else if (Value == 2) {
				MCUCtrl_SendCmd(UART_CMD_LED, UART_LED_BLUE, UART_PAR_BLK_F, 0);
			} else if (Value == 3) {
				MCUCtrl_SendCmd(UART_CMD_LED, UART_LED_BLUE, UART_PAR_BLK_S, 0);
			}
#endif
			break;
		default:
...
#if 0
//lhb add LED Cont  @20190109---------------------------------------------------------------------------------
#define LHB_MOVIE_REC_LED  0
void LHB_LED_TurnOnLED(UINT32 uiLED)
{
   	switch (uiLED) {
        case LHB_MOVIE_REC_LED:
                gpio_setPin(LHB_GPIO_REC_LED);
	  break;
	 default:
	  break;
	}
}
void LHB_LED_TurnOffLED(UINT32 uiLED)
{
    	switch (uiLED) {
        case LHB_MOVIE_REC_LED:
               gpio_clearPin(LHB_GPIO_REC_LED);
	  break;
	 default:
	  break;
	}
}
BOOL LHB_LED_IsLEDOn(UINT32 uiLED)
{
   	BOOL bvalue = FALSE;
	switch (uiLED) {
        case LHB_MOVIE_REC_LED:
                bvalue = gpio_getPin(LHB_GPIO_REC_LED);
	  break;
	 default:
	  break;
	}
