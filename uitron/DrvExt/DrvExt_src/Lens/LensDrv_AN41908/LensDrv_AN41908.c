...
UINT32 g_uiLensSpiSema = 0, g_uiLensMoveSema = 0;


void LensDrv_InstallID(void)
{
    OS_CONFIG_SEMPHORE(g_uiLensSpiSema, 0, 1, 1);
    OS_CONFIG_SEMPHORE(g_uiLensMoveSema, 0, 1, 1);
}
...
static INT32 AN41908_SetZoomPosition(LENSMOTOR_INFO *pmotor_info, INT32 target_pos)
{
    ...
    if (target_pos > pmotor_info->LensMotorConfig[1].iCurrPosition) {
        ...
        rest_step = target_pos - pmotor_info->LensMotorConfig[1].iCurrPosition;
        ...
        rest_step = pmotor_info->LensMotorConfig[1].iCurrPosition - target_pos;
