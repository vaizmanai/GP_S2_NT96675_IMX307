...
//HW FSADJ 3.3 K
#define TV_NTSC_BURST_LEVEL_3_3                 0x3b//0x38
#define TV_NTSC_BLAND_LEVEL_3_3                 0x75//0x74
...
#define TV_PAL_BLAND_LEVEL_3_3                  0x7d
...
//        pll_setConfig(PLL_CONFIG_ID_IDE2_CLK_SRC, PLL_SRC_ID_PLL7);
	...
	if (DevIOCtrl.SEL.GET_ACT_DEVICE.DevID == DISPDEV_ID_TVNTSC) {
		...
		if (fullscreen == TRUE) {
			...
			DevIOCtrl.SEL.SET_WINDOW_H_TIMING.uiHvldStart	 = 0x113;//0x113;//0x108; //108 //112
...
static ER dispdev_closeTV(void)
{
	...
	{
		if (tv_loaddet_getdet() == TV_LOADDET_PLUGOUT)
			ide_setTvTestModeSel(IDETV_DAC_TESTMODE_SEL_FIX);
		else
			ide_setTvTestModeSel(IDETV_DAC_TESTMODE_SEL_COLORBAR);
	}
...
/*
    ...
    @return void
*/
/*void dispdev_installID(void)
{
    OS_CONFIG_FLAG(uiTVFlagID);
    OS_CONFIG_TASK(uiTVTaskID,  18, 1024,    dispdev_tv_task);
}*/