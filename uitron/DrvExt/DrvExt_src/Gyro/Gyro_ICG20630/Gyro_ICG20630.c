/*
    Gyro Driver InvenSense ICG20660.

    @file       GyroDrv_ICG20660.c
    @ingroup
    @note       Nothing.
...
{
    0x7F, 0x00, // Register bank 0
    0x06, 0x80, // PWR_MGMT_1, Reset
    0xFF, 0x0A, // delay 10 ms
    0x06, 0x01, // PWR_MGMT_1,wake up from sleep
    0x07, 0x00, // PWR_MGMT_2, enable accel ,gyro
    0x0F, 0xE0, // interrupt set to low active, open drian
    0x03, 0x10, // reset I2C (I2C_IF_DIS)
    0x10, 0x00, // Disable WoM interrupt
    0xFF, 0x01, // delay 1 ms
    0x7F, 0x20, // Register bank 2
    0xFF, 0x01, // delay 1 ms
    0x01, 0x01, // Gyroscope Configuration FS_SEL set 1 , +-50dps
    0x12, 0x03, // ACCEL INTER CTRL
    0x13, 0x85, // Wake-on Motion Threshold,
    0x14, 0x07, // Accelerometer Configuration ACCEL_FS_SEL set 3 ,+-6g
    0xFF, 0x01, // delay 1 ms
    0x7F, 0x00, // Register bank 0
    0xFF, 0x01, // delay 1 ms
...
static ER gyro_writeRegOneByte(UINT32 uiAddr, UINT32 uiData);
...
ER gyro_close(void)
{
    ...
    // 111 ¡V Gyroscope (all axes) disabled
    gyro_writeRegOneByte(REG_PWR_MGMT_2, 0x07);
...
ER gyro_shutdown(void)
{
    // 111 ¡V Gyroscope (all axes) disabled
    {
        ...
        spi_setTransferLen(g_TestSpiID, SPI_TRANSFER_LEN_1BYTE);
        spi_setCSActive(g_TestSpiID, TRUE);
        spi_writeSingle(g_TestSpiID, REG_WRITE|(0x07&0x7f));
        spi_writeSingle(g_TestSpiID, 0x07);
        spi_setCSActive(g_TestSpiID, FALSE);
        spi_close(g_TestSpiID);
    }

    {
        // R/C interrupt
        UINT32 INT_STATUS;
        gyro_readReg(0x19,(UINT32*)&INT_STATUS);
    }
    ...
    #if 0
    {
        ...
        spi_setTransferLen(g_TestSpiID, SPI_TRANSFER_LEN_1BYTE);
        spi_setCSActive(g_TestSpiID, TRUE);
        spi_writeSingle(g_TestSpiID, REG_WRITE|(0x06&0x7f));
        spi_writeSingle(g_TestSpiID, 0x41);
        spi_setCSActive(g_TestSpiID, FALSE);
        spi_close(g_TestSpiID);
    }
    #endif
    ...
    // set bank 0
    {
        ...
        spi_setTransferLen(g_TestSpiID, SPI_TRANSFER_LEN_1BYTE);
        spi_setCSActive(g_TestSpiID, TRUE);
        spi_writeSingle(g_TestSpiID, REG_WRITE| 0x7f);
        spi_writeSingle(g_TestSpiID, 0x00);
        spi_setCSActive(g_TestSpiID, FALSE);
        spi_close(g_TestSpiID);
    }
    ...
    // set bank 2
    {
        ...
        spi_setCSActive(g_TestSpiID, TRUE);
        spi_writeSingle(g_TestSpiID, REG_WRITE| 0x7f);
        spi_writeSingle(g_TestSpiID, 0x20);
        spi_setCSActive(g_TestSpiID, FALSE);
        spi_close(g_TestSpiID);
    }
...
static ER gyro_writeRegOneByte(UINT32 uiAddr, UINT32 uiData)
{
    // write register
    {
        ...
        spi_setTransferLen(g_TestSpiID, SPI_TRANSFER_LEN_1BYTE);
        spi_writeSingle(g_TestSpiID, (REG_WRITE)|(uiAddr&0x7f));
        spi_writeSingle(g_TestSpiID, uiData);
        spi_setCSActive(g_TestSpiID, FALSE);
        spi_close(g_TestSpiID);
    }
    return E_OK;
...
ER gyro_getSingleData(INT32 *puiDatX, INT32 *puiDatY, INT32 *puiDatZ)
{
    ...
    #if 0
    gyro_readReg(0x1B,(UINT32*)puiDatX);
    DBG_ERR("0x1B value =0x%x\r\n",*puiDatX);
    gyro_readReg(0x6B,(UINT32*)puiDatX);
    DBG_ERR("0x6B value =0x%x\r\n",*puiDatX);
    #endif
