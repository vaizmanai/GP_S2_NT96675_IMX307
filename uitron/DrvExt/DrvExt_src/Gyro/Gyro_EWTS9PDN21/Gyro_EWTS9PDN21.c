...
static void gyro_HandleGyroEvent(SPI_GYRO_INT gyroInt)
{
    ...
    case SPI_GYRO_INT_SYNC_END:
        ...
        for (i=0; i<uiGyroQueueCount/2; i++)
        {
            // 0x7C
...
//    gyroInfo.uiTransferCount = g_uiGyroFrNumber/2;  // 4 Transfer => Total 8 CS in one frame
    ...
    gyroInfo.uiTransferCount = gyroFreeRunParm.uiDataNum/2;  // 4 Transfer => Total 8 CS in one frame
    gyroInfo.uiOpInterval       = 1;
    // reserve 200 us for last transfer
    gyroInfo.uiTransferInterval = (gyroFreeRunParm.uiPeriodUs-200)/gyroInfo.uiTransferCount;//gyroFreeRunParm.uiPeriodUs/gyroInfo.uiTransferLen;
...
static void gyro_AdjustDataOffset(INT32* gx, INT32* gy)
{
    INT32  tempX,tempY;
...
ER gyro_getSingleData(INT32 *puiDatX, INT32 *puiDatY, INT32 *puiDatZ)
{
    ...
    gyro_readReg(REG_GYRO_OUTPUTA,(UINT32*)puiDatX);
    gyro_readReg(REG_GYRO_OUTPUTB,(UINT32*)puiDatY);
    *puiDatZ = 0;
    gyro_AdjustDataOffset(puiDatX, puiDatY);
    return E_OK;
}
    ...
    if((uiGyroBufOutIdx+uiDataNum)==uiGyroBufInIdx)
    {
        ...
        for (i=0; i<uiDataNum; i++)
        {
            ...
            *(puiDatY+i) = puiGyroBufY[k][2];
            ...
            if (i==0)
            {
                DBG_IND("Count = %d, x=%5d, y=%5d\r\n",uiGyroQueueCount,*puiDatX,*puiDatY);
            }
        }
        uiGyroBufOutIdx = j;
        //*puiNum = uiDataNum;
        *puiNum = uiGyroQueueCount;
        memset((void *)puiDatZ, 0, uiDataNum*sizeof(UINT32)>>1);
    }
    else if((uiGyroBufOutIdx+uiDataNum)>uiGyroBufInIdx)
    {
        DBG_ERR("asking for future data!!\r\n");
        memset((void *)puiDatX, 0, uiDataNum*sizeof(UINT32)>>1);
        memset((void *)puiDatY, 0, uiDataNum*sizeof(UINT32)>>1);
        memset((void *)puiDatZ, 0, uiDataNum*sizeof(UINT32)>>1);
        *puiNum = 0;
    }
    else if((uiGyroBufOutIdx+uiDataNum)<uiGyroBufInIdx)
