...
typedef enum {
	...
	NMI_IMGCAP_SET_W            = 0x02, ///< set max image width
	NMI_IMGCAP_SET_H            = 0x03, ///< set max image height
	NMI_IMGCAP_SET_MEM_RANGE    = 0x04, ///< set memory range
	NMI_IMGCAP_SET_JPG_BUFNUM   = 0x05, ///< set JPEG buffer number
	NMI_IMGCAP_SET_EXIF_FUNC    = 0x06, ///< set EXIF function (1: on, 0: off)
	// get param
...
typedef enum {
	...
	NMI_IMGCAP_EVENT_ENC_PREPARED = 0x02, ///< ImgCap has prepared for encoding
	NMI_IMGCAP_EVENT_STAMP        = 0x03, ///< data stamp callback
...
typedef struct {
	...
	UINT32 user_id;                     ///< user id for identifying user device
	UINT32 exif;                        ///< does it need exif header (1: need, 0: no need)
...
typedef struct {
	UINT32 wid;
	UINT32 hei;
	UINT32 exif_func;                     ///< exif func on/off (1: on, 0: off)
} NMI_IMGCAP_INIT;
