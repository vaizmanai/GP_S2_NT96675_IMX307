...
#define BXMUER_CALCSEC_UNKNOWN_FSSIZE (UINT64)0xFFFFFFFF
...
enum {
	...
	BSMUXER_PARAM_FLUSH_SEC,
	BSMUXER_PARAM_WRITE_BLKSIZE,
	BSMUXER_PARAM_VID_GOP,

	BSMUXER_PARAM_MAX_CUT,      //set max cut file num
...
typedef enum {
	...
	BSMUXER_CBEVENT_GETGPSDATA, //p1: mdatid
...
/**
    ...
    Set user data address and size to save in file header. If size is equal to zero,
    the old user data will be clean up.

    @param[in] addr  user data address
    @param[in] size  user data size

    @return void
*/
extern void     NMR_BsMuxer_SetUserData(UINT32 addr, UINT32 size);
...
/**
    Trigger to make a EMR file and pause normal rec
    (If EMR is disable, it doesn't work!)

    @param[in]  emr_id (1/5) ; pause_id (0/4)
    @return void
*/
extern ER       NMediaRec_TrigEMRPause(UINT32 emr_id, UINT32 pause_id);

/**
    Trigger to start loop recording with rollback second.

    @param[in]  mdatID
    @return void
*/
extern ER       NMediaRec_TrigLoopStart(UINT32 mdatID);

/**
    Trigger to stop loop recording.

    @param[in]  mdatID
    @return void
*/
extern ER       NMediaRec_TrigLoopStop(UINT32 mdatID);
...
/**
	Set Extend Crash Info.

	@param[in] pSetting Extend Crash Info

	@return void
*/
extern void     NMediaRec_SetExtendCrashInfo(NMEDIAREC_EXTCRASH_SETTING *pSetting);
...
/**
    ...
    @return void
*/
extern ER       NMediaRec_SetExtendCrash(UINT32 pathID);
...
*/
extern UINT32   NMediaRec_IsExtendCrash(void);
...
/**
	...
	@return void
*/
extern void     NMR_BsMuxer_SetEnCryptoKey(UINT8 *key, UINT32 value);
