...
typedef struct {
	...
	UINT8 bMirror;						///< Enable mirror. Default value is NVT_FALSE.
	UINT8 bFlip;							///< Enable flip. Default value is NVT_FALSE.
//	ISPRotateType eRotate;				///< Image rotation mode (This parameter is not supported.)
	...
	UINT8 bDewarp;						///< Dewarp Mode control
} ISPImageSetting;
...
typedef struct {
	...
	UINT32 uiConvergenceSpeed;			///< The adjustment speed of auto white balance. Range: 1~10, default value is 4
	UINT32 uiConvergenceStep;			///< The adjustment step of auto white balance. Range: 1~10, default value is 10
	UINT32 uiManualRGain;				///< Manual red gain value. Range: 1~1023
	UINT32 uiManualGGain;				///< Manual green gain value. This parameter is not supported.
	UINT32 uiManualBGain;				///< Manual blue gain value. Range: 1~1023
} ISPWBSetting;
...
/* HDR */
typedef struct {
	UINT8 bHDREnable;					///< Set HDR enable or disable
	ISPCtrlMode eHDRMode;				///< Not supported
	UINT32 uiHDRStrength;				///< Not supported
} ISPHDRSetting;

/* Noise Reduction */
typedef struct {
	UINT32 uiNR2DStrength;				///< Noise reduction strength in space domain. Range: 0~100, 
	UINT32 uiNR3DStrength;				///< 0:Disable isp 3dnr    1: Enable isp 3dnr
} ISPNRSetting;

/* Digital Image Stabilization */
typedef struct {
	UINT8 bDISEnable;					///< Enable DIS. Default value is NVT_FALSE
} ISPDISSetting;

/* Lens Distortion Correction */
typedef struct {
	UINT8 bLDCEnable;					///< Enable LDC. Default value is NVT_FALSE.
	UINT32 uiRatio;						///< Correct ratio. Range: 1~100
} ISPLDCSetting;

/* Lens Shading Correction */
typedef struct {
	UINT8 bLSCEnable;					///< Enable LSC. Default value is NVT_FALSE.
	UINT32 uiRatio;						///< Correct ratio. Range: 1~100
...
/*  DeFog */
typedef struct {
        ISP_DEFOG_MODE eDeFogMode;                     ///< Defog Mode 
...
typedef struct {
	UINT8 bEnable;						///< Enable or disable PIP function
}ISPPIPSetting;
...
/* DeFog */
/**
	Get current anti-fog setting from ISP.

	@param[in] uiViChannelID		Video input channel ID
	@param[out] psDeFogSetting	Pointer to the data structure of anti-fog setting
...
/**
	Set new anti-fog setting to ISP.

	@param[in] uiViChannelID		Video input channel ID
	@param[in] psDeFogSetting		Pointer to the data structure of anti-fog setting
