...
typedef enum {
    ...
    */
    NMEDIAPLAY_SPEED_1_2X    = NMP_BSDEMUX_US_IN_SECOND / 2,       ///< 1/2x  speed, trigger by timer (Play 1/2*I-frame number in second)
    NMEDIAPLAY_SPEED_1_4X    = NMP_BSDEMUX_US_IN_SECOND / 4,       ///< 1/4x  speed, trigger by timer (Play 1/4*I-frame number in second)
    NMEDIAPLAY_SPEED_1_8X    = NMP_BSDEMUX_US_IN_SECOND / 8,       ///< 1/8x  speed, trigger by timer (Play 1/8*I-frame number in second)
    NMEDIAPLAY_SPEED_1_16X   = NMP_BSDEMUX_US_IN_SECOND / 16,      ///< 1/16x speed, trigger by timer (Play 1/16*I-frame number in second)
...
extern void NMP_FileIn_InstallID(void) _SECTION(".kercfg_text");
extern void NMP_BsDemux_InstallID(void) _SECTION(".kercfg_text");
extern void NMP_VdoDec_InstallID(void) _SECTION(".kercfg_text");
extern void NMP_AudDec_InstallID(void)  _SECTION(".kercfg_text");
