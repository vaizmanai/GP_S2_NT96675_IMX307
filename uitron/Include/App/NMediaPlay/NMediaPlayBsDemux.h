...
typedef enum {
	...
	NMI_BSDEMUX_PARAM_VDO_FIRSTFRMSIZE,                 ///< first video frame size
	...
	NMI_BSDEMUX_PARAM_TRIG_TSDMX,                             ///< TS only, trigger TS demux
	NMI_BSDEMUX_PARAM_FILESIZE,                               ///< TS only, total file size
	NMI_BSDEMUX_PARAM_TSDMX_BUFBLK,                           ///< TS only, TS demux buffer block number
	/* get param */
	...
	NMI_BSDEMUX_PARAM_STEP_PLAY,                              ///< step play (direct)
} NMI_BSDEMUX_PARAM;
...
typedef struct {
	...
	UINT32 disp_idx;    ///< current video display index
