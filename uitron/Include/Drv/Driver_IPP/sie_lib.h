...
#define SIE_GRID_LINE_EN        0x00000400 ///< Enable function
...
#define SIE_INT_DRAM_OUT5_END      0x00001000 ///< Enable interrupt
...
typedef enum {
	OUT3_SOURCE_YOUT_ACC   = 0, ///< SIE Dram output channel 3 source = variation accumulation
	OUT3_SOURCE_ACT2,
...
typedef enum {
	...
	SIE_BURST_96W  = 2,   ///< 96  words
	SIE_BURST_128W = 3,   ///< 128 words
...
typedef enum { // would be removed soon
	IN0_BURST_LENGTH_64   = 0,    ///< SIE 1-2 Dram input channel 0 burst length = 64 words
	IN0_BURST_LENGTH_32   = 1,    ///< SIE 1-2 Dram input channel 0 burst length = 32 words
...
typedef enum { // would be removed soon
	OUT1_BURST_LENGTH_64   = 0,    ///< SIE 1-2 Dram input channel 1 burst length = 64 words
	OUT1_BURST_LENGTH_48   = 1,    ///< SIE 1-2 Dram input channel 1 burst length = 48 words
	ENUM_DUMMY4WORD(SIE_DRAM_OUT1_BURST_SEL)
} SIE_DRAM_OUT1_BURST_SEL;// would be removed soon

typedef enum { // would be removed soon
	OUT2_BURST_LENGTH_32   = 0,    ///< SIE 1-8 Dram input channel 2 burst length = 32 words
	OUT2_BURST_LENGTH_16   = 1,    ///< SIE 1-8 Dram input channel 2 burst length = 16 words
	ENUM_DUMMY4WORD(SIE_DRAM_OUT2_BURST_SEL)
} SIE_DRAM_OUT2_BURST_SEL;// would be removed soon

typedef enum { // would be removed soon
	OUT3_BURST_LENGTH_48   = 0,    ///< SIE 1-2 Dram input channel 3 burst length = 48  words
	OUT3_BURST_LENGTH_32   = 1,    ///< SIE 1-2 Dram input channel 3 burst length = 32  words
	ENUM_DUMMY4WORD(SIE_DRAM_OUT3_BURST_SEL)
} SIE_DRAM_OUT3_BURST_SEL;// would be removed soon

typedef enum { // would be removed soon
	OUT4_BURST_LENGTH_64   = 0,    ///< SIE 1-2 Dram input channel 4 burst length = 64 words
	OUT4_BURST_LENGTH_32   = 1,    ///< SIE 1-2 Dram input channel 4 burst length = 32 words
	ENUM_DUMMY4WORD(SIE_DRAM_OUT4_BURST_SEL)
} SIE_DRAM_OUT4_BURST_SEL;// would be removed soon

typedef enum { // would be removed soon
	OUT5_BURST_LENGTH_48   = 0,    ///< SIE 1-2 Dram input channel 5 burst length = 48 words
	OUT5_BURST_LENGTH_32   = 1,    ///< SIE 1-2 Dram input channel 5 burst length = 32 words
...
typedef enum {
	...
	SIE_CLKSRC_320,      ///< SIE clock source as fixed-clk 320MHz
} SIE_CLKSRC_SEL;
//@}
...
//@{
typedef enum {
	SIE_CHG_CROP_WIN        =  0,   ///< refer to "SIE_CRP_WIN_INFO"        , change crop size and cfa_pat
	SIE_CHG_ACT_WIN,                ///< refer to "SIE_ACT_WIN_INFO"         , change Active size and cfa_pat
	...
	SIE_CHG_BP,                     ///< refer to "SIE_BREAKPOINT_INFO"     , change BP
	...
	SIE_CHG_YOUT,                   ///< refer to "SIE_YOUT_INFO"           , change YOUT parameters
	...
	SIE_CHG_LA_TH,                  ///< refer to "SIE_LA_TH"               , change STCS LA Threshold
...
typedef struct {
	...
	UINT8               uiWinSzX;    ///< Y accumulation - horizontal window size
	UINT8               uiWinSzY;    ///< Y accumulation - vertical window size
...
typedef struct _SIE_BURST_LENGTH {
	...
	SIE_BURST_SEL       BurstLenOut1; ///< for SIE1/2/
	SIE_BURST_SEL       BurstLenOut2; ///< for SIE1/2
	SIE_BURST_SEL       BurstLenOut3; ///< for SIE1/2
	SIE_BURST_SEL       BurstLenOut4; ///< for SIE1/2
	SIE_BURST_SEL       BurstLenOut5; ///< for SIE1/2
...
typedef struct { // would be removed soon
	...
	SIE_DRAM_OUT5_BURST_SEL  DramOut5Burst;
...
typedef struct {
	...
	LFN_CSB_TBL_NUM_SEL LfnCsbTblNumSel;
	SIE_BCC_ROUND_SEL   EncRoundSel;        ///< Raw encoder - encode rounding selection
	UINT32              uiEncRoundInit1;    ///< Raw encoder - encode random rounding 1st initial value
	UINT32              uiEncRoundInit2;    ///< Raw encoder - encode random rounding 2nd initial value
	UINT32              uiBrcSegBitNo;      ///< Raw encoder - BRC target encoded bit number per segment
	BCC_DIST_CAL_TYPE   DistCalType;
	UINT32              uiDistDiffWt;
	UINT32              uiDistSumWt;
	UINT32              uiDistShift;
	BOOL (*p_bQtblEn)[FINE_LBN];                  ///< assign a UINT32 [LFN_ENTRY_NUM][FINE_LBN] array for Q table enable settings
	UINT32              *p_uiLfnTbl;              ///< assign a UINT32 [MAX_LC_TBL_NUM] array for LFN table
	UINT32              *p_uiCsbTbl;              ///< assign a UINT32 [MAX_LC_TBL_NUM] array for CSB table
	BOOL                bBccGammaEn;
...
typedef struct {
	...
	UINT32                   uiOut3Addr; ///< SIE output ch3 dram addr; "zero value" for null configuration
	//UINT32                   uiOut4Addr; ///< SIE output ch4 dram addr; "zero value" for null configuration
	UINT32                   uiOut5Addr; ///< SIE output ch5 dram addr; "zero value" for null configuration
	...
	UINT32                   uiOut5Size; ///< SIE output ch5 dram size, in bytes, for cache-handle; "zero value" for null configuration
...
typedef struct {
	...
	UINT32                   uiOut5Lofs; ///< SIE output ch5 dram line-offset, in bytes; "zero value" for null configuration
