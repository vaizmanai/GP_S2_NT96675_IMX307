...
typedef struct _IFE2_PARAM {
	...
	IFE2_FUNC_EN            DrtDramOutEn;   ///< when direct-to-IME mode is enable, set for output data to dram
...
/**
    Dram output enable when direct output to IME

    Basically this function is used for debugging when engine data directly output to IME.
    How to use this function is shown in the following,

    1. ife2_setMode(*), set direct to IME mode
    2. ife2_chgDirectModeDramOutEnable(IFE2_FUNC_ENABLE), set output to Dram enable
    3. ife2_chgLineoffset(*), set output lineoffset
    4. ife2_chgDmaAddr(*), set output buffer address

    @param[in] SetEn  enable /disable function

    @return
        - @b E_OK: Done with no error.
        - Others: Error occured.
*/
extern ER ife2_chgDirectModeDramOutEnable(IFE2_FUNC_EN SetEn);
//------------------------------------------------------------------
...
/**
  ...
  @return VOID
*/
extern VOID ife2_setHwReset(BOOL bEnable);
//------------------------------------------------------------------------------
