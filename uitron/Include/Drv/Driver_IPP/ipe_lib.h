/**
    ...
    NT96510 IPE driver extern header file.
...
#define IPE_VACC_EN                    0x00100000  ///< Variation accumulation enable
//@}
...
#define IPE_INTE_VAOUTEND              0x00000080  ///< Interrupt enable for DRAM input done
...
#define IPE_INTE_ALL                   0x000000fe  ///< All interrupt enable
//@}
...
#define IPE_INT_VAOUTEND              0x00000080  ///< Interrupt enable for DRAM input done
...
#define IPE_INT_ALL                   0x000000fe  ///< All interrupt enable
//@}
...
#define IPE_SET_VA                  0x00080000  ///< Select to update VA parameters //sc
#define IPE_SET_INDEP_VA            0x00100000  ///< Select to update Independent VA parameters //sc
...
typedef enum {
	...
	ETH_OUT_BOTH = 2,                 ///< both origianl size and downsample size //sc
...
typedef struct {
	UINT32              uiWinStX;    ///< Variation accumulation - horizontal starting point
	UINT32              uiWinStY;    ///< Variation accumulation - vertical starting point
	UINT32              uiWinNmX;    ///< Variation accumulation - horizontal window number
	UINT32              uiWinNmY;    ///< Variation accumulation - vertical window number
	UINT32              uiWinSzX;    ///< Variation accumulation - horizontal window size
	UINT32              uiWinSzY;    ///< Variation accumulation - vertical window size
	UINT32              uiWinSpX;    ///< Variation accumulation - horizontal window spacing
	UINT32              uiWinSpY;    ///< Variation accumulation - vertical window spacing
} IPE_VA_WIN_PARAM;
...
typedef struct {
	...
	INT8                    iTapC;      ///<
	INT8                    iTapD;      ///<
	IPE_VA_FLTR_SYMM_SEL    FltrSymm;   ///<
	IPE_VA_FLTR_SIZE_SEL    FltrSize;   ///<
	UINT8                   uiDiv;      ///<
	UINT16                   uiThL;      ///<
	UINT16                   uiThH;      ///<
} IPE_VA_FLTR_PARAM;
...
typedef struct {
	IPE_VA_FLTR_PARAM       FltrH;
	IPE_VA_FLTR_PARAM       FltrV;
	BOOL                    bLineMax;   ///<
	BOOL                    bCnt;       ///<
...
typedef struct {
	...
	IPE_CC_OFSSEL CCOfsSel;    ///< Select CC offset mode //sc
...
typedef struct {
	...
	UINT32 *uiCConTab;  ///< Legal range : 0~1024, CbCr contrast adjust, 128 : NO contrst adjust, C' = (C * CCon) >> 7
...
typedef struct {
	...
	BOOL bOutYCToDRAM;              ///< Enable of YC data to DRAM
	BOOL bOutVAToDRAM;              ///< Enable of VA data to DRAM, combine with VACC
	...
	UINT32 uiAdrVA;                 ///< Legal range : multiples of 4(word aligned), DRAM output address
	...
	UINT32 uiLofsVA;                ///< Legal range : multiples of 4(word aligned), DRAM output address
	//UINT32 uiOutHeight;
	IPE_DMAOSEL DmaOutSel;          ///< DMA output channel data selection
	...
	IPE_ETH_SIZE EthOutInfo;          ///< ETH output information
	IPE_VA_OUTSEL VAOutSel;                   ///< VA output information
} IPE_OUTYCINFO;
//@}
...
typedef struct {
	...
	IPE_VA_FLTR_GROUP_PARAM *pVaFltrG1;     ///< VA Group 1 filter parameters
	IPE_VA_FLTR_GROUP_PARAM *pVaFltrG2;     ///< VA Group 1 filter parameters
	IPE_VA_WIN_PARAM    *pVaWinInfo;            ///< VA window parameters
	IPE_INDEP_VA_PARAM  pVaIndepWinInfo[5];     ///< Independent VA window parameters
} IPE_IQINFO;
//@}
...
extern void ipe_setHwReset(BOOL bEnable);
...
extern UINT32 ipe_getVADmaOutAddr(void);
extern UINT32 ipe_getVADmaOutOffset(void);
extern BOOL ipe_getVADmaOutSel(void);
extern void ipe_getVaWinInfo(IPE_VA_WIN_PARAM *VaWinInfo);
extern IPE_INDEP_VA_PARAM ipe_getVaIndepWin(UINT32 uiIdx);
extern void ipe_getVAIndepWinRslt(IPE_INDEP_VA_WIN_RSLT *pSetting, UINT32 uiIdx);
extern void ipe_getVAResult(UINT32 *puiG1H, UINT32 *puiG1V, UINT32 *puiG1HCnt, UINT32 *puiG1VCnt, UINT32 *puiG2H, UINT32 *puiG2V, UINT32 *puiG2HCnt, UINT32 *puiG2VCnt);
extern void ipe_getVAResultManual(UINT32 *puiG1H, UINT32 *puiG1V, UINT32 *puiG1HCnt, UINT32 *puiG1VCnt, UINT32 *puiG2H, UINT32 *puiG2V, UINT32 *puiG2HCnt, UINT32 *puiG2VCnt, IPE_VA_WIN_PARAM *VaWinInfo, UINT32 uiAddr, UINT32 uiLofs);
//parameter check
extern ER ipe_checkParam(IPE_MODEINFO *ModeInfo);
extern ER ipe_checkVAWinInfo(IPE_VA_WIN_PARAM *VaWinInfo, STR_IMG_SIZE size);
extern ER ipe_checkVALofs(IPE_VA_WIN_PARAM *VaWinInfo, IPE_OUTYCINFO *OutInfo);
//@}
