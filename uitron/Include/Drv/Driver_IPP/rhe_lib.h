...
#define RHE_INTE_DIRBUF_ERR         0x00010000   ///< SIE direct line buffer error
...
#define RHE_INT_DIRBUF_ERR          0x00010000   ///< SIE direct line buffer error

#define RHE_IN_CHL_NUM              2
...
*/
typedef enum {
	RHE_CFA_RGGB = 0,     ///< R/G/G/B;  or R/C/C/B;  R/G/B/G, G/I/G/I
	RHE_CFA_GRBG = 1,     ///< G/R/B/G;  or C/R/B/C;  G/B/G/R, I/G/I/G
	RHE_CFA_GBRG = 2,     ///< G/B/R/G;  or C/B/R/C;  G/I/G/I, B/G/R/G
	RHE_CFA_BGGR = 3,     ///< B/G/G/R;  or B/C/C/R;  I/G/I/G, G/R/G/B
	RHE_CFA_IR4B = 4,     ///  RGBIR Only             B/G/R/G, G/I/G/I
	RHE_CFA_IR5G = 5,     ///  RGBIR Only             G/R/G/B, I/G/I/G
	RHE_CFA_IR6G = 6,     ///  RGBIR Only             G/I/G/I, R/G/B/G
	RHE_CFA_IR7I = 7,     ///  RGBIR Only             I/G/I/G, G/B/G/R
...
/**
    RHE enum - Luma source selection
*/
typedef enum {
	RHE_LUMA_Y  = 0,   ///< Ref. R,G,B
	RHE_LUMA_G  = 1,   ///< G/C only
	ENUM_DUMMY4WORD(RHE_LUMA_SEL)
} RHE_LUMA_SEL;
//------------------------------------
...
/**
    RHE structure - DMA loop setting info.
*/
typedef struct _RHE_DMA_LOOP_INFO_ {
	RHE_FUNC_EN             DMAloopEn;      ///< DMA loop enable
	UINT32                  uiDMAloopline;  ///< DMA line buffer
} RHE_DMA_LOOP_INFO;
//------------------------------------
...
typedef struct _RHE_COLOR_GAIN_PARAM_ {
	...
	UINT32 uiCGainIR;       ///<  Color gain for IR channel
...
typedef struct _RHE_COLOR_OFS_PARAM_ {
	...
	UINT32 uiCOfsIR;       ///<  Color offset for IR channel
} RHE_COLOR_OFS_PARAM;
//------------------------------------
...
typedef struct _RHE_CADJ_PARAM_ {
	...
	UINT32                  *puiCAdjModLutA;    ///< contrast adjustment mod-A LUT, 8 entries, support auto mode
	UINT32                  *puiCAdjModLutB;    ///< contrast adjustment mod-B LUT, 8 entries, support auto mode
...
typedef struct _RHE_IN_IMG_INFO_ {
	...
	RHE_DMA_LOOP_INFO   DMAloopInfo;    ///< DMA loop info
}  RHE_IN_IMG_INFO;
//------------------------------------
...
typedef struct _RHE_MODE_PARAM_ {
	...
	RHE_LUMA_SEL            LumaSel;            ///< luma source selection
}  RHE_MODE_PARAM;
//-----------------------------------------------------------------------
...
Set HW reset flag
  ...
  HW will reset in rhe_open() and this flag will be cleared
...
extern VOID rhe_setHwReset(BOOL bEnable);
...
extern ER rhe_startdbg(UINT32 loadtype);
//-----------------------------------------------------
/**
    RHE Set Mode
...
/**
    RHE change DMA loop ctrl and line size

    @param[in] pChgDMALoopInfo  DMA loop ctrl and line size

    @return ER
*/

extern ER rhe_chgDMALoopInfo(RHE_DMA_LOOP_INFO *pChgDMALoopInfo);
//-----------------------------------------------------
...
/**
    RHE change luma source selection

    @param[in] RHE_LUMA_SEL  0:Y / 1:G

    @return ER  E_OK: setting done
                 Others: setting error
*/
extern ER rhe_chgLumaSource(RHE_LUMA_SEL LumSel);
//-----------------------------------------------------
...
extern VOID rhe_update_opmode(RHE_OPMODE rhe_opmode_set);
#endif
