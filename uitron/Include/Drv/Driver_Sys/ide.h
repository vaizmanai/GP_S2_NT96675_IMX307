...
typedef enum {
	...
	IDE_BLEND_O1 = 2,                           ///< Layer O1 (if FMT = ARGB1555 => source alpha = 1)
	IDE_BLEND_O1_GLBALPHA4 = 3,                 ///< Layer O1 & FMT=ARGB1555&source alpha = 0
...
typedef enum {
	...
	TV_LOADDET_UNKNOW  =  0x0, 				///< TV load det unknow
	TV_LOADDET_PLUGIN  =  0x1,				///< TV load det in
	TV_LOADDET_PLUGOUT =  0x2,				///< TV load det out
	ENUM_DUMMY4WORD(TV_LOADDET_STS)
} TV_LOADDET_STS;
//@}
...
void idec_setRgbdSwap(IDE_ID id, BOOL b_en);
...
void idec_waitLoadDone(IDE_ID id);
...
// IDE control
#define ide_setRgbdSwap(b_en)                   idec_setRgbdSwap(IDE_ID_1, b_en)
...
void ide_setTvOpRefVol(UINT8 vol);
...
void   tv_loaddet_open(void);
TV_LOADDET_STS  tv_loaddet_getdet(void);
void   tv_loaddet_close(void);
BOOL   tv_loaddet_getopen(void);
extern void tv_installID(void) _SECTION(".kercfg_text");
...
#define ide_waitLoadDone()                                 idec_waitLoadDone(IDE_ID_1)
