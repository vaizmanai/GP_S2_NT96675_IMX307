...
typedef struct {
	...
	ER(*norReadRegData)(UINT32, UINT32, UINT32, UINT8 *);
	//< Nor flash read specific register data
	//< Context are :
	//< - @b UINT32 : register address.
	//< - @b UINT32 : data address
	//< - @b UINT32 : data length
	//< - @b UINT8 *: buffer to be read
