...
#define SDIO_SRC_CLK_END       SDIO_SRC_CLK_192MHZ
//@}
...
typedef enum {
	...
	SDIO_CONFIG_ID_HS200_MAX_CLK,			 ///< Set maximum clock frequency in HS200 Mode
	///< (Valid value: 1~208000000, unit: Hz)
	///< (Valid when SDIO_CONFIG_ID_BUS_SPEED_MODE has SDIO_BUS_SPEED_MODE_HS200)
	SDIO_CONFIG_ID_HS200_DRIVING,			 ///< Set driving/sink when card is operated in HS200 Mode
	///< (valid context: (5, 10, 15, 20) * 10, unit: mA. Default 10 mA)
	///< (Valid when SDIO_CONFIG_ID_DRIVING_SINK_EN is set set to TRUE)
	SDIO_CONFIG_ID_HS200_INPUT_DELAY,		///< Set input delay parameter in HS200 Mode
	SDIO_CONFIG_ID_HS200_OUTPUT_DELAY, 	///< Set output delay parameter in HS200 Mode
	...
	///< valid context: (2.5, 5, 7.5, 10) * 10, unit: mA. Default 10 mA
	///< (Valid when SDIO_CONFIG_ID_DRIVING_SINK_EN & SDIO_CONFIG_ID_DRIVING_SINK_CMDDATA_EN  is set to TRUE)
	SDIO_CONFIG_ID_HS200_CMD_DRIVING,       ///< Set driving/sink of cmd pin when card is operated in HS200 Mode
	///< valid context: (2.5, 5, 7.5, 10) * 10, unit: mA. Default 10 mA
	///< (Valid when SDIO_CONFIG_ID_DRIVING_SINK_EN & SDIO_CONFIG_ID_DRIVING_SINK_CMDDATA_EN  is set to TRUE)
	SDIO_CONFIG_ID_HS200_DATA_DRIVING,      ///< Set driving/sink of data pin when card is operated in HS200 Mode
	///< valid context: (2.5, 5, 7.5, 10) * 10, unit: mA. Default 10 mA
	///< (Valid when SDIO_CONFIG_ID_DRIVING_SINK_EN & SDIO_CONFIG_ID_DRIVING_SINK_CMDDATA_EN  is set to TRUE)
...
extern ER                       sdio_sendIOTestCMD(UINT32 id, UINT32 cmd, UINT32 argu);
extern ER                       sdio1strg_eraseSectors(UINT32 id, UINT32 ulLBAddr, UINT32 ulSctCnt);
extern ER                       sdio2strg_eraseSectors(UINT32 id, UINT32 ulLBAddr, UINT32 ulSctCnt);
extern ER                       sdio3strg_eraseSectors(UINT32 id, UINT32 ulLBAddr, UINT32 ulSctCnt);
