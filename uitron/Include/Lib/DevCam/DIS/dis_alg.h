...
extern BOOL dis_set_blksize(DIS_BLKSZ blksz);		// 0 for large block size & 1 for small block size. Return FALSE if failed

//#NT#2018/09/07#joshua liu -begin
...
/**
  Retrieve the newest valid motion vectors and time stamp.  
  Return time stamp
*/
extern UINT64 dis_getReadyMotionVec(MV_INFO_SIZE *mvInfoSize);
extern BOOL dis_time_stamp_ready(void);
