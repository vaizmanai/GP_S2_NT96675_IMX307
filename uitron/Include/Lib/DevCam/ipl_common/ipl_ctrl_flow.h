...
typedef enum _IPC_RUNTIMECHG {
	...
	//IPC_Chg_ImeP5Out    = 0x00010000,   ///< chg path4 output size/lineofs/fmt/PathEn       ,data type :IPC_IMEOut_Info
	...
	IPC_Chg_CFA         = 0x00100000,   ///< chg sie/ipl cfa pattern			            ,data type :IPC_CFA_Info
...
typedef enum _IPC_RUNTIMECHG_IQ {
	...
	IPC_IME_IQLoad_ISR  = 0xd0000000,   ///< chg IME iq parameter ISR load                  ,data type :IPC_IQparam
...
typedef struct {
	...
	IME_HAL_MD_INFO	md_info;			//md info, will be set if evt_out_en
} IPC_IMEOut_Info;
...
/**
	IPL raw_image cfa runTimeChg

	definition for change sie and ipl cfa pattern when IPL in "IPL_RUN" status.
*/
typedef struct {
	IPL_HAL_PARASET idx;
	IPL_PROC_ID     id;
...
/**
	IPL Engine Interrupt counter
*/
typedef struct {
	UINT32 ipl_proc_end_cnt;
	UINT32 rhe_fmd_cnt;
	UINT32 ife_fmd_cnt;
	UINT32 dce_fmd_cnt;
	UINT32 ipe_fmd_cnt;
...
typedef enum _IPLCTRL_QUERY {
	...
	ICF_FLOWDIRECT      = ICF_FLOW_DIRECT,  ///< minimum buffer number needed by flow_DIRECT.
	...
	ICF_IPE_VAOUT_RDY	= 20,			///< get ipe va out information
	ICF_SIE_PRV_INTE_EN	= 21,			///< get sie preview interrupt enable bit, data type SIE_INTE_EN
	ICF_SIE_DFT_INTE_EN	= 22,			///< get sie default interrupt enable bit, data type SIE_INTE_EN
	ICF_SIE_YOUT_RDY	= 23,			///< get sie ready yout information, data type SIE_YOUT_PARAM
	ICF_RHE_SUBOUT_RDY	= 24,			///< get rhe ready subout address(defog), data type UINT32
	ICF_IME_STAINFO_RDY	= 25,			///< get ime ready tmnr stainfo, data type IME_HAL_STAINFO
	ICF_IPL_INTE_CNT	= 26,			///< get ipl engine interrupt count, data type IPC_INTE_CNT_INFO
