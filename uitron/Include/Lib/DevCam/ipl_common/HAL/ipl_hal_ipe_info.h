...
typedef enum _IPEFUNCEN {
	...
	IPE_VACCEn          = 0x00100000,       ///< VA Enable
...
typedef enum _CC_OFSSEL {
	CC_OFS_BYPASS_SEL = 0,      ///< bypass
	CC_OFS_Y_FULL_SEL,          ///< offset for full YCC to RGB
	CC_OFS_Y_BTU_SEL,           ///< offset for Bt601/Bt709 YCC to RGB
	ENUM_DUMMY4WORD(CC_OFSSEL)
} CC_OFSSEL;
...
typedef struct _IPE_CCON {
	...
	UINT32         *puiCConTab; ///< Legal range : 0~1024, CbCr contrast adjust, 128 : NO contrst adjust, C' = (C * CCon) >> 7
	///< tab num:17
} IPE_CCON;
...
typedef struct _IPE_ETH_DramRst {
	UINT32      addr[2];        ///< 0 -> original, 1 -> subsample
	UINT32      lineofs[2];     ///< 0 -> original, 1 -> subsample
	UINT32      width;          ///< -
	UINT32      height;         ///< -
	IPE_EthFmt  fmt;
	IPE_EthSel  outsel;
...
*/
typedef enum {
	IPE_VA_FLTR_MIRROR = 0,
	IPE_VA_FLTR_INVERSE = 1,
	ENUM_DUMMY4WORD(IPE_HAL_VA_FLTR_SYMM_SEL)
} IPE_HAL_VA_FLTR_SYMM_SEL;

typedef enum {
	IPE_VA_FLTR_SIZE_1 = 0,
	IPE_VA_FLTR_SIZE_3 = 1,
	IPE_VA_FLTR_SIZE_5 = 2,
	IPE_VA_FLTR_SIZE_7 = 3,
	ENUM_DUMMY4WORD(IPE_HAL_VA_FLTR_SIZE_SEL)
} IPE_HAL_VA_FLTR_SIZE_SEL;

typedef enum {
	IPE_VA_OUT_GROUP1 = 0,
	IPE_VA_OUT_BOTH = 1,
	ENUM_DUMMY4WORD(IPE_HAL_VA_OUTSEL)
} IPE_HAL_VA_OUTSEL;
...
typedef struct {
	...
	INT8                        iTapC;      ///<
	INT8                        iTapD;      ///<
	IPE_HAL_VA_FLTR_SYMM_SEL    FltrSymm;   ///<
	IPE_HAL_VA_FLTR_SIZE_SEL    FltrSize;   ///<
	UINT8                       uiDiv;      ///<
	UINT16                      uiThL;      ///<
	UINT16                      uiThH;      ///<
} IPE_HAL_VA_FLTR_PARAM;

typedef struct {
	IPE_HAL_VA_FLTR_PARAM       FltrH;
	IPE_HAL_VA_FLTR_PARAM       FltrV;
	BOOL                    bLineMax;   ///<
	BOOL                    bCnt;       ///<
} IPE_HAL_VA_FLTR_GROUP_PARAM;
...
typedef struct {
	IPE_HAL_INDEP_GRP indep_win_rst[5];
} IPE_HAL_INDEP_VA_WIN_RSLT;

typedef struct {
	UINT32                  addr;
	UINT32                  lofs;
	IPE_HAL_VA_WIN_PARAM    va_win;
	IPE_HAL_VA_OUTSEL       va_outsel;
	UINT32                  frm_cnt;
} IPE_VA_DramRst;
...
typedef struct {
	UINT32* p_g1h;			///< output
	UINT32* p_g1v;			///< output
	UINT32* p_g1hcnt;		///< output
	UINT32* p_g1vcnt;		///< output
	UINT32* p_g2h;			///< output
	UINT32* p_g2v;			///< output
	UINT32* p_g2hcnt;		///< output
	UINT32* p_g2vcnt;		///< output
	IPE_VA_DramRst param;	///< input parameters
} IPE_VARST;
...
typedef struct _IPE_SUBFUNC {
	...
	IPE_HAL_VA_FLTR_GROUP_PARAM *pVaFltrG1;     ///< VA Group 1 filter parameters
	IPE_HAL_VA_FLTR_GROUP_PARAM *pVaFltrG2;     ///< VA Group 2 filter parameters
	IPE_HAL_VA_WIN_PARAM         va_win;        ///< VA window parameters
	IPE_HAL_INDEP_VA_PARAM       va_indep_win[5];   ///< Independent VA window parameters

	UINT32 wait_gamma_load_end;		///< IPE Wait Gamma SRAM Load End Flag while IPE_LOAD
	UINT32 wait_3dcc_load_end;		///< IPE Wait 3dcc SRAM Load End Flag while IPE_LOAD
} IPE_SUBFUNC;
