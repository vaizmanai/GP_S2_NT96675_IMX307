...
typedef enum _CH3_SRC {
	CH3_YOUT_ACC        = 0,    ///< shdr sub-raw out
	CH3_ACTIVE_WIN2     = 1,    ///< output active window 2
...
typedef struct _IPP_SIE_SIGNAL {
	...
	SIE_PACKBUS parallel_bitsel;    ///< parallel input raw bit, support 10/12 bit
} IPP_SIE_SIGNAL;
...
typedef struct _SIE_WINDOW {
	...
	struct {
		...
		UINT32  SizeH;      ///< active window size
		UINT32  SizeV;      ///< active window size
	} Act_2;                          ///< active window 2, only for ch3 output
...
typedef enum {
	STATS_LA_RGB = 0,
	STATS_LA_G,
	ENUM_DUMMY4WORD(SIE_STATS_LA_RGB2Y_SEL)
} SIE_STATS_LA_RGB2Y_SEL;
...
typedef struct {
	...
	UINT8       LaRGB2YWeight;  ///<
	BOOL        bLaThEn;
	SIE_STATS_LA_RGB2Y_SEL  la_rgb2y1_sel;
	SIE_STATS_LA_RGB2Y_SEL  la_rgb2y2_sel;
} SIE_STATS_PATH_INFO;
...
typedef struct _SIE_BURSTLENGTH_SET {
	...
	SIE_OUT1_BURST_LENG burstLeng_OutCh1;                     //@SIE1/SIE2
	SIE_OUT2_BURST_LENG burstLeng_OutCh2;                     //@SIE1/SIE2
	SIE_OUT3_BURST_LENG burstLeng_OutCh3;                     //@SIE1
	SIE_OUT4_BURST_LENG burstLeng_OutCh4;                     //@SIE1

	SIE_IN0_BURST_LENG burstLeng_InCh0;                       //@SIE1
...
typedef struct _SIE_HAL_YOUT_INFO {
	...
	BOOL    rgb2y_sel;          ///< 0 -> y = sum all pixel, 1 -> y = g channel
} SIE_HAL_YOUT_INFO;
...
typedef struct _SIE_SUBFUNC {
	...
	PAT_GEN_INFO PatGen;        ///< pattern gen                                @SIE1/SIE2
