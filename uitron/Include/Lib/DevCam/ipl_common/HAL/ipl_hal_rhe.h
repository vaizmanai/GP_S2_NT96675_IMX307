...
typedef enum _RHE_UPDATE {
	...
	RHE_HWRST				= 0x20000000,   ///< set hw reset flag to driver, will do reset at rhe_open
...
typedef enum _RHE_READ {
	...
	RHE_R_RAW_IN_ADDR   = 0x0000000A,   ///< read rhe input address                             Data type: UINT32[2]
...
typedef struct _RHE_Manager {
	...
	RHE_HAL_HDR_EVGAP_SEL       hdr_frame_gaps;
	...
	BOOL                        dma_loop_en;                ///< RHE DMA loop enable(for direct mode SHDR SIE2 input)
	UINT32                      dma_loop_line;              ///< RHE DMA loop line buffer
	...
	BOOL    					subimg_lum_sel;      		///< RHE sub-input luma source select
															///< 0 -> y = sum all pixel, 1 -> y = g channel
