...
typedef enum _IPL_FLOW_PROC_MSG {
	...
	IPL_FLOW_RAW2YCC_DEFOG  = 0x00000E00,  ///< process LCA image + PRI: get sensor defog parameters
...
#define IPL_IME_OUT_TAG      MAKEFOURCC('I','M','E','O')
#define IPL_IME_MD_OUT_TAG   MAKEFOURCC('M','D','O','T')
#define IPL_IME_UNLOCK_TAG   MAKEFOURCC('U','N','L','K')
/**
...
typedef struct _IPL_YCC_ADDR_INFO {
	...
	UINT32 ext_addr;         ///< extend buffer start address	//remove when ctrl/image unit ready
	...
	BOOL p1_encode_en;       ///< ime path1/ref encode on/off
	UINT32 ext_buf_size;     ///< extend buffer besides yuv output(ex: md_evt)
} IPL_YCC_ADDR_INFO;

/**
    IPL IME enum - YUV data type
*/
typedef enum _IPL_IME_YUV_TYPE_ {
	IPL_IME_YUV_TYPE_FULL  = 0,  ///< full range
	IPL_IME_YUV_TYPE_BT601 = 1,  ///< BT.601
	IPL_IME_YUV_TYPE_BT709 = 2,  ///< BT.709
	ENUM_DUMMY4WORD(IPL_IME_YUV_TYPE)
} IPL_IME_YUV_TYPE;
...
typedef enum _IPL_RAW_PIX {
	...
	// RGBIR 4x4 data format
	IPL_RGBIR_PIX_OFFSET = 100,
    IPL_RGBIR_PIX_RGBG_GIGI = 0+IPL_RGBIR_PIX_OFFSET,
	IPL_RGBIR_PIX_GBGR_IGIG = 1+IPL_RGBIR_PIX_OFFSET,
	IPL_RGBIR_PIX_GIGI_BGRG = 2+IPL_RGBIR_PIX_OFFSET,
	IPL_RGBIR_PIX_IGIG_GRGB = 3+IPL_RGBIR_PIX_OFFSET,
	IPL_RGBIR_PIX_BGRG_GIGI = 4+IPL_RGBIR_PIX_OFFSET,
	IPL_RGBIR_PIX_GRGB_IGIG = 5+IPL_RGBIR_PIX_OFFSET,
	IPL_RGBIR_PIX_GIGI_RGBG = 6+IPL_RGBIR_PIX_OFFSET,
	IPL_RGBIR_PIX_IGIG_GBGR = 7+IPL_RGBIR_PIX_OFFSET,
	IPL_RGBIR_PIX_4X4_MAX_CNT = 8+IPL_RGBIR_PIX_OFFSET,

	// Y only
	IPL_PIX_Y = 0,      ///< start pixel Y only
...
*/
extern UINT32 IPL_UTI_CONV2_HDR_ORDER(UINT32 id, UINT32 mf_total_frm_bit);
...
/**
     IPL_UTI_SEN_FPS_ADJ_VTS

     @note adjust sensor vts signal by sensor fps
     @param[in] sen_fps      sensor fps
     @param[in] img_fps      image fps
     @param[in] sen_vts      sensor default vts
     @return
         - @b UINT32    current_vts
*/
extern UINT32 IPL_UTI_SEN_FPS_ADJ_VTS(UINT32 sen_fps, UINT32 img_fps, UINT32 sen_vts);
