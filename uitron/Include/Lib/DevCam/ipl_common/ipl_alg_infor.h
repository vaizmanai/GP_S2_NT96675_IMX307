...
typedef enum _IPL_SEL_ITEM {
	...
	IPL_SEL_PUSH_OUT_TYPE,             		///<ipl buffer output push type, data type #SEL_PUSH_TYPE
//ALG
	...
	IPL_SEL_SHDR_DELTA_EV,	             	///<SHDR Delta EV, , range = [64, 4095], default value: 512
	IPL_SEL_WDR_IMM,                    	///<WDR mode on/off, data type #SEL_WDR
	IPL_SEL_WDR_M_LEVEL_IMM,              	///<WDR Level, range = [0, 128]
	IPL_SEL_DEFOG_IMM,                     	///<DEFOG mode on/off, data type #SEL_DEFOG
	IPL_SEL_DEFOG_LEVEL_IMM,               	///<DEFOG Level, range = [0, 255]
	IPL_SEL_FD_IMM,                     	///<Face detection on/off, data type #SEL_FD
//Other
	IPL_USER_DEFINE1,
	IPL_USER_DEFINE2,
	IPL_SEL_FORCE_SEN_FLIP,              	///<force use sensor mirror/flip, data type #BOOL
	IPL_SEL_AD_AUTO,						///< auto detect, data type #SEL_AD_DET
	IPL_SEL_AD_FMT,							///< ad fmt, data type #SEL_AD_FMT
	IPL_SEL_MAX_CNT,
...
typedef enum _SEL_RAW_BITDEPTH {
	...
	SEL_RAW_BITDEPTH_MAX_CNT = SEL_RAW_BITDEPTH_16 + 1,
...
typedef enum _SEL_SHDR_FRM_NUM {
	...
	SEL_SHDR_FRAME_2 = 2,
	SEL_SHDR_FRAME_MAX_CNT = SEL_SHDR_FRAME_2+1,
...
/**
   AD auto detect on/off
*/
typedef enum {
	SEL_AD_DET_AUTO = 0,
	SEL_AD_DET_MANUAL,
	SEL_AD_DET_MAX_CNT,
	ENUM_DUMMY4WORD(SEL_AD_DET)
} SEL_AD_DET;
...
*/
typedef enum {
	SEL_AD_FMT_POGRESSIVE = 0,      ///< pogressive mode
	SEL_AD_FMT_INTERLACED,			///< interlaced mode
	SEL_AD_FMT_MAX_CNT,
	ENUM_DUMMY4WORD(SEL_AD_FMT)
} SEL_AD_FMT;

/**
    ime path output push type
*/
typedef enum _SEL_PUSH_TYPE {
	SEL_PUSH_TYPE_SEQ = 0,    ///<-push from ime path1
	SEL_PUSH_TYPE_SORT,       ///<-push from small image size after sort
	SEL_PUSH_TYPE_MAX_CNT,
	ENUM_DUMMY4WORD(SEL_PUSH_TYPE)
} SEL_PUSH_TYPE;
