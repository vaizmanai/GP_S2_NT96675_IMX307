...
typedef struct _IPLC_RHEIME {
	...
	struct {
		UINT32            proc_frame_cnt;  ///< desired setting channel, ex: (RHE_HAL_INPATH_1 | RHE_HAL_INPATH_2)
		UINT32            subout_addr;
...
/**
     ...
     @code
     {
        ...
        IPL_Ctrl_Runtime_Chg(IPC_Chg_ImeP2Out,(void*)&IPC_IMEOut_Info);
        IPL_Ctrl_Runtime_Chg(IPC_Chg_Crop,(void*)&IPC_IMEOut_Info);
...
/**
    ...
    @param[in] img_info:    input image information
    @param[in] header_addr: Raw header buffer address
*/
extern ER IPL_Ctrl_Virtual_Trigger(IPL_PROC_ID id, IPL_RAW_IMG *img_info, UINT32 header_addr);
