...
typedef struct _AE_SHDR_EXPY {
	UINT32 curr_se_lum;
} AE_SHDR_EXPY;

typedef struct _AE_SHDR_OVEREXP {
	AE_BOUNDARY hist_cnt;
	AE_BOUNDARY curr_range_cnt;
	AE_BOUNDARY curr_range_weight;
	AE_BOUNDARY curr_range_ratio;
...
typedef struct _AE_SHDR_OPT_EXPY {
	UINT32 expy_se;
	UINT32 smooth;
...
typedef struct _AE_SHDR_OPT_OVEREXP {
	UINT32 thr;
	UINT32 hist_expy;
} AE_SHDR_OPT_OVEREXP;
...
typedef struct _AE_SHDR {
	...
	AE_SHDR_OPT_EXPY opt_expy;
	AE_SHDR_OPT_OVEREXP opt_overexp;
	UINT32 tab_ev_ratio_min[AEALG_DYNAMIC_LV_NUM];
	UINT32 tab_ev_ratio_max[AEALG_DYNAMIC_LV_NUM];
	UINT32 tab_ycurve_weight_min[AEALG_DYNAMIC_LV_NUM];
	UINT32 tab_ycurve_weight_max[AEALG_DYNAMIC_LV_NUM];
	UINT32 reserve0;
	UINT32 reserve1;
} AE_SHDR;
...
INT32 AEAlg_OP_Intpl(INT32 idx, INT32 value_l, INT32 value_h, INT32 idx_l, INT32 idx_h);
