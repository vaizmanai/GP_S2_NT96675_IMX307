...
typedef int NvtUctrl_NoIPC_CmdCB(UINT32 CmdId,void *data, int dataSize,int *pOutRet);
...
#if USE_IPC
#define NVTUCTRL_PARAM_BUF_SIZE      (NVTUCTRL_BUF_CHK_SIZE*2+NVT_UCTRL_RAW_SIZE+(NVT_UCTRL_CMD_SIZE+4)) ///<shared memory size
#else
#define NVTUCTRL_PARAM_BUF_SIZE      (NVTUCTRL_BUF_CHK_SIZE*2+NVT_UCTRL_RAW_SIZE+(NVT_UCTRL_CMD_SIZE+4)+NVT_UCTRL_CFG_SIZE) ///<shared memory size
#endif
...
#if (!USE_IPC)
extern void NvtUctrl_NoIPC_RegCB(NvtUctrl_NoIPC_CmdCB *pCB);
#endif
