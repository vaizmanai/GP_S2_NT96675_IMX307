...
copylinkfile()
{
	...
	link_source_len=${#link_source}
	if [ ! -z "$issysrootfile" ] ; then
		sysroot_len=${#SYSROOT_PATH}
	else
		sysroot_len=${#LIBRARY_DIR}
		sysroot_len=`expr $sysroot_len - 4`
	fi
	sysroot_len=`expr $sysroot_len + 1`
	source_releative_path=`echo ${link_source} | cut -c $sysroot_len-$link_source_len`
	#echo linksoure $link_source
	#echo link_source_len $link_source_len
	#echo sysroot_len $sysroot_len
...
copyso()
{
	...
	if [ "$2" != "necessary" ] ; then
		...
		filelist="$(readelf -l $1 | grep "Requesting program interpreter" | awk -F'Requesting program interpreter: ' '{print $NF}' | sed 's/\[//g' | sed 's/\]//g') \
			$(readelf -d $1 | grep "Shared library" | awk -F'Shared library: ' '{print $$NF}' | sed 's/\[//g' | sed 's/\]//g')"
	else
		filelist=$1
	fi
	#copysoReturnList=""
...
copyNecessaryso()
{
	#search lib use shared library  - layer2
	#search lib file except link file
	glibc=$(echo ${CROSS_COMPILE} | grep 'glibc')
	#echo glibc = $glibc
	if [ ! -z "$glibc" ]; then
