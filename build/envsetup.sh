...
DEFAULT_NVT_CROSS=mipsel-24kec-linux-uclibc-4.9-2017.07
...
function set_stuff_for_environment()
{
	...
	export ECOS_DIR=${LINUX_BUILD_TOP}/ecos-kit
...
function setpaths()
{
	...
	if [ -z "$NVT_CROSS" ] || [ "$NVT_CROSS" == "mipsel-24kec-linux-glibc-4.9-2017.08" ]; then
		export NVT_CROSS=mipsel-24kec-linux-glibc-4.9-2017.08		
		...
		export NVT_HOST=mipsel-24kec-linux-gnu
		export SYSROOT_PATH=${CROSS_TOOLCHAIN_PATH}/usr/${NVT_HOST}/sysroot
		export CROSS_COMPILE=${CROSS_TOOLCHAIN_BIN_PATH}/${NVT_HOST}-
	elif [ "$NVT_CROSS" == "mipsel-24kec-linux-uclibc-4.8-2016.11" ]; then
		export NVT_CROSS=mipsel-24kec-linux-uclibc-4.8-2016.11
		...
		export NVT_HOST=mipsel-24kec-linux-uclibc
		export SYSROOT_PATH=${CROSS_TOOLCHAIN_PATH}/${NVT_HOST}/sysroot
		export CROSS_COMPILE=${CROSS_TOOLCHAIN_BIN_PATH}/${NVT_HOST}-
	else
		export NVT_CROSS=mipsel-24kec-linux-uclibc-4.9-2017.07
		if [ -d /opt/ivot/$NVT_CROSS ]; then
			export CROSS_TOOLCHAIN_PATH=/opt/ivot/${NVT_CROSS};
		else
			export CROSS_TOOLCHAIN_PATH=/opt/${NVT_CROSS};
	...
	#----------------------------------------------------------------------
	export PLATFORM_CFLAGS="-mdsp -EL -msoft-float -march=24kec -mtune=24kec -fno-builtin -fno-common"
...
function lunch()
{
	...
	# To configure your selected project type to uitron codebase
	sed -i '/ACTIVE_PROJECT =/d' ${UITRON_DIR}/Project/ProjectConfig.txt
	echo ACTIVE_PROJECT = `echo ${selection}` >> ${UITRON_DIR}/Project/ProjectConfig.txt
...
add_lunch_toolchain_combo mipsel-24kec-linux-glibc-4.9-2017.08
#add_lunch_toolchain_combo mipsel-24kec-linux-uclibc-4.8-2016.11
add_lunch_toolchain_combo mipsel-24kec-linux-uclibc-4.9-2017.07
