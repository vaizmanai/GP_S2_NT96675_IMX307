...
make_dep_by_modelselect = \
			...
			if [ ! -z "$(modelcfg_diff)" ]; then echo -e "\e[1;33mChanged model config items >>> $(modelcfg_diff) \e[0m"; else echo -e "\e[1;33mModel config without any change\e[0m"; fi; \
...
rm_nvt_pack = \
		NVT_BIN_NAME=FW`cat $(NVT_PRJCFG_MODEL_CFG) | grep -E '(^|\s)CHIP($|\s)' | grep -v FW | grep -v CFG | grep = | awk -F'= ' '{print $$NF}'`A; \
...
gen_linux_loadbase = \
	MIPS_COMPILER=`echo $${NVT_CROSS} | grep "mips"`	;\
	if [ -n "$${MIPS_COMPILER}" ] && [ "$${NVT_PRJCFG_CFG}" != "EmuKit" ]; then	\
		DSP1_CFG=`cat $(NVT_PRJCFG_MODEL_CFG) | grep "DSP1_TYPE" | grep "=" | awk -F'=' '{print $$NF}' | tr -d ' '`;	\
		if [ -n "$${DSP1_CFG}" ] && [ "$${DSP1_CFG}" == "DSP1_NONE" ]; then	\
			declare -i DSP1_ADDR=`cat $(NVT_PRJCFG_MODEL_CFG) | grep "BOARD_DSP1_ADDR" | awk -F"=" '{print $$2}'`;	\
			declare -i DSP1_SIZE=`cat $(NVT_PRJCFG_MODEL_CFG) | grep "BOARD_DSP1_SIZE" | awk -F"=" '{print $$2}'`;	\
			declare -i LINUX_ADDR=`cat $(NVT_PRJCFG_MODEL_CFG) | grep "BOARD_LINUX_ADDR" | awk -F"=" '{print $$2}'`;	\
			declare -i DSP1_END_ADDR=$${DSP1_ADDR}+$${DSP1_SIZE};	\
			if [ $${DSP1_END_ADDR} == $${LINUX_ADDR} ]; then	\
				LINUX_KERNEL_LOAD_ADDR=0xffffffff8`cat $(NVT_PRJCFG_MODEL_CFG) | grep "BOARD_DSP1_ADDR" | awk -F"0x0" '{print $$2}'`;	\
			else \
				LINUX_KERNEL_LOAD_ADDR=0xffffffff8`cat $(NVT_PRJCFG_MODEL_CFG) | grep "BOARD_LINUX_ADDR" | awk -F"0x0" '{print $$2}'`;	\
			fi; \
		else \
			LINUX_KERNEL_LOAD_ADDR=0xffffffff8`cat $(NVT_PRJCFG_MODEL_CFG) | grep "BOARD_LINUX_ADDR" | awk -F"0x0" '{print $$2}'`;	\
		fi;	\
		echo $${LINUX_KERNEL_LOAD_ADDR};	\
	else \
